...
The C33 representation follows D091: base-9 column-per-row integer codes, sorted complete lists chunked at 1,024 entries, exact one-level conjunction packs of at most 27 source facts, and no new proposition kind. Verify 81 exact domains (including authentic original-positive singleton sources), 27 canonical classic scopes, nine row covers per symbol and all explicit original/derived placement anchors without extra or duplicate sources. Three domain packs and one scope pack precede ceil(9 * selected-symbol count / 27) cover packs; additional nonempty packs of at most 27 carry canonical positive anchors, for at most ten premises. Duplicate rejection is per source category; the same given literal may establish both its domain slot and its anchor slot. Single/pair/triple overlay uses one/two/three symbols; pairwise incompatibility supports selected sets of two through nine symbols, including full-nine-symbol discovery, without enumerating nine-symbol completion products. Template-specific descriptor bounds expose both symbol maxima, 46,656 templates per symbol and 100,000 tuple tests. The complete relation uses candidate masks plus those anchors; an unresolved singleton is not implicitly placed. Check exact anchor membership against authentic current values/domains and original clues, and inherit each checked placement root's provenance and taint. The 64-premise and 16 KiB node caps remain binding; reserve decoded/flattened scratch before allocation. A nonempty complete negative conjunction is projected through ordinary `conjunction@1` and `domain-restrict@1`; every effect root must reach its exact matching certificate. No-effect C33 discovery creates no cache proposal.
