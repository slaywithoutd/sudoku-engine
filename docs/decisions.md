...
All 38 interview questions are answered. The expanded M2 specification, concrete contracts, bounded technique matrix and complete 27-task plan were approved for implementation under D065/D066; implementation is in progress. Explain is the default of two modes, and Perfect excludes uniqueness-dependent paths. No new product question blocks the approved implementation. Cost-table calibration, phase share, resource defaults and rollout benefit have explicit future benchmark gates, not measured answers. Historical entries retain their original context; subsequent decisions and approved specifications supersede earlier details. Other later-phase questions remain deferred under D044.
...
- Benchmark interpretation refinement, 2026-09-14: keep alternate proofs and known clue/coordinate variants of one source puzzle in one calibration or held-out group; record group IDs, normalized input hashes and distinct counts. Preserve timed-out trials in outcome/deadline metrics and label completion-conditioned latency separately. Several acceptance records share original puzzles, so fixture counts alone cannot support independent-sample performance claims. This refines T26 methodology and supplies no measurement or default validation.
...
- Status: Technical fixture clarification for T11, 2026-09-13; implemented and independently reviewed through `ed7ea09`. No named family or the 24-link cap is removed.
...
- Status: Technical graph representation clarification for T11, 2026-09-13; implemented and independently reviewed through `ed7ea09`.
...
## D081 — Make generalized-chain positions and terminal roles explicit

- Status: Technical grammar clarification for T15, 2026-09-13; implementation and independent review complete through `53776c3`. No generalized family is removed, and the recorded bound remains 12 pair positions.
- Count every CSP-variable position once, including a terminal position whose right candidate is absent. Thus an eleven-pair prefix plus a final no-right position reaches twelve; twelve full pairs closed by the final right candidate's conflict with the target also reaches twelve. Do not append an uncounted thirteenth variable. Record full-pair count, terminal form and total positions separately. This follows the length conventions in the [author's definitions](https://arxiv.org/pdf/1304.3210v1) and the [pinned whip closure](https://raw.githubusercontent.com/denis-berthier/CSP-Rules-V2.1/7eefd71566b37e55c15f330d474140bfc4f4371d/CSP-Rules-Generic/CHAIN-RULES-MEMORY/WHIPS/Whips%5B3%5D.clp).
- Distinguish the left candidate's continuity conflict from exclusions of extra alternatives. For the t-only profile, extra alternatives use earlier right candidates; the first position therefore remains bivalue. A terminal position may close through one explicitly designated remaining candidate conflicting with the target, while all other non-left alternatives use earlier rights. It may also close with no compatible alternative already remaining. Every alternative needs its actual permitted conflict witness; a terminal role cannot authorize target-dependent extras in an interior position.
- The [pinned t-whip construction](https://raw.githubusercontent.com/denis-berthier/CSP-Rules-V2.1/7eefd71566b37e55c15f330d474140bfc4f4371d/CSP-Rules-Generic/CHAIN-RULES-MEMORY/T-WHIPS/T-Whips%5B12%5D.clp) closes through the target and permits a more general target-restricted initial seed. The engine keeps its declared t-only extra policy: a start needing additional target-dependent exclusions uses the general whip grammar. Document this finite-profile distinction rather than claiming every source extension under the narrower alias. z extras remain target-only; whips allow target/earlier-right witnesses; braids additionally relax predecessor continuity.
- T15 must test productive twelve-position certificates, a thirteenth terminal rejection, t-only terminal closure, and a general-whip certificate rejected when relabeled t-only because an interior extra depends on the target. Source rule scheduling and earlier-rule exhaustion are never proof premises in this engine; reconstruct every surviving/excluded alternative explicitly.
## D082 — Count Multi-coloring's two lexical case assumptions

- Status: Technical descriptor correction during T11 implementation, 2026-09-13; implemented and independently reviewed through `ed7ea09`. The C14 matrix still permits two conjugate components and four joint color assignments.
- Set C14's common `maxBranchDepth` projection to 2. A checked two-alternative cover for each component gives four leaves through nested `cases@1`; those two lexical assumptions implement the existing four-assignment requirement. Single-component C14 proofs and C15 Medusa need one level, and C15 retains depth 1.
- Both scopes must be discharged. Each effect root must contain its complete claimed two/four-assignment proof; a simpler valid root cannot use an unused second component as explanatory decoration. An effect also derivable by a simpler technique remains admissible when its complete declared proof is actually supplied.
- This change does not enable recursive forcing discovery, extra components, more than four color leaves or deeper coloring assumptions. Test descriptor/registry agreement and the exact branch lineage. The cost of a mistaken projection is unnecessary rejection or excess scheduling allowance; independent primitive and named grammar checks continue to enforce correctness and shape.

## D083 — Preserve the canonical quadruple Fireworks form

- Status: Technical C29 grammar clarification for T16, 2026-09-13; implemented at `f62a47d`, independently reviewed through `1d9a36e`. Existing four-cell/four-symbol and complete joint-proof bounds remain unchanged.
...
- Status: Technical C20 grammar clarification for T13, 2026-09-13; implemented and independently reviewed through `fea0441`. Existing selected-cell, side-ALS and symbol bounds remain unchanged.
...
- With selected cells `C`, their symbol union `V`, and side unions `VL`/`VB`, require `sideCells = |V| - |C| + |VL \ V| + |VB \ V|`. Each side is an exact deficit-one ALS. Thus `|V \ (VL union VB)| = |C| - 2`; this does not require `|V| = |C| + 2`. Shared outside symbols contribute once per side.
...
## D085 — Preserve multi-symbol subset counting

- Status: Technical C21 grammar clarification for T13, 2026-09-13; implemented and independently reviewed through `fea0441`. No requested family is removed.
- Subset Counting includes the Extended Subset Principle: bound each digit's occupancy in a selected cell set, add the bounds, and reject a candidate whose consequences leave insufficient total capacity. This follows the [primary SudoCue explanation](https://www.sudocue.net/guide.php), inspected 2026-09-13. A single-symbol cover/count inequality alone cannot stand in for this named family.
- The finite profile uses 1…12 counted cells, 1…4 distinct cited all-different scopes and the exact original union of 1…9 symbols. An external target is the effect location, outside the counted-cell bound. Reduce scopes to relevant incidence through checked subset facts. Include exact selected and target domains; reconstruct any restrictions from a positive target assumption and cited conflicts. Zero capacities after restriction remain part of the sum.
- The reviewed `subset-count@1` checks every occupancy subset for each symbol (at most 4,096 per symbol), derives the maximum compatible count, and concludes contradiction only when their sum is smaller than the selected cell count. Work is cooperative and charged. Discharge the target assumption before any applied removal; preserve all other assumptions and uniqueness taint. Every named effect root needs its complete count certificate.
- Counted cells may include authenticated given or derived singleton domains; the target remains unresolved. This engine arithmetic profile is broader than the author's advice to search empty cells. Label singleton padding in maximum-bound fixtures explicitly: it verifies admission/accounting at twelve cells, not essential twelve-cell reasoning or measured discovery performance.
- Existing `cover-count@1` positive coefficient bounds of 1…81 remain unchanged. The matrix's symbol range is not a coefficient limit. Any separately advertised weighted-incidence discovery form must state the weight combinations it actually enumerates. Primitive acceptance of a weight does not establish exhaustive discovery coverage.
- T13 must verify productive multi-symbol counting, zero capacities, missing symbols, understated capacities, overlapping scopes, every-root substitution and independent original-clue replay. No whole-board solve result or unexplained table replaces the capacity proof.


## D086 — Confine hypothetical candidate publications to branch sessions

- Status: Technical authority refinement for T14, 2026-09-13; implemented in `a23e517`, corrected in `7f1c374`, with independent task and fix reviews passed. No new primary proof issuer or extra assumption depth is authorized.
- A hypothetical session may fork an authentic published view, copy candidate/value buffers and indexes, and share immutable accepted prefix objects. The engine issues branch identity; a caller label is display metadata. Authority binds the exact parent/source view, exact proof-node objects and actual lexical assumption ancestry, never numeric IDs or supplied scope arrays alone.
- Require an explicit caller-owned resource budget/lease before fork buffers, indexes or prefix bookkeeping are allocated. Sessions dispose reservations on success, cancellation, interruption and exceptions. There is no unbudgeted public fork default. Read-only ancestry helpers expose only immutable facts or boolean checks; authenticate view identity before inspecting payloads, including Proxy inputs.
- The confined session introduces assumptions through checked primitive rules and admits checked local primitive or C01-C05 bundles with a separately branded BranchCertificate. Ordinary proof/certificate admission, commitChecked and retainCheckedFacts reject every hypothetical publication, including assumption-free forks. Only the owning session publishes branch facts and candidate changes atomically. A checked empty domain ends in contradiction; it cannot become an inconsistent usable view.
- Branch discovery may use exact proved domains/facts from its lexical ancestors and own publication. Sibling and child-to-parent imports are rejected. The primary index source policy remains closed-only. The maximum nesting depth of two counts the actual open lexical assumption stack; session labels do not reset it.
- Branch verification binds the complete retained proof prefix to the exact source publication. A caller-supplied subset cannot reset node allocation or hide an existing ID. Publication independently rejects every reused node ID before applying any change. Rejected admission preserves the prior published view, facts, domains and resource accounting. T14 independent review reproduced a shortened-prefix collision; `7f1c374` adds its preservation regression and corrective checks, and independent fix review passed.
- A completed parent deduction replays its complete DAG through ordinary admission and discharges all branch assumptions. Import/remapping must avoid sibling ID collisions and rewrite every referenced node ID, including scope, primitive parameter and named-pattern references; copied shape is not inherited authority. BranchCertificate cannot commit a primary deduction or establish solution-count evidence.
- T14 must test immutable parent sharing, forged views rejected before field access, sibling/ancestor boundaries, branch certificates rejected by ordinary admission, checked dynamic-domain link use, empty-domain atomicity and all disposal exits. T19-T21 must retain this ownership/accounting distinction when scheduling and transporting completed proofs. A mistaken boundary design would permit speculative state to acquire primary authority, so independent review is a release gate.


## D087 — Give OR forcing and inserted OR whips distinct named grammars

- Status: Technical C28 source-mapping clarification for T15, 2026-09-13; implemented in `5d46367`, corrected in `53776c3`, with independent task and fix reviews passed. Both requested forms remain required.
- The pinned primary rules distinguish [OR forcing](https://raw.githubusercontent.com/denis-berthier/CSP-Rules-V2.1/7eefd71566b37e55c15f330d474140bfc4f4371d/CSP-Rules-Generic/CHAIN-RULES-EXOTIC/OR3-FORCING-WHIPS/OR3-Forcing-Whips%5B3%5D.clp) from an [OR relation inserted into a whip](https://raw.githubusercontent.com/denis-berthier/CSP-Rules-V2.1/7eefd71566b37e55c15f330d474140bfc4f4371d/CSP-Rules-Generic/CHAIN-RULES-EXOTIC/PARTIAL-OR3-WHIPS/Partial-OR3-Whips%5B2%5D.clp). The previous matrix assigned one exhaustive-case grammar to both names. Do not infer one name's coverage from the other.
- Keep OR forcing unchanged: a complete already-proved clause with 2...4 signed alternatives (positive or negative literals), a checked same-effect branch for every alternative, per-branch generalized bound of twelve positions or static chain bound of twenty-four links, and no recursive case nesting. Record individual branch lengths; source total-length rankings are not this engine's per-branch bound.
- Add the explicit inserted-OR-whip specialization within C28: one scalar continuous whip with exactly one already-proved positive-candidate OR clause of 2...4 alternatives in its ordered prefix/OR/suffix structure. At the OR position, a designated left alternative conflicts with the immediately preceding right (or target for an empty prefix); each other rejected OR alternative conflicts with the target or a preceding right. Exactly one OR alternative remains as its right. It need not be exclusive with other alternatives: the clause supplies OR only. Ordinary positions use the C26 general-whip alternative and continuity checks, including the OR right among preceding rights for suffix exclusions.
- The engine's finite bound for this specialization is twelve total positions, counting the OR position once and any no-right terminal once. A terminal contradiction may instead close through the last scalar right conflicting with the target, as D081 permits. A twelve-position prefix may not append an uncounted terminal. There is no second OR insertion, grouped OR insertion or recursive forcing extension in this profile. This resolves an unspecified representation, not a claim about all source OR-whip extensions.
- Compile the inserted form directly from one target assumption through the prefix, complete OR reduction and suffix, then discharge it. It has one lexical assumption level; adding exhaustive outer OR cases and a second target assumption merely to reuse a forcing compiler is unnecessary. Each applied root must reach the actual ordered OR proof. Generic OR case forcing cannot be relabeled inserted-OR-whip, and an unrelated retained clause is not explanatory evidence.
- T15 must provide independent productive inserted OR2/3/4 fixtures in addition to forcing OR2/3/4, actual discovery of both forms, complete-clause/source identity tests, prefix/suffix continuity failures, missing OR alternatives, a twelve-position admitted certificate and a thirteenth-position rejection. The [source terminal rule](https://raw.githubusercontent.com/denis-berthier/CSP-Rules-V2.1/7eefd71566b37e55c15f330d474140bfc4f4371d/CSP-Rules-Generic/CHAIN-RULES-EXOTIC/OR3-WHIPS/OR3-Whips%5B3%5D.clp) is a mathematical reference, not a scheduler-exhaustion proof premise. Independent checking must reconstruct all exclusions.


## D088 - Retain complete named proofs without applying their effects

- Status: Technical T15 source-publication refinement, 2026-09-13; implemented in `5d46367`, corrected in `53776c3`, with independent task and fix reviews passed. Uses existing checked-step and retained-fact authority, with no new primitive or trusted issuer.
- C22 and C25-C27 may have an explicitly marked certificate-only form: `effects=[]`, exactly one proved negative-target theorem root, and the complete existing named certificate. Preserve every case, alternative, ordered position, bound, source and causal root check. Discharge temporary assumptions and preserve inherited uniqueness dependence. This approval does not add positive-placement caches or cache forms for C23/C24.
- C28 needs authentic retained clauses without applying a source deduction. The independent OR3/OR4 fixtures use whole cell covers that participate in complete C22 case proofs; a single contradictory branch does not prove or causally use the full split cover. Complete named cache proofs allow these sources through the established same-revision retention path. They do not authorize a cover-only shell, an arbitrary fact root or an unchecked source registration.
- Prune unused effect/domain closure. Closed clauses within the complete proof retain their exact source identities and their own checked derivations; a consumed clause cannot depend on the later consumer's target-result root. Replay both the source publication and consumer from original clues. Construction grids and exact witnesses never become logical premises.
- Fact retention preserves candidate values, domains and revision while consuming retained proof/workspace budget. It invalidates clause-index completeness through the changed fact prefix. Source-bound checked-step admission rejects stale or repeated certificates as specified by the retainer. Proof-only publication never counts as candidate progress; T19 must deduplicate equivalent cached facts and prevent repeated no-progress work.
- T15 must test real source retention/consumption, malformed or substituted theorem roots, missing positions/cases, lexical scope and conditional taint, same-revision invalidation, unchanged candidate state, resource cleanup, original-clue replay, and ordinary effectful C22/C25-C27 behavior. Independent review must verify that the new cache form preserves the complete named proof contract.


## D089 - Join complete proof tables with checked filters

- Status: Technical T16 relational-algebra refinement, 2026-09-14; implemented at `f62a47d`, independently reviewed through `1d9a36e`. This adds no specialized-pattern truth issuer.
- Add `table-join-filter@1`: the first two premises are authenticated complete relations/tables; subsequent premises are checked all-different scopes or canonical clauses. Parameters are empty. Every constraint cell, including every alternative of a clause, must be in the sorted union of the two input cell sets. Never silently trim an external clause alternative.
- Enumerate shared-cell-compatible row pairs, apply every cited constraint, and reconstruct the exact surviving count and authenticated defining-node identity. Preserve every premise's rules, assumptions and conditional taint. The definition records the two children and all filters; later projection/replay recomputes the same filtered rows. A copied summary grants no authority.
- Retain the existing sixteen-cell and sixty-four-definition-depth bounds, node/proof byte limits, and cooperative work/resource accounting. Charge rejected pairs and each filter/literal traversal too. Reserve/check scratch before new maps, row buffers or definition storage; include nested iterator frames. Do not retain an uncharged expanded row cache.
- Ordinary `table-join@1` and existing leaf-filter semantics remain unchanged. Partition union must explicitly reject both join operations. Complete empty results may entail logical clauses but do not establish independent solution-count evidence.
- The operation avoids constructing unfiltered combinations between SK groups before testing their shared restrictions. C29-C32 still need complete independent component/geometry and every-root named certificates. Test exact counts, overlaps, malformed/incomplete/copied sources, outside-cell filters, scope/taint, forbidden partition union and interrupted checking against independent relational algebra. Check that new primitive IDs do not silently exceed the existing forcing-branch import/remapping contract.

## D090 - Derive signed clauses from complete count inequalities

- Status: Technical T16 count-algebra refinement, 2026-09-14; implemented at `f62a47d`, independently reviewed through `1d9a36e`. Existing `cover-count@1` behavior remains unchanged.
- Add `cover-count-clause@1` with the same explicit symbol, positive weighted cover/capacity premises and coefficients of 1 through 81. Reconstruct every incidence in `a = upper - lower` and the bound `U - L`, using checked safe-integer arithmetic. Consume/classify every premise; do not infer disjoint scopes or omit negative incidences.
- The conclusion is one canonical clause of two through sixty-four distinct signed literals, all using the counted symbol and cells in the incidence union. Require exact proved domain evidence for every nonzero-incidence cell and every clause cell. Reject missing, duplicate or unrelated domain evidence, other-symbol alternatives and complementary literal pairs.
- Falsify all alternatives: a negative literal fixes its Boolean occupancy to one; a positive literal fixes it to zero. Check these fixings against the proved domains, including counted-symbol singletons and absent candidates. For the nonempty compatible set of unary assignments, sum each term's exact minimum. Accept only when that minimum exceeds `U - L`.
- This version rejects incompatible falsification rather than labeling an already true clause as a nonvacuous count proof. An existing checked domain/table entailment may handle such a case separately. No guessed solution, search exhaustion or uniqueness evidence supplies a count premise. Inherited rule provenance, scopes and conditional taint remain intact.
- Junior Exocet needs a base-to-target clause. Three cross-line covers and the verified S/base capacities give an inequality of the form `2 * baseTruth - target0 - target1 <= 1` after accounting for the other incidences. Falsifying `not(baseTruth) OR target0 OR target1` violates that inequality. The named checker must reconstruct the actual complete component; this example is not a trusted pattern rule.
- Use a cooperative strategy with accounted scratch before allocation and work charged per scope, cell and literal. Independently test signed incidences, weights/overlap, forced and absent values, falsification compatibility, altered counts/domains, scope/taint and resource exits, alongside unchanged ordinary count behavior. Shared arithmetic helpers may be factored without changing prior semantics; independent acceptance algebra cannot reuse them.


## D091 - Complete template certificates with bounded recomputation

- Status: Technical T17 representation refinement, 2026-09-14; implemented at `414b5f7`, independently reviewed through `2fec0e0`. This resolves the approved C33 contract without adding a proposition kind or widening ordinary table bounds.
- `template-cover@1` carries a mode, selected symbols and canonical sorted complete per-symbol template lists. Each template is a base-9 integer encoding the selected column in each of nine rows, decoded into cells; it is not a reinterpreted `SymbolId`. Source lists use chunks of at most 1,024 integers and remain subject to the actual 16 KiB node and configured total-proof byte caps. The independent checker enumerates all 9! column permutations, checks box occupancy and exact domains, and compares the complete lists before recomputing the requested single, pair, triple or synchronous pairwise-incompatibility fixed point. An unfinished list, product or fixed point proves no absence.
- Preserve the existing 64-premise limit by packing authentic source facts into ordinary conjunctions with at most 27 terms. Flatten exactly one level and verify the exact canonical source set: 81 exact domains, 27 actual classic row/column/box all-different scopes, nine row covers per selected symbol and canonical positive anchors for every current placed value. Three domain packs and one scope pack are followed by ceil(9 * selected-symbol count / 27) cover packs; additional packs of at most 27 hold the nonempty sorted anchor set. The maximum nine-symbol case therefore uses at most ten premises. An original positive literal can establish its singleton domain and its anchor; a derived placement cites its actual checked positive root with inherited provenance and taint. Reject absent, extra, duplicate-within-category or nested-pack sources, and reconstruct geometry rather than trusting familiar house IDs. The same authentic given literal may intentionally serve both its domain slot and its anchor slot. Reserve workspace before allocation, flattening and decoding, and charge all actual enumeration and comparison work.
- Single/pair/triple overlay selects exactly one/two/three symbols. Pairwise incompatibility selects two through nine symbols, including full-nine-symbol discovery, under the shared tuple limit. It computes pairwise consistency to a fixed point, never nine-symbol Cartesian completion enumeration. Descriptor template bounds expose these distinct maxima, 46,656 templates per symbol and 100,000 tuple tests. The original three-symbol two-round fixture and a separate nine-symbol/four-round fixture both remain acceptance requirements.
- Template-list completeness is defined over the candidate-mask and explicit-anchor abstraction: each selected cell admits the digit, every row/column/box contains it once, and every original given or checked placement of that digit is selected. An unplaced singleton domain is a candidate-mask restriction, not an implicit applied placement or an extra anchor. This is a sound necessary relation for full solutions, not enumeration of full solution grids. The checker verifies the exact anchor set against the authentic current values/domains, including original-clue consistency; absent, extra, duplicate or forged anchors fail. Preserve the independently authored list expectations and test both unresolved singleton and derived-placement cases explicitly.
- The certificate concludes the canonical nonempty conjunction of every current unplaced unsupported negative candidate for that mode and symbol set. Ordinary indexed conjunction elimination produces each effect root, followed by domain restriction. Each supplied negative effect root must directly project its matching new template certificate, including exact mode/symbol/list equality and inherited scopes, rules and conditional taint. An unrelated valid deduction cannot decorate a template explanation. No-effect discovery emits no proposal; this adds no proof-only cache issuer.
- An explicit operation-owned `TemplateOperationContext` is passed through `DiscoveryContext.templates`. Missing context is a configuration error before work or allocation, not a missing-premise exclusion or exhausted search. Its monotone `advance(view, charge)` and `consumeTuple(view)` bind actual owned views, assembly, problem and branch; reject lower, replayed or cross-operation transitions. Same-candidate-revision proof-prefix updates and detector restarts never replenish the shared allowance. Runtime integration must preserve this ownership rather than install a module-global quota. A read-only authenticated accepted-lineage descendant query may validate advancement, with ancestry traversal charged through the caller; expose no owner or registration authority and preserve cold rebuild equivalence.
- The 100,000 tuple-test allowance applies to discovery/exploration across all C33 modes and restarts at one candidate revision. Independent proof recomputation separately obeys the same per-certificate semantic bound and charges actual verifier work; it does not consume or replenish discovery's counter. Geometry enumeration, tuple exploration and proof verification remain separately accounted within overall limits. Empty-grid indexes may contain all 46,656 legal templates while an overlarge wire certificate honestly interrupts at its caps. This is a representation capability, not evidence that proposed time or memory defaults are sufficient.
- Rationale: complete recomputation supplies checkable absence evidence while retaining current proposition, codec and ordinary-table semantics. Cost if wrong: revise the compact certificate or its operation-context integration and repeat proof, resource and replay acceptance; do not relax completeness or silently narrow C33.
- Review refinement: closed conditional source facts remain available to C33; open assumptions remain excluded. Primitive tests cover inherited taint, scopes and rule provenance. Reviewed T18 now covers actual uniqueness-derived placement through C33 compilation, discovery, checking, conditional commit and original replay, including primary admission and Perfect refusal. T25 retains browser integration acceptance. No synthetic source issuer substitutes for that gate.


## D092 - Conditional authority, replay and worker bootstrap

- Status: Implemented in `a2f7a53`, corrected in `5a19be1`, with independent task and fix reviews passed on 2026-09-14. User implementation authorization remains D065/D066.
- `acceptedUniqueParent(count, context, quarantined)` may issue an opaque parent handle only from the actual privately accepted unique count and authentic accepted unconditional primary path; the evidence context already contains the snapshot. Reject mismatched, quarantined or invalidated parent context. Its public metadata contains no witness digits. An arbitrary `CountEvidence` shape, evidence ID, branch label, checksum or fixture record supplies no authority. Ordinary conditional cancellation/rerun disposes that operation's capability while preserving the primary result and eligibility; parent-wide revocation is for input/primary replacement or quarantine.
- A `ConditionalOperation` owns the exact conditional RunKey, snapshot/problem, parent handle, accepted-prefix identity and active generation. `CheckContext` and `DiscoveryContext` receive its opaque local uniqueness capability. The old `uniqueEvidenceId` field is compatibility metadata only. Every conditional inference retains permanent taint; checking and candidate commit/retention revalidate active authority, including inherited C28/C33 consequences. A checked step cannot outlive revocation or cross to an independently initialized owner with the same labels.
- Keep ordinary primary replay unchanged. Conditional replay starts a fresh owner, deterministically rebinds only the unconditional proposals' state/branch fields, and rechecks every prefix bundle before conditional work. It cannot import a primary `CheckedStep` token. Bind canonical prefix digest and full run/snapshot/problem/parent identity, retain the actual accepted prefix on the main thread, and account serialization, hashing, copying, replay and retained resources. A digest checks identity; it never issues proof authority. Avoid redundant full-prefix string copies.
- Saved conditional bundles replay only under their exact bound request and branch, including on a fresh owner or worker. This does not migrate a previous conditional result to a new request ID. A conditional rerun starts from the accepted primary prefix and replaces the previous conditional result.
- A dedicated transferred `MessagePort` carries a one-use, bounded authorization grant from the authenticated main operation to a specifically installed worker bootstrap adapter. Installation is a trusted runtime entry point; normal proof/data messages cannot install or mint authority. Bind the installed port, complete expected identities, prefix digest, nonce and generation; reject duplicates, stale identities and foreign ports. Main-side acceptance retains actual parent authority and refuses revoked results without waiting for the worker to observe revocation. Close ports/listeners on cancellation, replacement, error and disposal. This is not isolation from hostile JavaScript in the same realm.
- The grant contains metadata only and obeys the 32 KiB control limit. No solution digits or count witness are transported as logical premises. The accepted unconditional prefix uses bounded proof chunks of at most 64 KiB and at most two unacknowledged chunks, with accounted staging and full recheck before conditional work. A multi-MiB prefix must not become a single new port/start frame. T20-T23 implement and test the actual worker/controller transport and lifecycle; T18 implements the authority/replay adapter and its focused tests.
- Claim a grant endpoint synchronously before asynchronous hashing. A failed claimed attempt closes that endpoint and does not make it reusable; a retry uses a fresh channel. Rejecting a duplicate attempt must not close the first caller's endpoint. The receiver still independently rejects duplicate grants.
- Active lifetime and readiness are distinct. The uniqueness query requires completed prefix replay; a valid grant alone does not enable U proofs on the fresh root owner. A lifetime-only ownership query may bind and revoke prefix bookkeeping, but cannot authorize a unique transformation. Bind every conditional-operation step, including unconditional prefix steps and bundles with conditional intermediate facts, so root-only classification cannot bypass revocation.
- Bind lifetime through the actual private candidate-origin identity, including checks whose caller omits the optional capability for an unconditional proof. Read-only origin/capability lookup exposes no registrar and preserves the revoked association while its views survive. Reject explicit foreign capabilities and require target-origin/checked-step binding equality at commit or retention. This automatic lifetime lookup cannot replace the explicit ready capability for unique primitives; hypothetical views and independent same-label owners receive no authority.
- Bootstrap replay must expose a resumable phase or an explicit task-yield driver; it cannot hide a synchronous drain of the entire prefix. Lock replay before its first yield, recheck active identity and deadline after resumption, and publish readiness only after complete successful replay. Report cumulative setup/hash/replay/check work and remaining allowances through a read-only accounting interface so later discovery shares the same operation budget. Test revocation during a yielded prefix and rejection of duplicate replay starts. Browser slice targets remain unmeasured until T26.
- The concrete adapter accepts `ConditionalReplayDriver { yieldTask(): Promise<void> }`; its default yields real timer tasks every 256 replay work units. `usage`, `remainingLimits()` and `charge(units)` expose operation accounting, including inherited bootstrap work and start time. A caller must not replace the task driver with a microtask-only loop or reset allowances after the prefix.
- Geometry-only eligibility remains separate from operation authorization. Missing unique authority is an explicit disabled outcome, not exhaustion or a watched missing-geometry exclusion. Runtime consumers must preserve that distinction.
- The platform supports transferable message ports and explicit disentangling/closure; see the [HTML channel-messaging standard](https://html.spec.whatwg.org/multipage/web-messaging.html#message-ports) and [structured transfer rules](https://html.spec.whatwg.org/multipage/structured-data.html#transferable-objects), checked 2026-09-14. The authorization and bounded protocol above are project design choices, not browser-provided uniqueness verification.
- Rationale: private count membership cannot survive structured cloning, and conditional operations do not run a new exact phase. Cost if wrong: revise bootstrap/context ownership and rerun revocation, clone, prefix, worker and acceptance tests; never accept witness-shaped metadata as proof authority.

## D093 - Unique-trade preservation and per-family bounds

- Status: Implemented in `a2f7a53` and independently reviewed through `5a19be1`, 2026-09-14. Every U01-U05 form and ordinary proof/resource cap is preserved.
- `unique-transform@1` must certify a nonidentity transformation that preserves original givens and every declared rule for every possible completion satisfying its rejected core condition. Include complete current-domain/derived-placement premises and original rule handles. Avoidable transformations may change derived assignments, whose provenance remains in the conditional theorem; original clues must remain fixed.
- Actual all-different scopes admit structural multiset/support-pair preservation checks. A derived or merely necessary relation does not establish preservation of its entire declared rule. An unchanged disjoint rule scope is preserved trivially; an affected rule without an exact supported preservation certificate is explicitly unsupported. Do not infer rule equivalence from IDs, selected example assignments or a necessary capability. Mixed-rule swap rejection is required; variant implementation is outside M2.
- Keep distinct bounds for trade geometry, guardian occurrences, virtual-subset symbols and consequence links. U03 permits loops of 4 through 12 cells and at most four guardian occurrences; U04 has exactly one extra for BUG+1; U05 has two through four extras. The four-extra limit is not a global cap on rectangle roof literals. U01's four-cell/two-core geometry can require up to 28 noncore occurrences; a virtual subset has at most four distinct extra symbols, with every roof occurrence included. U02's six-cell/three-core extended geometry can require up to 36 noncore occurrences; hidden/avoidable forms retain their own exact geometry. Ordinary clauses remain limited to 64 literals.
- Within each new U consequence branch, count distinct checked inference links, at most 24 per conditional case branch or per single target-assumption discharge DAG. The sum across independent guardian case branches is not this per-branch bound. Reusing a link does not require counting each printed path copy, and duplicated unrelated links cannot hide a larger proof. This does not change existing C22/C28 path-sum grammars. A guardian clause's length is distinct from a case split's alternative count. U03's twelve-cell loop bound does not impose an unrelated twelve-link consequence limit; its consequence proofs may need one discharged assumption level. Registry/descriptor projections must match these actual dimensions.
- Preserve actual independent virtual-pair/triple/quad and BUG+4 acceptance. A genuine fourteen-cell loop with a valid conditional consequence must pass the general trade mathematics but fail the U03 named bound. Actual U03-to-C28 and uniqueness-derived-placement-to-C33 check/commit/replay pipelines require authenticated authority and permanent taint, with primary/Perfect refusal.
- Unique Rectangle Type 6 publishes the two diagonal-roof removals atomically. Each target has its own complete denial DAG, within the per-target link bound. The selected pair of parallel strong houses must participate across the two roots; requiring both in each root would demand irrelevant proof nodes. Independently check the named geometry's support restriction in both rows and both columns, even when only the parallel pair is needed in the proof. Missing either effect or its causal proof rejects the named bundle. This preserves the [Type 6 definition](https://hodoku.sourceforge.net/en/tech_ur.php), rechecked 2026-09-14, while separating geometric annotation from causal proof ancestry.
- Hidden Rectangle requires a bivalue corner opposite the target and the target's row/column strong supports; the target may itself have extra candidates. Keep independent two-roof and three-roof acceptance, including a three-roof example whose target is not bivalue. Do not replace the opposite-corner condition with a target-domain restriction.
- Rationale: copying generic placeholder bounds would silently drop approved rectangle forms, while a necessary relation could falsely certify an affected rule's symmetry. Cost if wrong: revise the finite grammar or preservation certificate and repeat original-clue, counterfactual and named-admission gates; do not narrow the requested families implicitly.

## D094 - Scheduler implementation and policy identities

- Status: implemented at `bcf4812`; independent T19 review is incomplete and paused by user request, 2026-09-14.
- `RunKey.scheduler` identifies the shared implementation, initially `scheduler@1`. The selected policy (`explain-fair@1`, `analyze-fair@1`, `fixed-scan@1` or `event-fixed@1`) is independently versioned and included in canonical `optionsKey`, alongside mode, rollout, limits and every other behavior-affecting option.
- A conditional request may change mode/policy from its primary parent without changing the implementation version. Preserve both complete run identities; matching implementation versions never substitutes for complete operation or parent binding. No additional mode restriction follows from the old generic test placeholder.
- Record implementation and policy versions separately in deterministic traces and benchmark results. Changes to frozen policy tables or implementation behavior require the corresponding version update; no online timing-based policy mutation is introduced.
- Rationale: the existing conditional parent check compares scheduler implementations, while Explain/Analyze select different policies. Cost if wrong: revise canonical option encoding and rerun determinism, stale-message and conditional mode-change acceptance tests. This changes no deduction or uniqueness authority.


## D095 - Account preparation and keep rollout ownership confined

- Status: approved technical direction implemented at `bcf4812`; independent T19 review is incomplete with reproduced findings and paused by user request.
- A successful checked step may carry a private token for its exact source publication. A read-only `checkedStepMatchesSource(step, view)` query may compare that token; only normal successful checking registers it. Do not retain a strong reference from every accepted step to its full historical ReadView, which would retain copied candidate/fact maps for the whole trace.
- A confined rollout factory may adopt one actual checked productive primary step into an isolated hypothetical owner after exact source, active lifetime, imports, scope and conditional-provenance checks. Preserve actual node/inference objects and effects; no supplied arbitrary owner, generic issuer, primary reducer, template-context advance or branch-whitelist relaxation. Published consequences must be closed and unconditional. Archived lexical assumption nodes below a discharged primary proof remain in its DAG with their exact scopes; they do not become available closed premises. An advanced discharged first candidate is permitted. Subsequent cheap steps keep the existing named C01-C05 branch gate. Confined rollout may borrow its actual immutable accepted prefix through private append layers instead of copying the entire map per continuation; owned additions and domains remain isolated, borrowing preserves source lifetimes, and the 16-step cap bounds layer growth. This optimization grants no general assumption or branch-graph publication path. Reserve and charge before copies, and always release resources even if a charging checkpoint throws.
- Include proof-check setup, hidden bookkeeping, rejection and early closure in accounting. Incrementally copy/validate retained prefixes. Seed already authenticated retained table definitions during that charged traversal rather than collect and recopy the whole table list afterward; this is equivalent to the existing registry constructor input and confers no new authority. An accessor available only after successful checking cannot account for failed checks. Resource rejection codes remain resource interruptions, not evidence of absent deductions.
- Permit a small owned source index and equivalent builder/named-checker lookup replacements to remove repeated full-prefix scans. Entries remain exact immutable facts, complete for the actual accepted prefix, with unchanged order, scopes and conditional policy. Use an explicit operation-owned `prepareSources(view, workspace)` generator and lease, prepared before discovery/checking and after an accepted prefix change; reserve and charge its traversal/copies. Do not hide eager indexing in the reducer constructor. Read-only registration is exact-view and lifetime-bound; disposing one preparation cannot invalidate another live preparation of the same view. Existing callers retain fallback semantics, but scheduled work must hold its required preparation lease. An index is not a new truth issuer. Targeted pair-incidence preparation and resumable named reconstruction may remove the identified repeated-prefix loops in coloring and pattern-conflict validation. Inner pair/tuple loops remain charged/cooperative; partial preparation cannot establish absence. A read-only per-node premise facade must preserve its captured stage cutoff rather than expose future graph nodes. Named-grammar source reads may be metered through that read-only facade to charge actual imported-ancestry visits, combined with explicit conservative bounds for finite geometry/enumeration work. A resource exception must remain an interruption even when it originates inside a named checker; it cannot become a claim that the deduction is false or the search is exhausted.
- Work reservations may hold worst-case capacity before bounded source capture, then settle actual work and release unused held capacity. Consumed work and logical service tickets are never refunded. Capture/measurement failure, cancellation and double settlement must remain bounded and release storage. A facts-only preparation level may omit unused pair incidence for C01-C05 rollout; capability completeness is explicit, so an unavailable conflict index cannot establish absence.
- Initial measurement of the conservative source model (65 units per fact) made an ordinary root exceed the entire 8,192-unit rollout cap before continuation. Implement actual-size settlement and test a genuine productive cheap continuation under the unchanged total/equal-share/16-step limits. Shared setup stays inside the allowance and is allocated deterministically. Some candidate shares may remain honestly incomplete; this does not validate rollout performance or enable it by default.
- Remaining finite synchronous sections need explicit conservative work/storage bounds, including rejection, and a recorded size for browser measurement. Debit full work; a large section is not one 256-unit quantum. A work event pays its full consumed cost before any further producer advancement; only its deterministic service debt may span several quanta. A live checker remaining-work gate may narrow its frozen local allowance so hidden work cannot consume credit already spent elsewhere. It cannot increase the local limit, refund work or grant proof authority. Split prefix-dependent loops where needed for cooperative checking. Deterministic quanta and selection windows remain independent of task slices. The 4/8 ms targets are not established by accounting and remain T26 acceptance measurements.
- Reason: runtime integration exposed repeated source scans and a success-only checker usage accessor; rollout must preserve isolation without making accepted history retain every old board map. Cost if wrong: revise source-index/accounting or confined-adoption interfaces and rerun affected ownership, scheduling and independent checker tests. No technique bounds, proof authority or product behavior is reduced.


## D096 - User-requested implementation pause and durable handoff

- Status: confirmed user instruction, 2026-09-14.
