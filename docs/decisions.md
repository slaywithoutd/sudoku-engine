...
- Status: Technical C28 source-mapping clarification for T15, 2026-09-13; implementation and independent review pending. Both requested forms remain required.
...
- Keep OR forcing unchanged: a complete already-proved clause with 2...4 alternatives, a checked same-effect branch for every alternative, per-branch generalized bound of twelve positions or static chain bound of twenty-four links, and no recursive case nesting. Record individual branch lengths; source total-length rankings are not this engine's per-branch bound.
