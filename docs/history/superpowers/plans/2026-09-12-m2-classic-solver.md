...
**Files:** create `src/solver/techniques/csp-variables.ts`, `generalized-chains.ts`, `generalized-grammar.ts`, `generalized-runtime.ts`, `or-forcing.ts`, `or-grammar.ts`, `or-runtime.ts` and `src/solver/indexes/proved-clauses.ts`; fixtures `C25.json` through `C28.json`; independent compiler `tests/solver/generalized-acceptance.ts`; test `tests/unit/solver/generalized-chains.test.ts`; `docs/solver/techniques/generalized-chains.md`. Update `forcing.ts`, `forcing-grammar.ts`, `grammar.ts`, registry/manifest, coverage tests and fixture provenance for the approved D088 retention and named-grammar integration.

**Interfaces:** `buildCspVariables(view)` yields complete cell/house-symbol alternatives with FactIds; generalized pair certificate contains variable ID, left/right literals or group, every excluded alternative and conflict reference. OR forcing consumes authentic complete signed clauses, not arbitrary lists. D087 separately requires an inserted-OR-whip grammar with a scalar C26 prefix, exactly one positive-candidate OR2...4 position and suffix, twelve total positions including OR/terminal, and one discharged target assumption. `buildProvedClauses(view, workspace)` yields a leased `ProvedClauseIndex`: bind exact publication/premises and invalidate `completeFor` on same-revision source growth while authentic existing entries remain reusable through charged `acceptsView`. Cover/relation indexes alone do not enumerate direct clauses. D088 permits explicitly marked, complete negative-target C22/C25-C27 named proofs to supply retained facts with no applied effects, through existing checked-step/retention authority. Preserve complete theorem lineage and unchanged candidates; scheduling later deduplicates cache work.
