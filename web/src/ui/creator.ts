...
export function missing(
  ...
  state.append(
    ...
    button("Open library", () => services.navigate({ screen: "library", tab: "puzzles" }), "primary"),
  );
...
export function importAsDrafts(services: ScreenServices, puzzles: ParsedPuzzle[]): string {
  ...
  services.controller.update((data) =>
    puzzles.reduce((next, puzzle, i) => createDraft(next, ids[i], services.now(), puzzle.givens, puzzle.name), data),
  );
...
export function nameField(label: string, value: string, onInput: (value: string) => void): HTMLInputElement {
...
export function mountCreator(
    ...
    dispatch(action) {
      services.controller.update((data) => {
        ...
        return editor === current.editor
          ? data
          : { ...data, drafts: { ...data.drafts, [id]: { ...current, editor, updatedAt: services.now() } } };
      });
    },
  });
  ...
  const importButton = labeledButton("upload", "Import", () =>
    openImportDialog({
      ...
      onImport: (puzzles) => services.navigate({ screen: "create", id: importAsDrafts(services, puzzles) }),
    }),
  );
  const more = menuButton(iconButton("more", "More actions", () => {}), () => {
    ...
    return [
      ...
      { label: "Analyze in solver", icon: "solve", onSelect: () => services.navigate({ screen: "solve", source: { kind: "draft", id } }) },
      "separator",
      {
        label: "Clear board",
        icon: "reset",
        ...
        onSelect: () => services.controller.update((data) => {
          const d = data.drafts[id];
          const editor = reduceEditor(context, d.editor, { type: "reset" });
          return editor === d.editor ? data : { ...data, drafts: { ...data.drafts, [id]: { ...d, editor, updatedAt: services.now() } } };
        }),
      },
    ];
  });
  const finish = labeledButton("check", "Finish", () => {
    const current = draft();
    ...
    const complete = () => {
      ...
      const d = dialog("Puzzle ready", { description: "Clues are now locked. Edit a copy to change them later." });
      let playing = false;
      d.actions.append(
      ...
      d.node.addEventListener("close", () =>
        services.navigate(playing ? { screen: "play", id: puzzleId } : { screen: "library", tab: "puzzles" }),
      );
    };
    if (clues < 17)
      confirmDialog({
  ...
  const update = () => {
    ...
    finish.title = conflicts ? "Resolve the highlighted conflicts first" : "Lock the clues and add the puzzle to your library";
