...
export function conflictingCells(values: readonly Value[]): number[] {
  ...
  for (const unit of units) {
    ...
    for (const i of unit) if (values[i]) groups.set(values[i], [...(groups.get(values[i]) ?? []), i]);
    for (const group of groups.values()) if (group.length > 1) for (const i of group) conflicts.add(i);
  }
  return [...conflicts].sort((a,b) => a-b);
}
export function isComplete(values: readonly Value[]): boolean {
  return values.length === 81 && values.every(v => Number.isInteger(v) && v >= 1 && v <= 9) && conflictingCells(values).length === 0;
}
