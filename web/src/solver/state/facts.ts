...
export function originalFact(node: ProofNode): Fact | undefined {
  return originalNodes.get(node);
}

/** Size of the complete initialization that issued this node, including capabilities. */
export function originalRootCount(node: ProofNode): number | undefined {
  return rootCounts.get(node);
}
...
export function createRoots(
  assembly: Assembly,
  branch: BranchId = "primary",
): ReadonlyMap<FactId, Fact> {
  assertM2RootAssemblyBounds(assembly);
  const problem = canonicalProblem(assembly.problem);
  requireProof(
    problem.key === assembly.problem.key && typeof branch === "string" && branch.length > 0,
    "invalid-root-state",
  );
  ...
  for (const cell of problem.cells)
    if (problem.givens[cell] !== 0)
      add(
        ...
        Object.freeze({
          kind: "literal",
          value: Object.freeze({ cell, symbol: problem.givens[cell], positive: true }),
        }),
      );
