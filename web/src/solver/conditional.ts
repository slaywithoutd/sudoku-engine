...
import {
  captureConditionalPrefix,
  type ConditionalPrefixDescriptor,
...
export interface UniqueAuthority {
  readonly [uniqueAuthorityBrand]: true;
}
...
export function isConditionalOperation(value: unknown): value is ConditionalOperation {
  return (
    typeof value === "object" && value !== null && operations.has(value as ConditionalOperation)
  );
}
...
export function uniqueAuthorityMatches(
...
export function uniqueAuthorityOwns(
  authority: UniqueAuthority | undefined,
  view: ReadView,
): boolean {
  const record = authority && authorities.get(authority);
...
const defaultReplayDriver: ConditionalReplayDriver = Object.freeze({
...
export class ConditionalOperation {
  ...
  private constructor(
    ...
    this.assembly = details.assembly;
    this.#limits = Object.freeze({ ...limits });
    this.#workspace = workspace;
    this.#work = details.priorWork ?? 0;
    this.#startedAt = details.startedAt ?? performance.now();
    this.#deadline = this.#startedAt + limits.timeMs;
    this.#lease = workspace.reserve(1, 65536);
    try {
      const estimator = initializationReservation(this.assembly);
      let next = estimator.next();
      while (!next.done) {
        this.charge(next.value.kind === "work" ? next.value.units : 0);
        next = estimator.next();
      }
      this.charge(next.value.workUnits);
      requireProof(
        next.value.nodes <= limits.runNodes && next.value.proofBytes <= limits.proofBytes,
        "conditional-initialization-limit",
      );
      ...
      const prefixLength =
        source.kind === "accepted" ? source.steps.length : source.proposals.length;
  ...
  get prefix(): readonly DeductionProposal[] {
    return this.#prefix;
  }
  get active(): boolean {
    return (
      this.#record.active && (!this.#record.parent || !!uniqueParentDetails(this.#record.parent))
    );
  }
  get prefixDigest(): string | undefined {
    return this.#digest;
  }
  ...
  remainingLimits(): Limits {
    ...
    return Object.freeze({
      ...this.#limits,
      workUnits: Math.max(0, this.#limits.workUnits - this.#work),
      timeMs: Math.max(0, Math.floor(this.#deadline - performance.now())),
  ...
  async digestPrefix(): Promise<string> {
    requireProof(this.active, "revoked-unique-authority");
    if (!this.#descriptor) {
      const captured = await captureConditionalPrefix(
        this.#prefix,
        this.#limits,
        this.#workspace,
        (units) => this.charge(units),
        () => this.active,
        false,
      );
      try {
        this.#descriptor = captured.descriptor;
        this.#digest = captured.descriptor.digest;
      } finally {
   ...
   */
  async grantBootstrap(
    port: MessagePort,
    nonce: string,
    generation: number,
  ): Promise<ConditionalBootstrap> {
    requireProof(
      port instanceof MessagePort &&
        this.active &&
        this.#record.parent &&
        !installedPorts.has(port),
  ...
  acceptsResult(run: RunKey, prefix: ConditionalPrefixDescriptor): boolean {
    return (
      this.active &&
      this.#record.ready &&
      sameValue(run, this.run) &&
      sameValue(prefix, this.#descriptor)
    );
  }
   ...
   */
  static installTrustedBootstrap(
    port: MessagePort,
    expected: ConditionalBootstrap,
    assembly: Assembly,
    limits: Limits,
    workspace: IndexWorkspace,
    driver: ConditionalReplayDriver = defaultReplayDriver,
  ): TrustedConditionalBootstrap {
    requireProof(
      port instanceof MessagePort && !installedPorts.has(port),
      "conditional-port-already-installed",
    );
    MessagePort.prototype.start.call(port);
    ...
    try {
      ...
      requireProof(
        sameValue(envelope.snapshot.problem, assembly.problem) &&
          envelope.run.problemKey === assembly.problem.key &&
          envelope.run.snapshotId === envelope.snapshot.snapshotId &&
          envelope.run.inputRevision === envelope.snapshot.inputRevision &&
          envelope.run.operation === "conditional" &&
          envelope.run.profile === "classic-conditional@1" &&
          typeof envelope.run.parentEvidenceId === "string" &&
          envelope.run.parentEvidenceId.length > 0 &&
          typeof envelope.nonce === "string" &&
          envelope.nonce.length >= 16 &&
          envelope.nonce.length <= 128 &&
          Number.isSafeInteger(envelope.generation) &&
          envelope.generation >= 0,
        "conditional-bootstrap-binding",
      );
    } catch (error) {
    ...
    let active = true,
      ...
      completed = false,
      replaying = false,
      captured: CapturedConditionalPrefix | undefined;
    let operation: ConditionalOperation | undefined,
      work = 0;
    const deadline = performance.now() + limits.timeMs;
    let resolve!: (operation: ConditionalOperation) => void, reject!: (error: unknown) => void;
    const ready = new Promise<ConditionalOperation>((yes, no) => {
      resolve = yes;
      reject = no;
    });
    const charge = (units: number) => {
      workspace.checkpoint();
      requireProof(active, "conditional-bootstrap-disposed");
      ...
      try {
        ...
        operation = new ConditionalOperation(
          constructorKey,
          {
            ...
            assembly,
            prefixSource: { kind: "decoded", proposals: captured.proposals },
            priorWork: work,
            startedAt: deadline - limits.timeMs,
          },
          limits,
          workspace,
        );
        operation.#descriptor = captured.descriptor;
        operation.#digest = captured.descriptor.digest;
        cursor = operation.rebuildPrefix();
        let units = 0;
        for (;;) {
    ...
    function onMessage(event: MessageEvent) {
      try {
        ...
        if (
          frame.kind === "conditional-revoke@1" &&
          frame.nonce === envelope.nonce &&
          frame.generation === envelope.generation
        )
          throw Error("revoked-unique-authority");
        requireProof(
          !granted &&
            sameValue(Object.keys(frame).sort(), ["envelope", "kind"]) &&
            frame.kind === "conditional-grant@1" &&
            sameValue(frame.envelope, envelope),
          "conditional-grant-mismatch",
        );
        granted = true;
        void finish();
      } catch (error) {
    ...
    return Object.freeze({
      ...
      async providePrefix(proposals: readonly DeductionProposal[]) {
        try {
          requireProof(active && !supplied, "conditional-prefix-already-supplied");
          supplied = true;
          captured = await captureConditionalPrefix(
            proposals,
            limits,
            workspace,
            charge,
          ...
          requireProof(
            sameValue(captured.descriptor, envelope.prefix),
      ...
      dispose() {
        fail(Error("conditional-bootstrap-disposed"));
      },
    });
  }
  ...
  *rebuildPrefix(): Generator<CheckEvent, void, void> {
    requireProof(this.active && !this.#started, "conditional-prefix-lifecycle");
    this.#started = true;
    try {
      for (const original of this.#prefix) {
        this.charge(1);
        requireProof(
          original.state.branch === "primary" &&
            sameValue(original.state, original.proof.state) &&
            original.state.problemKey === this.run.problemKey &&
            original.state.revision === this.#view.state.key.revision,
        ...
        for (const event of checkProposal(proposal, {
          ...
          uniqueAuthority: this.authority,
          limits: {
            ...this.#limits,
            workUnits: Math.max(0, this.#limits.workUnits - this.#work),
            timeMs: Math.max(0, Math.floor(this.#deadline - performance.now())),
            proofBytes: this.#limits.proofBytes - this.#headers,
          },
        })) {
          if (event.kind === "work") {
            ...
            this.#lease.grow(
              event.step.proposal.proof.nodes.length,
              checkedHeaderBytes(event.step) + event.step.proposal.proof.nodes.length * 4096,
            );
            this.#view = event.step.proposal.effects.length
              ? commitChecked(this.#view, event.step).view
              : retainCheckedFacts(this.#view, event.step);
            accepted = true;
            yield event;
          }
        }
      ...
      if (!this.#record.ready) this.dispose();
    }
  }
  ...
  *checkAndCommit(proposal: DeductionProposal): Generator<CheckEvent, void, void> {
    ...
    for (const event of checkProposal(proposal, {
      ...
      if (event.kind === "work") {
        ...
        this.#lease.grow(
          event.step.proposal.proof.nodes.length,
          bytes + event.step.proposal.proof.nodes.length * 4096,
        );
        this.#headers += bytes;
        this.#view = event.step.proposal.effects.length
          ? commitChecked(this.#view, event.step).view
