...
import { buildIndex, combinations, evidence, freezeRecord, IndexInterrupted, IndexWorkspace, provedSources, OwnedIndex, reserveRecord, work } from "./workspace";
...
export class AlsIndex extends OwnedIndex<AlsEntry> {
  ...
  constructor(view: ReadView, entries: AlsEntry[], reservation: WorkspaceReservation, private readonly workspace: IndexWorkspace,
    members: WeakSet<AlsEntry>) {
  ...
  *rccSteps(a: AlsEntry, b: AlsEntry, symbol: number): Generator<IndexEvent<boolean>> {
    ...
    try {
      ...
      this.workspace.checkpoint(); scratch.dispose();
  ...
  private *query(a: AlsEntry, b: AlsEntry, symbol: number): Generator<{ kind: "work"; units: number }, boolean> {
    const view = this.source;
    yield* this.tick();
    const left = a.occurrences.find(o => o.symbol === symbol), right = b.occurrences.find(o => o.symbol === symbol);
    if (!left || !right) return false;
    for (const x of left.cells) for (const y of right.cells) {
      yield* this.tick();
      if (x === y) return false;
      let conflict = false;
      for (const event of provedSources(view, this.workspace)) {
        if (event.kind === "work") { yield* this.tick(); continue; }
        const p = event.fact.proposition;
        if ((p.kind !== "all-different" && p.kind !== "relation") || !p.cells.includes(x) || !p.cells.includes(y)) continue;
        if (p.kind === "all-different") { conflict = true; break; }
        let compatible = false;
        for (const tuple of p.tuples) {
          yield* this.tick();
          if (tuple[p.cells.indexOf(x)] === symbol && tuple[p.cells.indexOf(y)] === symbol &&
            p.cells.every((c,i) => view.state.domains[c] & (1 << (tuple[i]-1)))) { compatible = true; break; }
        }
        if (!compatible) { conflict = true; break; }
      }
      if (!conflict) return false;
    }
    return true;
  }
  override dispose(): void { this.#members = new WeakSet(); super.dispose(); }
}

export function* buildAls(view: ReadView, workspace: IndexWorkspace): Generator<IndexEvent<AlsIndex>> {
  yield* buildIndex(view, workspace, function* (reservation) {
    ...
    for (const event of provedSources(view, workspace)) {
      ...
      for (const selection of combinations(available, 5, workspace)) {
        ...
        const entry = freezeRecord({ ...evidence(view, [fact.id,...cells.map(c => view.state.domainFacts[c])]), cells, symbols,
          occurrences: symbols.map(symbol => ({ symbol, cells: cells.filter(c => view.state.domains[c] & (1 << (symbol-1))) })),
          recipe: { kind: "als-domains" as const, source: fact.id } });
        entries.push(entry); members.add(entry);
      }
    }
