...
export type ClassicSolveEvent =
  | {readonly kind:"step";readonly index:number;readonly technique:string;readonly name:string;readonly effects:readonly Effect[];readonly values:readonly number[];
      ...
      readonly candidates:readonly number[];readonly cells:readonly number[]}
...
export function patternCells(pattern:unknown):number[]{
  const found=new Set<number>();let budget=2000;
  const isCell=(n:unknown):n is number=>Number.isInteger(n)&&(n as number)>=0&&(n as number)<81;
  const walk=(x:unknown,depth:number)=>{
    ...
    for(const [key,value] of Object.entries(x as Record<string,unknown>)){
      ...
      else if(key==="cells"&&Array.isArray(value))value.forEach(v=>isCell(v)&&found.add(v));
      else walk(value,depth+1);
    }
  };
  walk(pattern,0);return [...found].sort((a,b)=>a-b);
}
/** Independent exact count for a 9x9 grid (used for correctness marks), bounded by the precount share. */
export function countClassic(givens:readonly number[],clock:{now():number},limits:Readonly<Limits>=CLASSIC_SOLVE_LIMITS):{kind:"zero"|"multiple"|"inconclusive"}|{kind:"unique";solution:readonly number[]}{
  const problem=normalizeClassic({kind:"classic",version:1,width:9,height:9,givens:[...givens]});
...
export async function solveClassic(input:ClassicSolveInput,ports:ClassicSolvePorts):Promise<void>{
  ...
  let pending:{technique:string;effects:readonly Effect[];candidates:readonly number[];cells:readonly number[]}|undefined;
  const emitStep=(step:NonNullable<typeof pending>)=>ports.emit({kind:"step",index:steps,technique:step.technique,name:techniqueNames.get(step.technique)??step.technique,effects:step.effects,values:[...view.state.values],candidates:step.candidates,cells:step.cells});
  await runSolver({snapshot,assembly,view,registry:assembleTechniqueJobs(assembly,PROFILE),workspace,limits,options,run,
    ...
    publish:async(raw:unknown)=>{
      ...
      if(event.kind==="work"){
        ...
        if(pending){emitStep(pending);pending=undefined;}
        ...
        steps++;pending={technique:event.step.proposal.technique,effects:event.step.proposal.effects,candidates:[...view.state.domains],cells:patternCells(event.step.proposal.pattern)};
