  ...
  *paths(start: Literal, g: UniqueGeometry): Generator<Work, Map<string, ForcingLink[]>> {
    ...
    try {
      ...
      for (let i = 0; i < queue.length; i++) {
        const path = paths.get(signedKey(queue[i]))!;
          ...
          if (
            ...
            (link.reason.kind === "house-cover" ||
              (link.reason.kind === "cell-cover" &&
                !g.auxiliaryCells.includes(link.from.cell) &&
                !g.cells.includes(link.from.cell)))
          )
            continue;
          if (
            g.strongSymbol !== null &&
            link.reason.kind === "house-cover" &&
            link.reason.symbol === g.strongSymbol &&
            !(g.kind === "type6" ? g.causalHouses : g.strongHouses).includes(link.reason.house!)
          )
            continue;
...
function* effects(view: ReadView, g: UniqueGeometry): Generator<Effect> {
  const core = uniqueSymbols(g.coreMasks[0]),
    roofs = g.cells.filter((c) => g.guardians.some((a) => a.cell === c));
  const peers = (a: number, b: number) =>
    a !== b && view.assembly.allDifferent.some((h) => h.cells.includes(a) && h.cells.includes(b));
  if (g.row === "U04") {
    yield { kind: "place", cell: g.guardians[0].cell, symbol: g.guardians[0].symbol };
    return;
  }
      ...
      for (const symbol of uniqueSymbols(view.state.domains[cell]))
        for (const kind of ["remove", "place"] as const) {
          ...
          if (["type2", "type5", "avoidable2"].includes(g.kind))
            allowed =
              kind === "remove" &&
              !g.cells.includes(cell) &&
              symbol === g.guardians[0].symbol &&
              roofs.every((c) => peers(c, cell));
          if (g.kind === "type3")
            allowed =
              kind === "remove" &&
              !g.cells.includes(cell) &&
              !g.auxiliaryCells.includes(cell) &&
              g.guardians.some((a) => a.symbol === symbol) &&
              !!view.assembly.allDifferent
                .find((h) => h.id === g.subsetHouse)
                ?.cells.includes(cell);
          if (g.kind === "type4")
            allowed =
              kind === "remove" &&
          ...
          if (g.kind === "type6")
            allowed = kind === "remove" && roofs.includes(cell) && symbol === g.strongSymbol;
          if (g.kind === "hidden")
            allowed =
              kind === "remove" &&
              g.cells.includes(cell) &&
              g.cells.some(
                (c) =>
                  ...
                  c % 9 !== cell % 9 &&
                  view.state.domains[c] === g.coreMasks[0],
              ) &&
              core.includes(symbol) &&
              symbol !== g.strongSymbol &&
...
export function* discoverUnique(
  ...
  const cursors: UniqueGeometryCursor[] =
    row === "U01" || row === "U02"
      ? new UniqueRectangles().cursors(view, row)
      : row === "U03"
        ? [new UniqueLoops().geometries(view)]
        : [new Bug().geometries(view, row)];
  try {
    ...
    while (cursors.length)
      for (let i = 0; i < cursors.length;) {
        ...
        const g = next.value.geometry,
          storage = context.workspace.reserve(1, 4000000 + g.guardians.length * 1500000);
        try {
          const results: {
            assumption: Literal;
          ...
          for (const assumption of g.guardians) {
            const cursor = search.paths(assumption, g);
          ...
          for (const effect of effects(view, g)) {
            ...
            if (
              g.kind === "type6" &&
              effect.cell !== g.cells.find((c) => g.guardians.some((a) => a.cell === c))
            )
              continue;
            let plan: UniquePlan | undefined;
            if (g.row === "U01" || g.row === "U02" || g.guardians.length === 1) {
              const cursor = search.paths(opposite(target), g);
              let next = cursor.next();
              try {
                while (!next.done) {
                  tick();
              ...
              if (
                paths.every((p) => p !== undefined) &&
                new Set(paths.flatMap((p) => p!.map((e) => JSON.stringify(e)))).size <= 24
              )
                plan = {
                  geometry: g,
            ...
            if (
              ...
              plan.consequence.branches.some(
                (b) => new Set(b.paths.flat().map((e) => JSON.stringify(e))).size > 24,
              )
            )
              continue;
            if (g.kind === "type6") {
              const cell = g.cells.find(
                  (c) => c !== effect.cell && g.guardians.some((a) => a.cell === c),
                )!,
                otherEffect: Effect = { kind: "remove", cell, symbol: effect.symbol };
              const cursor = search.paths({ cell, symbol: effect.symbol, positive: true }, g);
              let next = cursor.next();
              try {
                while (!next.done) {
                  tick();
            ...
            const allPaths = [
              ...(plan.consequence.kind === "denial"
                ...
                : plan.consequence.branches.flatMap((b) => b.paths)),
              ...(plan.companion?.consequence.kind === "denial"
                ? plan.companion.consequence.paths
                : []),
            ];
            if (
              (g.kind === "type6" ? g.causalHouses : g.strongHouses).some(
                (h) =>
                  !allPaths
                    .flat()
                    .some(
                      (e) =>
                        e.reason.kind === "house-cover" &&
                        e.reason.house === h &&
                        e.reason.symbol === g.strongSymbol,
                    ),
              )
            )
              continue;
            if (
              g.kind === "type3" &&
              g.auxiliaryCells.some(
                (c) =>
                  !allPaths
                    .flat()
                    .some((e) => e.reason.kind === "cell-cover" && e.reason.cell === c),
              )
            )
              continue;
...
function descriptor(row: UniqueGeometry["row"]): TechniqueDescriptor {
  const entry = coverageEntries.find((e) => e.id === row)!,
    tradeCells = row === "U01" ? 4 : row === "U02" ? 6 : row === "U03" ? 12 : 81;
  return Object.freeze({
    ...
    bounds: {
      maxLength: 24,
      maxBranchDepth: 1,
      maxAlternatives: row === "U01" ? 28 : row === "U02" ? 36 : row === "U04" ? 1 : 4,
      maxPatternCells: tradeCells,
      maxSetSize: row === "U01" ? 4 : row === "U02" ? 3 : row === "U03" ? 2 : 4,
      uniqueness: {
        maxTradeCells: tradeCells,
        maxLoopCells: row === "U03" ? 12 : 0,
        maxGuardianOccurrences: row === "U01" ? 28 : row === "U02" ? 36 : row === "U04" ? 1 : 4,
        maxConsequenceLinksPerBranch: 24,
        maxVirtualSubset: row === "U01" ? 4 : 0,
      },
    },
    ...
    discover: (view: ReadView, context: DiscoveryContext) => discoverUnique(row, view, context),
  });
}
