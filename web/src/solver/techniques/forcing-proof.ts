...
export function forcingProofFits(proposal: DeductionProposal, limits: Limits): boolean {
  ...
  return (
    bytes(proposal) <= limits.stepBytes &&
...
export class ForcingProof {
  ...
  cover(cell: number, source = this.view): number {
    return this.add(
      "cover-clause@1",
      [source.state.domainFacts[cell]],
      proposedClause(symbols(source, cell).map((symbol) => ({ cell, symbol, positive: true }))),
    );
  }
  ...
  path(assumption: number, path: readonly ForcingLink[], frozen = this.view): PathCertificate {
    ...
    for (const link of path) {
      const clause = this.edge(link, link.reason.origin === "parent" ? frozen : this.view);
      clauses.push(clause);
      root = this.add("resolution@1", [root, clause], proposedClause([link.to]));
      links.push(root);
    }
  ...
  finish(technique: string, pattern: unknown, effect: Effect, root: number): DeductionProposal {
    ...
    if (effect.kind === "place")
      for (const cell of this.view.assembly.problem.cells) {
        ...
        const weak = this.add(
          "weak-link@1",
          [this.fact({ kind: "all-different", cells: house.cells })],
          proposedClause([
        ...
        roots.push(
          this.add(
            "resolution@1",
            [root, weak],
            proposedClause([{ cell, symbol: effect.symbol, positive: false }]),
          ),
        );
  ...
  bundle(
    ...
    const all = new Map(this.nodes.map((n) => [n.id, n])),
    ...
    while (stack.length) {
      ...
      reachable.add(id);
      const n = all.get(id);
      if (n) stack.push(...n.premises, ...n.scope);
    }
    return {
      technique,
