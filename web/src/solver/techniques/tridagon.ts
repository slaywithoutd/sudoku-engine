...
export function* compileTridagon(
  ...
  for (const group of p.triples) {
    ...
    for (const row of product(
      group.map((c) => p.coreSymbols.filter((s) => view.state.domains[c] & bitOf(s))),
    )) {
  ...
  for (const selected of product(indexes)) {
    ...
    for (let i = 0; i < 12 && !conflict; i++)
      for (let j = i + 1; j < 12; j++)
        if (assignment[i] === assignment[j] && b.houses.peer(all[i], all[j])) {
  ...
  for (const triple of p.triples) locals.push(yield* b.local(triple, [triple]));
  let table = locals[0];
  const joins: number[] = [];
  for (const next of locals.slice(1)) {
    table = yield* b.joinPeers(table, next);
    joins.push(table.id);
  }
  if (!table.rows.length) return null;
  const conclusion = clause(p.guardians.map((g) => ({ ...g, positive: true }))),
    theorem = b.project(table, conclusion);
  const pattern = {
    ...p,
    certificate: {
      permutations,
      rejections,
      locals: locals.map((t) => t.id),
      joins,
      table: table.id,
      theorem,
    },
  ...
  if (p.guardians.length === 1)
    return b.wire.finish("c32@1", pattern, { kind: "place", ...p.guardians[0] }, theorem);
...
export class TridagonSearch implements SpecializedStrategy {
  *plans(view: ReadView) {
    ...
    for (const core of choose(view.assembly.problem.symbols, 3))
      for (const bands of choose([0, 1, 2], 2))
        for (const stacks of choose([0, 1, 2], 2)) {
          yield specializedWork;
          const boxes = bands.flatMap((b) => stacks.map((s) => b * 3 + s)),
            options: { cells: number[]; guardians: { cell: number; symbol: number }[] }[][] = [];
          for (const box of boxes) {
            const choices = [];
            for (const group of choose(
              (h.boxes[box] ?? []).filter((c) => candidates(view, c).some((s) => core.includes(s))),
              3,
            )) {
              yield specializedWork;
              ...
              let possible = false;
              for (const row of product(
                group.map((c) => core.filter((s) => candidates(view, c).includes(s))),
              ))
          ...
          for (const selected of product(options.map((v) => v.map((_, i) => i)))) {
            ...
            const groups = selected.map((j, i) => options[i][j]),
              guardians = groups
                .flatMap((g) => g.guardians)
                .sort((a, b) => a.cell - b.cell || a.symbol - b.symbol);
            if (guardians.length < 1 || guardians.length > 4) continue;
            const triples = groups.map((g) => g.cells),
              sparse = triples.some((t) =>
                t.some((c) => core.some((s) => !candidates(view, c).includes(s))),
              );
            yield {
              kind: "plan" as const,
              plan: {
  ...
  compile(view: ReadView, p: TridagonPlan, lease?: WorkspaceReservation) {
    return compileTridagon(view, p, lease);
  }
}
