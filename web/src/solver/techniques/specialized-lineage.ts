...
export const orderedNumbers = (p: unknown): p is number[] =>
  Array.isArray(p) && p.every((x, i) => Number.isSafeInteger(x) && (!i || x > p[i - 1]));
...
export class SpecializedAdmission {
  ...
  house(kind: "row" | "column" | "box", i: number): readonly number[] {
    ...
    const cells = Array.from({ length: 81 }, (_, c) => c).filter(
      (c) => (kind === "row" ? Math.floor(c / 9) : kind === "column" ? c % 9 : cellBox(c)) === i,
    );
    requireProof(
      this.view.assembly.allDifferent.some((h) => sameValue(h.cells, cells)),
      "missing-specialized-house",
    );
    return cells;
  }
  domain(c: number): number {
    requireProof(this.view.assembly.problem.cells.includes(c), "specialized-cell-bound");
  ...
  peer(a: number, b: number): boolean {
    return (
      a !== b &&
      this.view.assembly.allDifferent.some((h) => h.cells.includes(a) && h.cells.includes(b))
    );
  }
  scope(id: number, cells: readonly number[]): void {
    ...
    requireProof(
      n.premises.length === 1 &&
        sameValue(n.conclusion, { kind: "all-different", cells: [...cells].sort((a, b) => a - b) }),
    ...
    requireProof(
      fact &&
        ...
        cells.every(
          (c) => fact.proposition.kind === "all-different" && fact.proposition.cells.includes(c),
        ),
  ...
  local(
    ...
    domainIds = cells.map((c) => this.view.state.domainFacts[c]),
    masks = cells.map((c) => this.domain(c)),
  ): void {
    const leaves: number[][] = [];
    const walk = (id: number): number[] => {
      const n = this.node(id),
        p = n.parameters as { cells: number[]; box: number[] };
      requireProof(
      ...
      if (n.rule === "table-union@1") {
        ...
        const a = walk(n.premises[0]),
          b = walk(n.premises[1]),
          diff = a.flatMap((x, i) => (x === b[i] ? [] : [i]));
        requireProof(
          diff.length === 1 && !(a[diff[0]] & b[diff[0]]),
      ...
      scopes.forEach((g, i) => this.scope(n.premises[cells.length + i], g));
      leaves.push([...n.premises]);
      return p.box;
    };
    requireProof(
      sameValue(walk(id), masks) && leaves.every((s) => sameValue(s, leaves[0])),
    ...
    const sources = leaf.premises.slice(0, cells.length),
      masks = cells.map((cell, i) => {
        const required = [...new Set(roots)].filter((root) => {
          ...
          return c.kind === "literal" && c.value.positive && c.value.cell === cell;
        });
  ...
  joinPeers(id: number, left: number, right: number, extra: readonly number[] = []): void {
    const a = this.node(left).conclusion,
    ...
    for (const x of a.cells)
      for (const y of b.cells)
        if (x !== y && this.peer(x, y) && !a.cells.includes(y) && !b.cells.includes(x))
          pairs.push([x, y]);
    requireProof(
