...
const advanced=[...fishTechniques,...shortPatternTechniques,...wingTechniques,...bentSubsetTechniques,...remotePairTechniques,...coloringTechniques,...chainTechniques,...loopTechniques,...alsPatternTechniques,...deathBlossomTechniques,...sueDeCoqTechniques,...alignedExclusionTechniques,...forcingTechniques,...netTechniques,...krakenTechniques];
const descriptors: readonly TechniqueDescriptor[] = Object.freeze(coverageEntries.map(entry => advanced.find(d=>d.id===entry.version)??Object.freeze({
  ...
  bounds: Object.freeze({ maxLength: dimensions[entry.id][0], maxBranchDepth: dimensions[entry.id][1],
    maxAlternatives: dimensions[entry.id][2], maxPatternCells: dimensions[entry.id][3], maxSetSize: dimensions[entry.id][4] }),
  ...
  eligible: (view: ReadView) => {
    ...
    if (!detectors.has(entry.id)) return { kind: "excluded" as const, reason: "specified-not-implemented", dependencies };
    if ((entry.capabilities.includes("A") && !view.assembly.allDifferent.length) || (entry.capabilities.includes("H") && !view.assembly.covers.length))
...
export function getTechniques(profile: VersionId): readonly TechniqueDescriptor[] {
  if (!["classic-expanded@1", "classic-conditional@1"].includes(profile)) throw Error("unknown-profile");
  return profile === "classic-conditional@1" ? descriptors : Object.freeze(descriptors.filter(d => d.assumptionPolicy !== "unique-only"));
}

/** Catalogue visibility is distinct from runtime readiness; partial M2 cannot stall successfully. */
export function profileReadiness(profile: VersionId): { readonly ready: boolean; readonly missing: readonly string[] } {
  getTechniques(profile);
  const missing=coverageEntries.filter(e => (profile === "classic-conditional@1" || e.assumptionPolicy !== "unique-only") &&
    e.status !== "independently-verified").map(e => e.id);
...
export function assembleTechniqueJobs(assembly: Assembly, profile: VersionId) {
  ...
  if (assembly.problem.constraints.length + techniques.length > 256) throw Error("profile-job-limit");
  if (!profileReadiness(profile).ready) throw Error("profile-incomplete");
  return Object.freeze({ rules: Object.freeze(assembly.problem.constraints.map(rule => Object.freeze({
    id: "rule-propagation@1", scopeKey: rule.id,
    discover: (view: ReadView): Discovery => assembly.modules.get(rule.id)!.propagate(view, rule),
  }))), techniques });
}
