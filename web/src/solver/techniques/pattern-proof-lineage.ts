...
class BentTableLineage {
  ...
  constructor(readonly view: ReadView, readonly pattern: BentPattern,
    readonly available: ReadonlyMap<number, ProofNode>) {}

  check(root: number): void {
    const pending = [{ id: root, expanded: false }], active = new Set<number>();
    while (pending.length) {
      const { id, expanded } = pending.pop()!;
      if (this.#checked.has(id)) continue;
      const node = this.available.get(id);
      requireProof(node?.conclusion.kind === "table" && node.conclusion.definition === id &&
        sameValue(node.conclusion.cells, this.pattern.cells), "invalid-bent-table-lineage");
      if (!expanded) {
        requireProof(!active.has(id), "cyclic-bent-table-lineage");
        active.add(id); pending.push({ id, expanded: true });
      ...
      if (node.rule === "table-filter@1") this.#checked.set(id, this.checkLeaf(node));
        ...
        const a = this.#checked.get(node.premises[0])!, b = this.#checked.get(node.premises[1])!;
        ...
        requireProof(changed.length === 1 && (a.box[changed[0]] & b.box[changed[0]]) === 0,
    ...
    requireProof(sameValue(complete.box, this.pattern.cells.map(c => this.view.state.domains[c])),
  ...
  private checkLeaf(node: ProofNode): LocalTable {
    const p = this.pattern, parameters = node.parameters as { cells: number[]; box: number[] };
    requireProof(sameValue(parameters.cells, p.cells) && parameters.box.length === p.cells.length &&
      node.premises.length === p.cells.length + p.conflicts.length, "incomplete-bent-table-sources");
    const currentDomains = p.cells.map(c => this.view.state.domainFacts[c]);
    requireProof(currentDomains.every(id => node.premises.includes(id)), "stale-bent-table-domains");
    const conflicts = node.premises.filter(id => !currentDomains.includes(id));
    const declared = new Set(p.conflicts.map(pair => pair.join(":"))), found = new Set<string>();
    for (const id of conflicts) {
      const subset = this.available.get(id);
      requireProof(subset?.rule === "all-different-subset@1" && subset.conclusion.kind === "all-different" &&
        subset.premises.length === 1 && subset.scope.length === 0, "invalid-bent-table-conflict");
      const key = subset.conclusion.cells.join(":"), source = this.view.facts.get(subset.premises[0]);
      requireProof(declared.has(key) && !found.has(key) && source?.openAssumptions.length === 0 &&
        source.proposition.kind === "all-different" && subset.conclusion.cells.every(c =>
          source.proposition.kind === "all-different" && source.proposition.cells.includes(c)), "invalid-bent-table-conflict");
      found.add(key);
    }
    requireProof(found.size === declared.size, "incomplete-bent-table-conflicts");
    requireProof(parameters.box.every((mask, i) => Number.isSafeInteger(mask) && mask >= 0 &&
      (mask & this.view.state.domains[p.cells[i]]) === mask), "invalid-bent-table-partition");
...
export function requireBentEffectLineage(proposal: DeductionProposal, view: ReadView,
  available: ReadonlyMap<number, ProofNode>, pattern: BentPattern): void {
  const expected = clause(pattern.occurrences[pattern.nonrestrictedSymbol].map(cell =>
    ({ cell, symbol: pattern.nonrestrictedSymbol, positive: true })));
  const tables = new BentTableLineage(view, pattern, available), projections = new Set<number>();
  for (const node of proposal.proof.nodes) if (node.rule === "table-project@1" && sameValue(node.conclusion, expected)) {
    requireProof(node.premises.length === 1, "invalid-bent-projection");
    tables.check(node.premises[0]); projections.add(node.id);
  }
  ...
  for (const node of proposal.proof.nodes)
    if (projections.has(node.id) || node.premises.some(id => fromProjection.has(id))) fromProjection.add(node.id);
  for (const effect of proposal.effects) {
    const roots = proposal.proof.roots.filter(id => sameValue(available.get(id)?.conclusion,
      { kind: "literal", value: { cell: effect.cell, symbol: effect.symbol, positive: false } }));
    requireProof(roots.length > 0 && roots.every(id => fromProjection.has(id)), "missing-bent-effect-lineage");
  }
}
...
class ShortComponentLineage {
  ...
  constructor(readonly proposal: DeductionProposal, readonly view: ReadView,
    readonly available: ReadonlyMap<number, ProofNode>, readonly path: ShortPath) {
    const [a, b, c, d] = path.vertices, symbol = path.symbol;
    const covers = [[...a, ...b], [...c, ...d]].map(cells => clauseKey(cells.map(cell => positive(cell, symbol))));
    const middle = new Map<string, number>();
    for (const left of b) for (const right of c)
      middle.set(clauseKey([negative(left, symbol), negative(right, symbol)]), 1 << (middle.size + 2));
    const required = (1 << (middle.size + 2)) - 1, local = new Map<number, number>();
    ...
    for (const node of proposal.proof.nodes) {
      ...
      if (node.rule === "cover-clause@1") {
        ...
        local.set(node.id, node.premises.reduce((mask, id) => mask | local.get(id)!, 0));
      }
  ...
  private matchesCover(node: ProofNode, which: number): boolean {
    ...
    const scope = this.view.assembly.allDifferent.find(h => h.id === this.path.strongHouses[which])!.cells;
    const source = this.view.facts.get(support.premises[0]);
    return source?.openAssumptions.length === 0 && source.proposition.kind === "cover" &&
      source.proposition.symbol === this.path.symbol && sameValue(source.proposition.cells, scope) &&
      sameValue(support.premises.slice(1), scope.map(c => this.view.state.domainFacts[c]));
  }

  rootsFor(effect: Effect): readonly number[] {
    if (effect.symbol !== this.path.symbol || !this.#endpoints.size) return [];
    const outer = [...this.path.vertices[0], ...this.path.vertices[3]], conflicts = new Map<string, number>();
    if (outer.includes(effect.cell)) return [];
    for (const cell of outer) conflicts.set(clauseKey([negative(cell, effect.symbol), negative(effect.cell, effect.symbol)]),
      1 << (conflicts.size + 1));
    const required = (1 << (conflicts.size + 1)) - 1, local = new Map<number, number>();
    ...
    return this.proposal.proof.roots.filter(id => local.get(id) === required && sameValue(this.available.get(id)?.conclusion,
