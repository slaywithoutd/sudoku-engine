...
export function* localSets(view: ReadView, graph: PatternGraph, max: number): Generator<ChainWork, LocalSet[]> {
  ...
  for (const house of view.assembly.allDifferent) {
    ...
    for (let size = 1; size <= Math.min(max, empty.length); size++) for (const cells of combinations(empty, size)) {
      yield { kind: "work", units: 1 };
      const symbols = setUnion(view, cells), key = house.id + "/" + cells.join();
      if (symbols.length !== size + 1 || seen.has(key)) continue;
      graph.lease.grow(1, 512 + size * 32); seen.add(key); sets.push({ cells, house: house.id, symbols });
    }
  }
  return sets;
}
...
export function setDescriptor(id: "C20" | "C21", strategy: SetStrategy): TechniqueDescriptor {
  ...
  return Object.freeze({ id: row.version, aliases: row.aliases, tier: row.tier, requires: row.capabilities, assumptionPolicy: row.assumptionPolicy,
    bounds: { maxLength: 0, maxBranchDepth: id === "C21" ? 1 : 0, maxAlternatives: id === "C21" ? 6561 : 9, maxPatternCells: id === "C20" ? 11 : 12, maxSetSize: id === "C20" ? 4 : 5 },
    watches: () => [{ kind: "all" as const }], eligible: (view: ReadView) => eligibility(view, id), estimate: () => ({ hit: 1, gain: 1, cost: 10 }),
    *discover(view: ReadView, context: DiscoveryContext): Discovery {
      assertOwnedView(view);
      const applicable = eligibility(view, id); if (applicable.kind === "excluded") { yield applicable; return; }
      let index: ImplicationIndex | undefined, lease: WorkspaceReservation | undefined, work = 0;
      const cursors: SetCursor[] = [];
      const tick = () => { context.workspace.checkpoint(); if (++work > context.limits.workUnits) throw Error("set-work-limit"); };
      try {
        ...
        const preparer = localSets(view, graph, id === "C20" ? 4 : 5); let sets: LocalSet[];
        try { while (true) { tick(); const event = preparer.next(); if (event.done) { sets = event.value; break; } yield event.value; } }
        finally { preparer.return([]); }
        cursors.push(...strategy(view, graph, sets));
        while (cursors.length) for (let i = 0; i < cursors.length; i++) {
          tick(); const event = cursors[i].next();
          if (event.done) { cursors.splice(i--, 1); continue; }
          if (event.value.kind === "work") { yield event.value; continue; }
          graph.compilation = context.workspace.reserve(0, 65536);
          const compiler = new SetCertificate(view, graph).compile(event.value.pattern, event.value.effects);
          try {
            while (true) {
        ...
        else if (error instanceof Error && error.message === "set-work-limit") yield { kind: "interrupted", reason: "work-limit" };
        else throw error;
      } finally { cursors.forEach(c => c.return(undefined)); lease?.dispose(); index?.dispose(); }
    } });
}
