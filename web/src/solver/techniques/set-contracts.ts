...
export interface SdcPattern {
  kind: "sdc";
  alias: "Sue de Coq" | "Two-sector disjoint subsets";
  line: string;
  box: string;
  intersection: number[];
  lineSide: number[];
...
export interface AlignedPattern {
  kind: "aligned";
  alias: "Aligned Pair Exclusion" | "Aligned Triple Exclusion" | "Generalized Aligned Exclusion";
  selected: number[];
  domains: number[];
  auxiliaries: (LocalSet & { domains: number[]; table: number })[];
