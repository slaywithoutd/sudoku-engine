...
function scaffold(view: ReadView): void {
  ...
  for (const coordinate of [row, col, box])
    for (let n = 0; n < 9; n++) {
      ...
      requireProof(
        view.assembly.allDifferent.some((h) => sameValue(h.cells, cells)),
...
export function checkUniqueGeometry(
  ...
  requireProof(
    Array.isArray(cells) &&
      cells.length >= 4 &&
      cells.length <= 81 &&
      cells.every((c, i) => p.cells.includes(c) && (!i || c > cells[i - 1])) &&
      cells.every((c) => !p.givens[c]) &&
      g.coreMasks.length === cells.length,
    "unique-named-cells",
  );
  const guardians = cells.flatMap((cell, i) =>
    symbols(domains[cell] & ~g.coreMasks[i]).map((symbol) => ({ cell, symbol, positive: true })),
  );
  requireProof(
    sameValue(g.guardians, guardians) && guardians.length > 0 && guardians.length <= 64,
    "unique-named-guardians",
  );
  if (g.kind === "bug") {
    requireProof(
      (g.row === "U04" && ["BUG", "BUG+1"].includes(g.alias) && guardians.length === 1) ||
        (g.row === "U05" &&
          ["Generalized BUG", "BUG+n"].includes(g.alias) &&
          guardians.length >= 2 &&
          guardians.length <= 4),
      "unique-bug-bound",
    );
    requireProof(
      g.permutation === null &&
        sameValue(
          cells,
          p.cells.filter((c) => !view.state.values[c]),
        ) &&
        cells.every(
          (c, i) =>
            symbols(g.coreMasks[i]).length === 2 &&
            (domains[c] & g.coreMasks[i]) === g.coreMasks[i],
        ),
  ...
  requireProof(
    g.coreMasks.every((mask) => mask === g.coreMasks[0]),
    "unique-common-core",
  );
  const core = symbols(g.coreMasks[0]),
    roofs = cells.filter((c) => guardians.some((a) => a.cell === c));
  if (g.kind === "loop") {
    requireProof(
      g.row === "U03" &&
        cells.length <= 12 &&
    ...
    requireProof(
      g.loopOrder.length === cells.length &&
        new Set(g.loopOrder).size === cells.length &&
        g.loopOrder.every((c) => cells.includes(c)),
      "unique-loop-order",
    );
    for (let i = 0; i < cells.length; i++)
      requireProof(
        view.assembly.allDifferent.some(
          (h) =>
            h.cells.includes(g.loopOrder[i]) &&
            h.cells.includes(g.loopOrder[(i + 1) % cells.length]),
        ),
        "unique-loop-edge",
      );
    for (const h of view.assembly.allDifferent) {
      ...
      requireProof(
        selected.length === 0 ||
          (selected.length === 2 &&
            g.loopOrder.indexOf(selected[0]) % 2 !== g.loopOrder.indexOf(selected[1]) % 2),
        "unique-loop-alternation",
      );
    }
    requireProof(
      cells.every((c) => !view.state.values[c] && (domains[c] & g.coreMasks[0]) === g.coreMasks[0]),
      "unique-loop-core",
    );
    return;
  }
  if (g.kind === "extended") {
    requireProof(
      g.row === "U02" &&
        ...
        g.permutation?.length === 6 &&
        guardians.length <= 36 &&
        cells.every(
          (c) => !view.state.values[c] && (domains[c] & g.coreMasks[0]) === g.coreMasks[0],
        ),
  ...
  if (avoidable) {
    ...
    requireProof(
      derived.length === (g.kind === "avoidable1" ? 3 : 2) &&
        derived.every((c) => core.includes(view.state.values[c])) &&
  ...
  if (g.kind === "type1" || g.kind === "avoidable1")
    requireProof(
      roofs.length === 1 &&
        effect.kind === "remove" &&
        effect.cell === roofs[0] &&
  ...
  if (["type2", "type5", "avoidable2"].includes(g.kind)) {
    requireProof(
      count(guardians.map((a) => a.symbol)) === 1 &&
        effect.kind === "remove" &&
        effect.symbol === guardians[0].symbol &&
        !cells.includes(effect.cell) &&
        roofs.every((c) => peers(c, effect.cell)),
  ...
  if (g.kind === "type3") {
    ...
    requireProof(
      adjacent &&
        ...
        extra.length <= 4 &&
        h &&
        roofs.every((c) => h.cells.includes(c)) &&
        g.auxiliaryCells.length === extra.length - 1 &&
        new Set(g.auxiliaryCells).size === g.auxiliaryCells.length &&
        g.auxiliaryCells.every(
          (c) =>
            h.cells.includes(c) &&
            !cells.includes(c) &&
            !view.state.values[c] &&
            symbols(domains[c]).every((s) => extra.includes(s)),
        ) &&
        effect.kind === "remove" &&
        h.cells.includes(effect.cell) &&
        !cells.includes(effect.cell) &&
        !g.auxiliaryCells.includes(effect.cell) &&
  ...
  if (["type4", "type6", "hidden"].includes(g.kind)) {
    requireProof(
      core.includes(g.strongSymbol!) &&
        g.strongHouses.length === (g.kind === "type4" ? 1 : g.kind === "type6" ? 4 : 2) &&
    ...
    if (g.kind === "type4")
      requireProof(
        adjacent &&
          roofs.every((c) => houses[0]!.cells.includes(c)) &&
          effect.kind === "remove" &&
          roofs.includes(effect.cell) &&
          core.includes(effect.symbol) &&
          effect.symbol !== g.strongSymbol,
        "unique-type4-effect",
      );
    if (g.kind === "type6") {
      ...
      requireProof(
        roofs.length === 2 &&
          ...
          roofs.includes(effect.cell) &&
          effect.symbol === g.strongSymbol &&
          houses.filter((h) => count(h!.cells.map(row)) === 1).length === 2 &&
          houses.filter((h) => count(h!.cells.map(col)) === 1).length === 2 &&
          g.causalHouses.length === 2 &&
          new Set(g.causalHouses).size === 2 &&
          g.causalHouses.every((id) => g.strongHouses.includes(id)) &&
          (causal.every((h) => count(h!.cells.map(row)) === 1) ||
            causal.every((h) => count(h!.cells.map(col)) === 1)),
        "unique-type6-effect",
      );
    }
    if (g.kind === "hidden")
      requireProof(
        effect.kind === "remove" &&
          ...
          cells.some(
            (c) =>
              row(c) !== row(effect.cell) &&
              col(c) !== col(effect.cell) &&
              domains[c] === g.coreMasks[0],
          ) &&
          core.includes(effect.symbol) &&
          effect.symbol !== g.strongSymbol &&
          houses.every((h) => h!.cells.includes(effect.cell)) &&
          houses.some((h) => count(h!.cells.map(row)) === 1) &&
          houses.some((h) => count(h!.cells.map(col)) === 1),
...
export function checkUniquePattern(
  ...
  requireProof(
    p.companion && p.certificate?.companion && proposal.effects.length === 2,
    "unique-type6-atomic-effects",
  );
  const roofs = p.geometry.cells.filter((c) => p.geometry.guardians.some((a) => a.cell === c));
  requireProof(
    new Set(proposal.effects.map((e) => e.cell)).size === 2 &&
      proposal.effects.every(
        (e) =>
          e.kind === "remove" && roofs.includes(e.cell) && e.symbol === p.geometry.strongSymbol,
      ) &&
      sameValue(proposal.effects[1], p.companion.effect),
    "unique-type6-atomic-effects",
  );
  const roots = new Set<number>();
  for (const [i, effect] of proposal.effects.entries()) {
    const certificate = i ? p.certificate.companion : p.certificate,
      consequence = i ? p.companion.consequence : p.consequence;
    const domainRoots = proposal.proof.roots.filter((id) => {
      const q = available.get(id)?.conclusion;
      return q?.kind === "domain" && q.cell === effect.cell;
    });
    const selected = [certificate!.root, ...domainRoots];
    selected.forEach((id) => roots.add(id));
    checkUniqueOne(
      {
        ...proposal,
        effects: [effect],
        pattern: { geometry: p.geometry, consequence, certificate } as any,
...
function checkUniqueOne(
  ...
  requireProof(
    p.geometry &&
      p.consequence &&
      c &&
      proposal.technique === `${p.geometry.row.toLowerCase()}@1` &&
      proposal.effects.length > 0,
    "unique-pattern",
  );
  checkUniqueGeometry(p.geometry, view, proposal.effects[0]);
  const l = new ForcingLineage(view, available),
    trade = l.node(c.trade),
    g = p.geometry;
  requireProof(
    trade.rule === "unique-transform@1" &&
      !trade.scope.length &&
      sameValue(trade.conclusion, clause(g.guardians)) &&
      sameValue((trade.parameters as any).cells, g.cells) &&
      sameValue((trade.parameters as any).coreMasks, g.coreMasks) &&
      sameValue((trade.parameters as any).permutation, g.permutation),
  ...
  if (p.consequence.kind === "denial") {
    requireProof(
      c.branches.length === 1 && p.consequence.paths.length === g.guardians.length,
      "unique-denial-complete",
    );
    const b = c.branches[0],
      a = l.node(b.assumption),
      result = l.node(b.result);
    requireProof(
      a.rule === "assume@1" &&
        !a.scope.length &&
        sameValue(a.conclusion, clause([opposite(target)])) &&
        b.paths.length === g.guardians.length,
      "unique-denial-assumption",
    );
    p.consequence.paths.forEach((path, i) => {
      l.path(a.id, path, b.paths[i], [a.id]);
      requireProof(
        sameValue(l.node(b.paths[i].end).conclusion, clause([opposite(g.guardians[i])])),
        "unique-denial-alternative",
      );
    });
    requireProof(
      new Set(b.paths.flatMap((p) => p.links)).size <= 24 &&
        result.rule === "contradiction@1" &&
        sameValue(result.premises, [trade.id, ...b.paths.map((p) => p.end)]) &&
        root.rule === "discharge@1" &&
        sameValue(root.premises, [a.id, result.id]),
      "unique-denial-lineage",
    );
  } else {
    requireProof(
      p.consequence.kind === "cases" &&
        p.consequence.branches.length === g.guardians.length &&
        c.branches.length === g.guardians.length,
    ...
    for (const [i, b] of p.consequence.branches.entries()) {
      ...
      requireProof(
        !seen.has(key) &&
          g.guardians.some((v) => sameValue(v, b.assumption)) &&
          a.rule === "assume@1" &&
          !a.scope.length &&
          sameValue(a.conclusion, clause([b.assumption])),
      ...
      if (b.result === "false")
        ...
        requireProof(
          result.id === certificate.paths[0].end && sameValue(b.result, target),
          "unique-case-target",
        );
    }
    const order = p.consequence.branches
      .map((b, i) => ({ b, c: c.branches[i] }))
      .sort(
        (a, b) =>
          a.b.assumption.cell - b.b.assumption.cell ||
          a.b.assumption.symbol - b.b.assumption.symbol,
      );
    requireProof(
      root.rule === "cases@1" &&
        sameValue(root.premises, [trade.id, ...order.flatMap(({ c }) => [c.assumption, c.result])]),
  ...
  while (stack.length) {
    ...
    ancestors.add(id);
    if (!view.facts.has(id)) stack.push(...l.node(id).premises);
  }
  const neededHouses =
    g.kind === "type6"
  ...
  if (g.kind === "type3")
    for (const cell of g.auxiliaryCells)
      requireProof(
        [...ancestors].some((id) => {
