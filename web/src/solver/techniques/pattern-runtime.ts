...
export class PatternGraph {
  ...
  readonly scopes = new Map<string, number>();
  readonly covers = new Map<string, CoverEntry>();
  compilation: WorkspaceReservation | undefined;
  nextNode = 0;
  constructor(
    readonly index: ImplicationIndex,
  ...
  *prepare(view: ReadView): Generator<{ kind: "work"; units: number }> {
    for (const [id, fact] of view.facts) {
      ...
      this.nextNode = Math.max(this.nextNode, id + 1);
      ...
      for (let i = 0; i < cells.length; i++)
        for (let j = i + 1; j < cells.length; j++) {
          ...
          if (!this.scopes.has(key)) {
            this.lease.grow(1, 256);
            this.scopes.set(key, id);
          }
        }
    }
...
export interface PatternStrategy {
  ...
  compile(
    ...
    pattern: Json,
    effects: Effect[],
  ): Generator<{ kind: "work"; units: number }, DeductionProposal>;
}
...
export class PatternBuilder {
  ...
  readonly values = new Map<number, Proposition>();
    ...
    if (source === undefined) {
      ...
      for (const cell of relation.cells) {
        ...
        const filter = this.add(
          "table-filter@1",
          [this.view.state.domainFacts[cell]],
          { kind: "table", cells: [cell], count: digits.length, definition: this.next },
          { cells: [cell], box: [mask] },
        );
  ...
  *path(
    vertices: Literal[][],
    strong: number[],
  ): Generator<{ kind: "work"; units: number }, number> {
    let root = strong[0];
    for (let offset = 2; offset < vertices.length; offset += 2) {
      const exclusions: number[] = [];
      for (const next of vertices[offset]) {
        let excluded = root;
        for (const previous of vertices[offset - 1]) {
          yield { kind: "work", units: 1 };
          const weak = yield* this.weak(previous, next);
          excluded = this.resolve(excluded, weak, previous);
        }
        exclusions.push(excluded);
      }
      root = strong[offset / 2];
      for (let i = 0; i < vertices[offset].length; i++)
        root = this.resolve(root, exclusions[i], vertices[offset][i]);
    }
    return root;
  }
  finish(
