 ...
 */
export function* initializationReservation(assembly: Assembly): Generator<CheckEvent, InitializationReservation, void> {
  ...
  for (const cell of problem.cells) if (problem.givens[cell]) {
    yield { kind: "work", units: 1 };
    add("given@1", { kind: "literal", value: { cell, symbol: problem.givens[cell], positive: true } });
  }
  const handles = new Map<number,string>();
  for (const rule of problem.constraints) {
    yield { kind: "work", units: 1 };
    handles.set(nodes, rule.id); add("rule-instance@1", { kind: "rule", constraintId: rule.id });
  }
  ...
  for (const scope of sorted(assembly.allDifferent)) {
    yield { kind: "work", units: 1 };
    add("all-different@1", { kind: "all-different", cells: scope.cells }, [scope.premise], { constraintId: handles.get(scope.premise) });
  }
  for (const cover of sorted(assembly.covers)) {
    yield { kind: "work", units: 1 };
    add("cover@1", { kind: "cover", cells: cover.cells, symbol: cover.symbol }, [cover.premise], { constraintId: handles.get(cover.premise) });
  }
  for (const relation of sorted(assembly.relations)) {
    yield { kind: "work", units: 1 };
    add("relation@1", { kind: "relation", cells: relation.cells, tuples: relation.tuples }, [relation.premise], { constraintId: handles.get(relation.premise) });
  }
  const scopeEntries = [...problem.constraints, ...assembly.allDifferent, ...assembly.covers, ...assembly.relations]
    .reduce((total, scope) => total + scope.cells.length, 0);
  const tupleEntries = assembly.relations.reduce((total, relation) => total + relation.cells.length * relation.tuples.length, 0);
  return Object.freeze({ nodes, proofBytes,
...
export function* replay(snapshot: SolverSnapshot, bundles: readonly DeductionProposal[],
  assembly: Assembly, limits: Limits): Generator<CheckEvent, void, void> {
  try {
    limits = { ...limits };
    requireProof(sameValue(Object.keys(limits).sort(), ["timeMs", "workUnits", "exactNodes", "stepNodes", "runNodes", "proofBytes",
      "stepBytes", "batchBytes", "inFlightBatches", "workspaceBytes"].sort()), "invalid-proof-limit");
    for (const value of Object.values(limits)) requireProof(Number.isSafeInteger(value) && value >= 0, "invalid-proof-limit");
    requireProof(limits.stepNodes <= 16384 && Number.isSafeInteger(limits.timeMs) && limits.timeMs > 0, "invalid-proof-limit");
    const deadline = performance.now() + limits.timeMs;
    let work = 0, headers = 0;
    ...
    while (!next.done) { charge(next.value.kind === "work" ? next.value.units : 0); yield next.value; next = estimating.next(); }
    const startup = next.value;
    requireProof(startup.nodes <= limits.runNodes, "proof-run-node-limit");
    requireProof(startup.proofBytes <= limits.proofBytes && startup.proofBytes + startup.workspaceBytes <= limits.workspaceBytes, "proof-byte-limit");
    for (let remaining = startup.workUnits; remaining > 0; remaining -= 256) {
      const units = Math.min(remaining, 256); charge(units); yield { kind: "work", units };
    }
    ...
    for (const proposal of bundles) {
      ...
      for (const event of checkProposal(proposal, { view, retained: retainedProof(view), policy: "discharged", uniqueEvidenceId: null,
        limits: { ...limits, timeMs: Math.max(0, Math.floor(deadline - performance.now())), workUnits: Math.max(0, limits.workUnits-work),
          proofBytes: Math.max(0, limits.proofBytes-headers), workspaceBytes: Math.max(0, limits.workspaceBytes-headers-startup.workspaceBytes) } })) {
        if (event.kind === "work") { work += event.units; yield event; }
          ...
          view = event.step.proposal.effects.length > 0 ? commitChecked(view, event.step).view : retainCheckedFacts(view, event.step);
          checked = true; yield event;
        }
      }
