...
export function checkedStepMatchesSource(step: CheckedStep, view: ReadView): boolean {
  const token = sourceTokens.get(view);
  return !!token && stepSources.get(step) === token;
}
...
export function checkUsage(cursor: object): Readonly<Usage> {
  const usage = checkMeters.get(cursor);
  requireProof(usage, "unknown-check-cursor");
...
const stepUsage = new WeakMap<
  CheckedStep,
  { readonly headerBytes: number; readonly workUnits: number; readonly stepBytes: number }
>();
...
export function checkedImportsMatch(
  step: CheckedStep,
  retained: ReadonlyMap<number, ProofNode>,
): boolean {
 ...
 */
export function checkedEffectState(
  view: ReadView,
  ...
  for (const effect of effects) {
    ...
    requireProof(
      consequences.some((item) =>
        sameValue(item.conclusion, {
          kind: "literal",
    ...
    if (effect.kind === "place") {
      requireProof(
        values[effect.cell] === 0 && (domains[effect.cell] & bit) !== 0,
        "conflicting-effects",
      );
    ...
    if (effect.kind === "place") {
      ...
      const peers = new Set(
        view.assembly.allDifferent
          .filter((scope) => scope.cells.includes(effect.cell))
          .flatMap((scope) => scope.cells)
          .filter((cell) => cell !== effect.cell),
      );
  ...
  for (const cell of cells)
    requireProof(
      consequences.some((item) =>
        sameValue(item.conclusion, { kind: "domain", cell, mask: domains[cell] }),
      ),
...
export function captureProofRecord<T>(
  input: T,
  limit: number,
): { readonly value: T; readonly bytes: number } {
  requireProof(
    Number.isSafeInteger(limit) && limit >= 0 && limit <= HEADER_BYTES,
...
function* captureProposal(
  input: DeductionProposal,
  cap: number,
  stepNodes: number,
  tick: () => void,
): Generator<
  CheckEvent,
  { header: ReturnType<typeof copyBounded<DeductionProposal>>; references: ProofNode[] },
  void
> {
  ...
  requireProof(
    Array.isArray(nodes) && nodes.length <= stepNodes && nodes.length <= 16384,
    "proof-step-node-limit",
  );
  requireProof(Reflect.ownKeys(nodes).length === nodes.length + 1, "invalid-proof-array");
  const count = nodes.length;
  const header = copyBounded(
    { ...input, proof: { ...input.proof, nodes: [] } },
    Math.min(cap, HEADER_BYTES),
  );
  ...
  checkProposal(input: DeductionProposal, source: CheckContext): Generator<CheckEvent, void, void> {
    ...
    const cursor = (function* (): Generator<CheckEvent, void, void> {
      for (const event of verifyGraph(input, source, "named", meter)) {
        if (event.kind === "verified" || event.kind === "branch-checked")
          throw Error("internal-admission-error");
        yield event;
      }
    })();
    checkMeters.set(cursor, meter);
    return cursor;
  }
}
...
class ProofStage implements ReadonlyMap<number, ProofNode> {
  readonly #map: ReadonlyMap<number, ProofNode>;
  readonly #cutoff: number;
  constructor(map: ReadonlyMap<number, ProofNode>, cutoff: number) {
    this.#map = map;
    this.#cutoff = cutoff;
    Object.freeze(this);
  }
  ...
  get(id: number): ProofNode | undefined {
    return id < this.#cutoff ? this.#map.get(id) : undefined;
  }
  has(id: number): boolean {
    return id < this.#cutoff && this.#map.has(id);
  }
  *entries(): MapIterator<[number, ProofNode]> {
    for (const [id, node] of this.#map) if (id < this.#cutoff) yield [id, node];
  }
  *keys(): MapIterator<number> {
    for (const [id] of this.entries()) yield id;
  }
  *values(): MapIterator<ProofNode> {
    for (const [, node] of this.entries()) yield node;
  }
  [Symbol.iterator](): MapIterator<[number, ProofNode]> {
    return this.entries();
  }
  forEach(
    callback: (value: ProofNode, key: number, map: ReadonlyMap<number, ProofNode>) => void,
    thisArg?: unknown,
  ): void {
    for (const [id, node] of this.entries()) callback.call(thisArg, node, id, this);
  }
}
...
class MeteredProofStage extends ProofStage {
  ...
  constructor(
    map: ReadonlyMap<number, ProofNode>,
    cutoff: number,
  ...
  override get(id: number): ProofNode | undefined {
    this.#charge();
    return super.get(id);
  }
  ...
  override *entries(): MapIterator<[number, ProofNode]> {
...
function* verifyGraph(
  input: DeductionProposal,
  source: CheckContext,
  admission: "named" | "certificate" | "branch",
  meter?: Usage,
): Generator<CheckEvent | CertificateEvent | BranchEvent, void, void> {
  try {
    ...
    requireProof(
      !ownedAuthority || !suppliedAuthority || suppliedAuthority === ownedAuthority,
      "foreign-unique-authority",
    );
    ...
    if (admission === "branch")
      requireProof(isHypotheticalView(sourceView), "not-hypothetical-view");
    else {
      try {
        requireProof(!isHypotheticalView(sourceView), "hypothetical-primary-admission");
      } catch (error) {
        if (!(error instanceof ProofError) || error.code !== "inauthentic-candidate-view")
          throw error;
      }
    }
    ...
    const tick = (units = 1) => {
      ...
      requireProof(
        work + units <= limits.workUnits && (!remainingWork || units <= remainingWork()),
        "proof-work-limit",
      );
      work += units;
      if (meter) meter.workUnits = work;
    };
    ...
    for (const [id, node] of suppliedRetained) {
      tick();
    ...
    const problem =
      admission === "certificate"
        ? canonicalProblem(sourceView.assembly.problem)
        : sourceView.assembly.problem;
    const state = copyBounded(sourceView.state.key, NODE_BYTES).value;
    requireProof(
      state.problemKey === problem.key &&
    ...
    requireProof(
      ["unconditional", "discharged", "unique-only"].includes(source.policy) &&
        (source.uniqueEvidenceId === null || typeof source.uniqueEvidenceId === "string"),
      "invalid-proof-policy",
    );
    if (source.policy === "unique-only")
      requireProof(
        uniqueAuthorityMatches(suppliedAuthority, sourceView),
    ...
    const context: CheckContext = {
      ...
      uniqueEvidenceId: source.uniqueEvidenceId,
      uniqueAuthority: suppliedAuthority,
      authorityView: sourceView,
      view: {
        ...sourceView,
        assembly: { ...sourceView.assembly, problem },
    ...
    const captured = yield* captureProposal(
      input,
      Math.min(limits.stepBytes, limits.workspaceBytes),
      limits.stepNodes,
      tick,
    );
    let runBytes = 0;
    ...
    for (const [id, node] of retained) {
      ...
      const checked =
        acceptedNodes.get(node) ??
        (admission === "branch" && ownsBranchNode(sourceView, node)
          ? branchNodes.get(node)
          : undefined) ??
        (admission === "certificate" ? certificateAuthorities.get(node) : undefined);
      ...
      requireProof(
        authority &&
          ...
          id >= 0 &&
          authority.state.problemKey === state.problemKey &&
          (authority.state.branch === state.branch ||
            (admission === "branch" && ownsBranchNode(sourceView, node))) &&
          authority.state.revision <= state.revision,
        "inauthentic-retained-node",
      );
      if (admission === "branch")
        requireProof(ownsBranchNode(sourceView, node), "foreign-branch-prefix");
      for (const [index, premise] of node.premises.entries()) {
        ...
        requireProof(
          retained.get(premise) === authority.premises[index],
          "substituted-retained-dependency",
        );
      }
      for (const [index, ancestor] of node.scope.entries())
        requireProof(
          retained.get(ancestor) === authority.scopes?.[index],
          "substituted-retained-scope",
        );
      if (authority.table) {
        tick();
        registry.seedRetainedTable(node, authority.table);
        yield { kind: "work", units: 1 };
      }
      const encoded = copyBounded(node, NODE_BYTES);
      runBytes += encoded.bytes;
      requireProof(
        runBytes <= limits.proofBytes && runBytes <= limits.workspaceBytes,
        "proof-byte-limit",
      );
    ...
    const chargeStep = () =>
      requireProof(
        stepBytes <= limits.stepBytes &&
          runBytes + stepBytes <= limits.proofBytes &&
          runBytes + stepBytes <= limits.workspaceBytes,
        "proof-byte-limit",
      );
    chargeStep();
    ...
    requireProof(
      sameState(proposal.state, state) && sameState(proposal.proof.state, state),
      "stale-proof-state",
    );
    const effectful = proposal.effects.length > 0;
    const proof = proposal.proof;
    requireProof(
      retained.size + captured.references.length <= limits.runNodes,
      "proof-run-node-limit",
    );
    ...
    for (const reference of captured.references) {
      ...
      const copied = copyBounded(
        ...
        Math.min(
          NODE_BYTES,
          limits.stepBytes - stepBytes,
          limits.proofBytes - runBytes - stepBytes,
          limits.workspaceBytes - runBytes - stepBytes,
        ),
      );
      ...
      const nodeContext = {
        ...context,
        retained: new ProofStage(available, node.id),
      ...
      while (!next.done) {
        tick();
        yield { kind: "work", units: next.value };
        next = checking.next();
      }
      const checked = next.value;
      requireProof(
        checked.openAssumptions.every(
          (id) => node.scope.includes(id) || (node.rule === "assume@1" && id === node.id),
        ),
        "escaped-assumption",
      );
    ...
    requireProof(
      [...available.keys()].every((id) => reachable.has(id)),
      "unused-proof-node",
    );
    tick();
    ...
    if (assumption) {
      requireProof(
        lexical.length < 2 &&
          stagedNodes.length === 2 &&
          stagedNodes[0].rule === "assume@1" &&
          sameValue(stagedNodes[0].scope, lexical) &&
          stagedNodes[1].rule === "domain-restrict@1" &&
          sameValue(stagedNodes[1].scope, [...lexical, stagedNodes[0].id]) &&
          sameValue(proof.roots, [stagedNodes[1].id]) &&
          proposal.effects.length === 0,
        "invalid-branch-assumption",
      );
    ...
    requireProof(
      consequences.every(
        (item) =>
          (admission === "branch"
            ? item.openAssumptions.every((id) => resultScope.includes(id))
            : item.openAssumptions.length === 0) &&
          (!item.conditional || context.policy === "unique-only"),
      ),
      "open-proof-root",
    );
    ...
    if (
      ...
      const preparation =
        ...
        4 * scopeSlots +
        stagedNodes.length * (proposal.effects.length + 4) +
        Math.ceil(captured.header.bytes / 256);
      tick(preparation);
      yield { kind: "work", units: preparation };
    }
    if (admission === "branch" && !assumption) {
      requireProof(
        stagedNodes.every((n) => sameValue(n.scope, lexical)),
        "foreign-branch-scope",
      );
      requireProof(
        ["c01@1", "c02@1", "c03@1", "c04@1", "c05@1", "branch-graph@1"].includes(
          proposal.technique,
        ),
        "branch-technique-out-of-profile",
      );
      if (proposal.technique === "branch-graph@1")
        requireProof(
          stagedNodes.every((n) =>
            [
              "support@1",
        ...
        checkTechniqueGrammar(
          {
            ...proposal,
            proof: { ...proof, nodes: stagedNodes.map((n) => ({ ...n, scope: [] })) },
          },
          sourceView,
          available,
        );
    ...
    if (admission === "named") {
      ...
      for (const units of checkTechniqueGrammarSteps(
        { ...proposal, proof: { ...proof, nodes: stagedNodes } },
        sourceView,
        grammarSources,
        tick,
      )) {
        for (let i = 0; i < units; i++) tick();
    ...
    const checkedProposal = Object.freeze({
      ...proposal,
    ...
    if (admission === "branch") {
      const certificate = Object.freeze({
        ...
        consequences,
        scope: Object.freeze([...resultScope]),
      }) as BranchCertificate;
      ...
      yield { kind: "branch-checked", certificate };
      return;
    }
    ...
    stepUsage.set(
      step,
      Object.freeze({
        headerBytes: captured.header.bytes + Math.max(0, stagedNodes.length - 1),
        workUnits: work,
        stepBytes,
      }),
    );
...
): Generator<CertificateEvent, void, void> {
  for (const event of verifyGraph(input, context, "certificate")) {
    if (event.kind === "checked" || event.kind === "branch-checked")
      throw Error("internal-admission-error");
    yield event;
  }
}
...
export function certificateImportsMatch(
...
export function branchCertificateSource(certificate: BranchCertificate): ReadView | undefined {
  return branchSources.get(certificate);
}
export function branchNodeInference(node: ProofNode): CheckedInference | undefined {
  return branchNodes.get(node)?.inference;
}
/** No caller-selected admission mode; this entry always requires an authentic fork. */
export function verifyBranch(
  proposal: DeductionProposal,
  context: CheckContext,
): Generator<BranchEvent> {
  const meter: Usage = { workUnits: 0 };
  const cursor = (function* (): Generator<BranchEvent> {
    for (const event of verifyGraph(proposal, context, "branch", meter)) {
      if (event.kind === "checked" || event.kind === "verified")
        throw Error("internal-admission-error");
      yield event;
    }
  })();
