...
export function assertM2ProblemBounds(problem: EngineProblem): void {
  ...
  requireProof(
    Array.isArray(problem.cells) &&
      problem.cells.length > 0 &&
      problem.cells.length <= M2_ROOT_LIMITS.cells,
    "root-cell-limit",
  );
  requireProof(
    Array.isArray(problem.symbols) &&
      problem.symbols.length > 0 &&
      problem.symbols.length <= M2_ROOT_LIMITS.symbols,
    "root-symbol-limit",
  );
  requireProof(
    Array.isArray(problem.givens) && problem.givens.length === problem.cells.length,
    "root-given-limit",
  );
  requireProof(
    Array.isArray(problem.constraints) && problem.constraints.length <= M2_ROOT_LIMITS.rules,
    "root-rule-limit",
  );
}
...
function requireRootScope(cells: readonly number[]): void {
  requireProof(
    Array.isArray(cells) && cells.length > 0 && cells.length <= M2_ROOT_LIMITS.scopeCells,
    "root-scope-limit",
  );
  requireDenseRootArray(cells);
  requireProof(
    cells.every((cell) => Number.isSafeInteger(cell) && cell >= 0 && cell < M2_ROOT_LIMITS.cells),
    "invalid-root-scope",
  );
}
...
function requireRootIdentifier(id: string, suffixAllowance = 0): void {
  requireProof(
    typeof id === "string" &&
      id.length > 0 &&
      id.length <= M2_ROOT_LIMITS.identifierCharacters + suffixAllowance,
    "root-identifier-limit",
  );
}
...
export function assertM2RootAssemblyBounds(assembly: Assembly): void {
  ...
  requireProof(
    Array.isArray(assembly.allDifferent) &&
      assembly.allDifferent.length <= M2_ROOT_LIMITS.allDifferent &&
      Array.isArray(assembly.covers) &&
      assembly.covers.length <= M2_ROOT_LIMITS.covers,
    "root-capability-limit",
  );
  requireProof(
    Array.isArray(assembly.relations) && assembly.relations.length <= 256,
    "root-relation-limit",
  );
  const problem = assembly.problem;
  requireProof(
    problem.cells.length +
      problem.givens.length +
      problem.constraints.length +
      assembly.allDifferent.length +
      assembly.covers.length +
      assembly.relations.length <=
      M2_ROOT_LIMITS.nodes,
    "root-node-limit",
  );
  for (const values of [
    problem.cells,
  ...
  for (const rule of problem.constraints) {
    ...
    requireProof(
      rule.parameters !== null &&
        ...
        Object.getPrototypeOf(rule.parameters) === Object.prototype &&
        Reflect.ownKeys(rule.parameters).length <= 16,
    ...
    for (const key of Reflect.ownKeys(rule.parameters)) {
      ...
      requireProof(
        typeof key === "string" &&
          ...
          "value" in descriptor &&
          (typeof descriptor.value === "boolean" || Number.isSafeInteger(descriptor.value)),
        "invalid-root-parameters",
      );
    }
  }
  for (const relation of assembly.relations) {
    ...
    requireProof(
      relation.cells.length <= 16 &&
        Array.isArray(relation.tuples) &&
        relation.tuples.length <= 256 &&
        relation.cells.length * relation.tuples.length <= 2048,
      "root-relation-tuple-limit",
    );
    requireDenseRootArray(relation.tuples);
    for (const tuple of relation.tuples) {
      requireProof(
        Array.isArray(tuple) && tuple.length === relation.cells.length,
        "invalid-root-tuple",
      );
  ...
  for (const [capabilities, fields] of capabilityFields)
 ...
 */
export function domainAssertion(
...
export function premises(
  ...
  requireProof(
    input.premises.length === count &&
      new Set(input.premises).size === count &&
...
export function validLiteral(value: Literal, context: CheckContext): boolean {
  return (
    context.view.assembly.problem.cells.includes(value.cell) &&
    context.view.assembly.problem.symbols.includes(value.symbol) &&
...
function restrictDomain(input: PrimitiveInput, context: CheckContext): CheckedInference {
  const [base, restriction] = premises(input, context, 2),
    domain = domainAssertion(base);
  requireProof(
    domain &&
      restriction.kind === "literal" &&
      validLiteral(restriction.value, context) &&
...
function weakLink(input: PrimitiveInput, context: CheckContext): CheckedInference {
  ...
  requireProof(
    alternatives.length === 2 &&
      alternatives.every((value) => validLiteral(value, context) && !value.positive) &&
...
function coverClause(input: PrimitiveInput, context: CheckContext): CheckedInference {
  const [source] = premises(input, context, 1),
    domain = domainAssertion(source);
  let alternatives: Literal[];
  if (source.kind === "cover")
    alternatives = source.cells.map((cell) => ({ cell, symbol: source.symbol, positive: true }));
  else {
...
function domain(input: PrimitiveInput, context: CheckContext): CheckedInference {
  ...
  requireProof(
    input.premises.length === 0 && sameValue(input.parameters, {}),
    "invalid-root-parameters",
  );
  requireProof(
    conclusion.kind === "domain" &&
      problem.cells.includes(conclusion.cell) &&
      sameValue(conclusion, {
        kind: "domain",
        cell: conclusion.cell,
        mask: 2 ** problem.symbols.length - 1,
      }),
    "invalid-domain-axiom",
  );
...
function given(input: PrimitiveInput, context: CheckContext): CheckedInference {
  ...
  requireProof(
    conclusion.kind === "literal" &&
      problem.cells.includes(conclusion.value.cell) &&
      problem.givens[conclusion.value.cell] !== 0 &&
      sameValue(conclusion, {
        kind: "literal",
        value: {
          cell: conclusion.value.cell,
          symbol: problem.givens[conclusion.value.cell],
          positive: true,
        },
      }),
    "invalid-given",
...
function declaredRule(input: PrimitiveInput, context: CheckContext): CheckedInference {
  ...
  requireProof(
    sameValue(checked.conclusion, input.conclusion) &&
      sameValue(checked.openAssumptions, []) &&
      checked.conditional === false &&
      sameValue(checked.rules, [rule.id]),
    "invalid-rule-inference",
  );
...
export class PrimitiveRegistry {
  ...
  readonly #strategies: ReadonlyMap<string, Strategy> = new Map([
    ["domain-axiom@1", domain],
    ["given@1", given],
    ["rule-instance@1", declaredRule],
    ["all-different@1", declaredRule],
    ["cover@1", declaredRule],
    ["relation@1", declaredRule],
    ["domain-restrict@1", restrictDomain],
    ["weak-link@1", weakLink],
    ["cover-clause@1", coverClause],
    ["resolution@1", resolve],
    ...[
      new AssumptionStrategy(),
      new ConjunctionStrategy(),
      new ContradictionStrategy(),
      new DischargeStrategy(),
      new CasesStrategy(),
      new SupportStrategy(),
      new HallStrategy(),
      new CoverCountStrategy(),
      new AllDifferentSubsetStrategy(),
    ].map(
      (strategy) =>
        [
          strategy.id,
          (input: PrimitiveInput, context: CheckContext) => strategy.check(input, context),
        ] as const,
    ),
  ]);
  ...
  seedRetainedTable(node: ProofNode, definition: TableDefinition): void {
    this.#tables.seedRetained(node, definition);
  }

  tableDefinition(node: ProofNode): TableDefinition | undefined {
    return this.#tables.get(node);
  }
  ...
  has(id: string): boolean {
    return (
      ...
      this.#tables.has(id) ||
      id === this.#subsetCount.id ||
      id === this.#countClause.id ||
  ...
  *checkSteps(
    input: PrimitiveInput,
    context: CheckContext,
  ): Generator<number, CheckedInference, void> {
