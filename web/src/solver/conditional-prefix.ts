...
export interface ConditionalPrefixDescriptor { readonly bundles: number; readonly nodes: number; readonly bytes: number; readonly digest: string; }
...
function encode(value: unknown): Uint8Array {
  return encoder.encode(JSON.stringify(value, (_key, item) => item && typeof item === "object" && !Array.isArray(item)
...
function shape(value: unknown, keys: readonly string[]): void {
  requireProof(value !== null && typeof value === "object" && !Array.isArray(value) &&
    (Object.getPrototypeOf(value) === Object.prototype || Object.getPrototypeOf(value) === null), "conditional-prefix-data");
  const actual = Reflect.ownKeys(value);
  requireProof(actual.length === keys.length && actual.every(k => typeof k === "string" && keys.includes(k)), "conditional-prefix-fields");
 ...
 */
export async function captureConditionalPrefix(input: readonly DeductionProposal[], limits: Limits, workspace: IndexWorkspace,
  charge: (units: number) => void, active: () => boolean, copy: boolean): Promise<CapturedConditionalPrefix> {
  boundedArray(input, limits.runNodes);
  const bundleCount=input.length;
  const lease = workspace.reserve(bundleCount+1, bundleCount*64+128), proposals: DeductionProposal[] = [];
  let nodes = 0, bytes = 0, done = false;
  const checkpoint = (units = 1) => { workspace.checkpoint(); charge(units); requireProof(active(), "revoked-unique-authority"); };
  ...
  try {
    ...
    for (let b = 0; b < bundleCount; b++) {
      checkpoint();
      const proposal = data(input,String(b)); shape(proposal,["technique","state","effects","proof","pattern"]);
      const proof = data(proposal,"proof"); shape(proof,["state","nodes","imports","roots"]);
      const wireNodes = data(proof,"nodes"); boundedArray(wireNodes,limits.stepNodes);
      const nodeCount=wireNodes.length;
      nodes += nodeCount; requireProof(nodes <= limits.runNodes, "conditional-prefix-node-limit");
      const header = {technique:data(proposal,"technique"),state:data(proposal,"state"),effects:data(proposal,"effects"),pattern:data(proposal,"pattern"),
        proof:{state:data(proof,"state"),imports:data(proof,"imports"),roots:data(proof,"roots"),nodes:[]}};
      const bundleLease = workspace.reserve(1,(nodeCount+1)*48+16384*12);
      try {
        requireProof(Reflect.ownKeys(wireNodes).length===nodeCount+1,"conditional-prefix-array");
        ...
        for(let n=0;n<nodeCount;n++) {
          checkpoint(); const nodeLease=workspace.reserve(1,16384*12);
          try {
            const node=captureProofRecord(data(wireNodes,String(n)) as ProofNode,16384);
            bundleBytes+=node.bytes;requireProof(bundleBytes<=limits.stepBytes,"conditional-prefix-step-limit");
            framed.set(await hash(encode(node.value)),(n+1)*32);
            if(copy){lease.grow(1,node.bytes*2+512);copiedNodes.push(node.value);}
          }finally{nodeLease.dispose();}
        }
        bytes+=bundleBytes;requireProof(bytes<=limits.proofBytes,"conditional-prefix-byte-limit");
        ...
        proposals.push(copy?Object.freeze({...captured.value,proof:Object.freeze({...captured.value.proof,nodes:Object.freeze(copiedNodes)})}):proposal);
      }finally{bundleLease.dispose();}
    }
    bundleHashes.set(await hash(encoder.encode(`conditional-prefix@1:${bundleCount}:${nodes}:${bytes}`)),0);
    const digest=Array.from(await hash(bundleHashes),b=>b.toString(16).padStart(2,"0")).join("");
    done=true;
    return Object.freeze({descriptor:Object.freeze({bundles:bundleCount,nodes,bytes,digest}),proposals:Object.freeze(proposals),lease});
  }finally{prefixBufferLease?.dispose();if(!done)lease.dispose();}
}
