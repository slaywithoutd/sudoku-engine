...
export type HumanEvent =
  ...
  | {
      ...
      readonly human: "solved" | "stalled-within-profile" | "incomplete" | "contradiction";
...
function stopStatus(
  view: ReadView,
  event: Extract<SelectionEvent, { kind: "logical-stop" }>,
): HumanStopStatus {
  if (
    event.reason === "logical-budget" ||
    event.reason === "selection-window" ||
    event.reason === "work-limit" ||
    event.reason === "time-limit"
  )
    return "incomplete";
  if (event.complete)
    return isWitness(view.assembly.problem, view.assembly, view.state.values)
      ? "solved"
...
export function* humanSteps(
  initial: ReadView,
  registry: TechniqueJobs,
  options: HumanOptions,
): Generator<HumanEvent, void, HumanAcceptance | undefined> {
  let view = initial;
  const accepted: CheckedStep[] = [];
  let selection: StepSelection | undefined;
  let activeCursor: Generator<SelectionEvent, void, unknown> | undefined;
  let nextStep = 0;
  try {
    try {
      selection = new StepSelection(view, registry, options);
    } catch (error) {
      if (error instanceof Error && /work-limit|time-limit/.test(error.message)) {
        yield {
          kind: "logical-stop",
          human: "incomplete",
          reason: "logical-budget",
    ...
    for (;;) {
      ...
      for (;;) {
        ...
        const done = cursor.next();
        if (!done.done) throw Error("selection-not-drained");
        break;
      }
      if (proposal) {
        const ack = yield {
          ...
          stepId: ++nextStep,
          step: proposal.step,
          budgetLimited: proposal.budgetLimited,
        };
        if (ack !== "accepted") {
          if (ack === "human-stopped") {
            yield {
              kind: "logical-stop",
              human: "incomplete",
              reason: "logical-budget",
        ...
        const next = options.accept(proposal.step);
      ...
      if (stopped) {
        const human = stopStatus(view, stopped);
        yield {
          kind: "logical-stop",
          human,
          reason: human === "incomplete" ? "logical-budget" : stopped.reason,
          accepted: Object.freeze([...accepted]),
        };
        return;
      }
