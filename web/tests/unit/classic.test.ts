...
import { conflictingCells, effectiveValues, isComplete, parsePuzzleString } from '../../src/domain/classic';
...
test('strict string syntax is independent of conflicts and completion', () => {
  ...
  for (const text of ['0'.repeat(80), '0'.repeat(82), '0'.repeat(80)+'x']) expect(() => parsePuzzleString(text)).toThrow();
...
test.each([[0,27], [0,10], [0,3]])('finds both participants %i and %i', (a,b) => {
  const values = Array(81).fill(0); values[a]=4; values[b]=4;
  expect(conflictingCells(values)).toEqual([a,b]);
});
test('effective values prioritize givens without mutating editor', () => {
  const editor = emptyEditor(); editor.cells[1].value=3;
  const givens = Array(81).fill(0); givens[0]=5;
