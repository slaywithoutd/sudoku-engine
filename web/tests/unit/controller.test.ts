...
test("live edits are immediate and older commit cannot mark newer work saved", async () => {
  ...
  const pending: {
    data: LibraryData;
    expected: number;
    resolve: (data: LibraryData) => void;
  }[] = [];
  const c = createController(
    {
      load: async () => stored,
      close() {},
      commit: (data, expected) =>
        new Promise((resolve) => pending.push({ data, expected, resolve })),
    },
    stored,
  );
  c.update((d) => createDraft(d, "a", NOW));
  c.update((d) => createDraft(d, "b", NOW));
  expect(Object.keys(c.snapshot().drafts)).toEqual(["a", "b"]);
  expect(c.status().kind).toBe("saving");
  const first = pending.shift()!;
  stored = { ...first.data, revision: 1 };
  first.resolve(stored);
  await Promise.resolve();
  expect(c.status().kind).toBe("saving");
  expect(pending[0].expected).toBe(1);
  const last = pending.shift()!;
  stored = { ...last.data, revision: 2 };
  last.resolve(stored);
  await c.flush();
  expect(Object.keys(stored.drafts)).toEqual(["a", "b"]);
  expect(c.status().kind).toBe("saved");
});
test("failure keeps memory; retry commits; no-op and unsubscribe do not produce work", async () => {
  ...
  expect(c.snapshot().drafts.a).toBeDefined();
  off();
...
test("revision conflicts never advance expected revision on retry", async () => {
  ...
  const c = createController(
    {
      ...
      commit: async (_, r) => {
        revisions.push(r);
        throw new RevisionConflictError("other tab");
      },
    },
    emptyLibrary(),
  );
  ...
  await expect(c.retry()).rejects.toThrow();
  expect(revisions).toEqual([0, 0]);
});
test("concurrent retry requests settle as saved without duplicate commits", async () => {
  let fail = true,
    resolve: ((d: LibraryData) => void) | undefined,
    pending: LibraryData | undefined,
    calls = 0;
  const c = createController(
    {
      load: async () => emptyLibrary(),
      ...
      commit: async (d, r) => {
        calls++;
        if (fail) throw new Error("quota");
        pending = { ...d, revision: r + 1 };
        return new Promise<LibraryData>((done) => {
          resolve = done;
        });
      },
    },
  ...
  c.update((d) => createDraft(d, "d", NOW));
  await expect(c.flush()).rejects.toThrow();
  fail = false;
  const first = c.retry(),
    second = c.retry();
