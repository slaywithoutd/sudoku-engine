...
test("finish archives authoring; resume and edit-copy preserve puzzle/session", () => {
  ...
  d = finishDraft(d, "d1", "p1", NOW);
  d = startPlay(d, "p1", NOW);
  const original = structuredClone(d);
  expect(startPlay(d, "p1", NOW)).toBe(d);
  d = copyPuzzleToDraft(d, "p1", "d2", NOW);
  expect(d.puzzles).toEqual(original.puzzles);
  expect(d.sessions).toEqual(original.sessions);
  expect(d.drafts.d2.sourcePuzzleId).toBe("p1");
  expect(d.drafts.d2.editor.past).toEqual([]);
...
test("delete removes session/archive, drops provenance and preserves active copy clues", () => {
  ...
  d = copyPuzzleToDraft(d, "p", "copy", NOW);
  const cells = structuredClone(d.drafts.copy.editor.cells);
  d = deleteRecord(d, "puzzle", "p");
  expect(d.puzzles).toEqual({});
  expect(d.sessions).toEqual({});
  expect(Object.keys(d.drafts)).toEqual(["copy"]);
  expect(d.drafts.copy.sourcePuzzleId).toBeUndefined();
  expect(d.drafts.copy.editor.cells).toEqual(cells);
