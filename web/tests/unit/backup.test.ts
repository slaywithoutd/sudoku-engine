...
import { exportBackup, parseBackup, previewRestore, validateLibrary } from '../../src/domain/backup';
...
test('roundtrip keeps hidden notes, redo, settings; identical records skip', () => {
  ...
  s=reduceEditor(ctx,s,{type:'digit',digit:5,corner:false}); s=reduceEditor(ctx,s,{type:'undo'});
  d.sessions.p.editor=s; d.settings.showConflicts=true;
  const parsed=envelope(d); expect(parsed.data).toEqual(d);
  const preview=previewRestore(d,parsed,()=> 'copy',false);
  expect(preview).toMatchObject({added:0,copied:0,skipped:3}); expect(preview.data).toEqual(d);
});
test('changed session copies its identical puzzle and archived history branch', () => {
  const current=fixture(),incoming=structuredClone(current); incoming.sessions.p.editor.selected=1;
  let n=0; const before=structuredClone(current);
  const result=previewRestore(current,envelope(incoming),()=>`new${++n}`,false);
  expect(result.copied).toBe(3); expect(current).toEqual(before);
  ...
  expect(Object.values(result.data.drafts).some(d=>d.id!=='d'&&d.finishedPuzzleId===imported.id)).toBe(true);
...
test('changed puzzle remaps two draft links and retries generated collisions', () => {
  let incoming=copyPuzzleToDraft(fixture(),'p','copy',NOW); incoming.puzzles.p.name='Changed';
...
test('settings remain local unless requested; revision remains local', () => {
  const current=emptyLibrary(); current.revision=4; const incoming=emptyLibrary(); incoming.revision=99; incoming.settings.showConflicts=true;
  expect(previewRestore(current,envelope(incoming),()=> 'x',false).data).toMatchObject({revision:4,settings:{showConflicts:false}});
  expect(previewRestore(current,envelope(incoming),()=> 'x',true).data.settings.showConflicts).toBe(true);
});
test.each([
  (x:any)=>x.version=2,
  ...
  (x:any)=>Object.defineProperty(x.data.drafts,'__proto__',{value:x.data.drafts.d,enumerable:true}),
])('rejects malformed payload %# without touching source', mutate => {
  const x=JSON.parse(exportBackup(fixture(),NOW)); mutate(x); expect(()=>parseBackup(JSON.stringify(x))).toThrow();
});
test('rejects inconsistent past/future and given edits even in hidden history', () => {
  const d=fixture(); const cell={value:0 as const,notes:[]};
  d.sessions.p.editor.past=[{label:'digit',changes:[{index:0,before:cell,after:{value:2,notes:[]}}]}];
  expect(()=>validateLibrary(d)).toThrow();
  d.sessions.p.editor.past=[]; d.sessions.p.editor.future=[{label:'digit',changes:[{index:0,before:{value:2,notes:[]},after:cell}]}];
  expect(()=>validateLibrary(d)).toThrow();
  d.puzzles.p.definition.givens[0]=2; d.drafts.d.editor.cells[0].value=2;
  d.sessions.p.editor.future=[{label:'digit',changes:[{index:0,before:cell,after:{value:2,notes:[]}}]}];
  expect(()=>validateLibrary(d)).toThrow();
});
