...
function index(view = fixture()) {
  const events = [...buildAls(view, budget())],
    end = events.at(-1);
  if (end?.kind !== "ready") throw Error("missing-ready");
  return end.value;
}
...
test("RCC permits overlap lacking its digit and rejects a shared tested-digit occurrence", () => {
  const view = overlapView();
  expect(view.state.domains.slice(0, 3)).toEqual([3, 6, 3]);
  const als = index(view),
    a = als.entries.find((e) => e.cells.join() === "0,1")!,
    b = als.entries.find((e) => e.cells.join() === "1,2")!;
  ...
  for (let x = 1; x <= 3; x++)
    for (let y = 1; y <= 3; y++)
      for (let z = 1; z <= 3; z++)
        if (x !== 3 && z !== 3 && y !== 1 && x !== y && x !== z && y !== z)
          assignments.push([x, y, z]);
  expect(assignments.length).toBeGreaterThan(0);
  for (const values of assignments)
    expect(Number(values[0] === 1) + Number(values[2] === 1)).toBeLessThanOrEqual(1);
});
...
test("independent subset oracle enumerates all ALS occurrences, including singleton and five-cell boundaries", () => {
  ...
  for (const fact of view.facts.values())
    if (fact.proposition.kind === "all-different") {
      ...
      for (let membership = 1; membership < 2 ** scope.length; membership++) {
        ...
        const symbols = view.assembly.problem.symbols.filter((s) =>
          cells.some((c) => Math.floor(view.state.domains[c] / 2 ** (s - 1)) % 2 === 1),
        );
        if (symbols.length === cells.length + 1) expected.push({ source: fact.id, cells, symbols });
      }
    }
  const key = (e: { source: number; cells: readonly number[]; symbols: readonly number[] }) =>
    JSON.stringify(e);
  expect(
    als.entries
      .map((e) => key({ source: e.recipe.source, cells: e.cells, symbols: e.symbols }))
      .sort(),
  ...
  for (const symbols of [6, 7]) {
    ...
    const result = assemble(
      canonicalProblem({
        schema: 1,
        cells,
        symbols: cells.map((c) => c + 1),
        givens: cells.map(() => 0),
        constraints: [{ id: "house", type: "all-different@1", cells, parameters: {} }],
      }),
      mockRuleRegistry,
    );
    if (!result.ok) throw Error("fixture");
...
test("RCC cancellation and work interruption cannot be misreported as false", () => {
  ...
  rest.dispose();
  expect(workspace.usage).toEqual(used);
  cancelled = true;
...
test("RCC cursors resume across work quanta and release query scratch on close or cancellation", () => {
  ...
  const als = end.value,
    a = als.entries.find((e) => e.cells.join() === "0,1")!,
    b = als.entries.find((e) => e.cells.join() === "1,2")!;
  const baseline = workspace.usage,
    query = als.rccSteps(a, b, 1);
  ...
  expect(workspace.usage).toEqual(baseline);
  const interrupted = als.rccSteps(a, b, 1);
  interrupted.next();
  cancelled = true;
  ...
  cancelled = false;
  const disposed = als.rccSteps(a, b, 1);
  disposed.next();
  als.dispose();
...
test("RCC rejects an unseen cross-pair and overlaps, charging every tested pair", () => {
  const als = index();
  const a = als.entries.find((e) => e.cells.join() === "0,1")!,
    b = als.entries.find((e) => e.cells.join() === "2,3")!;
  let work = 0;
  expect(
    als.rcc(a, b, 1, (units) => {
      work += units;
  ...
  const c = complete.entries.find((e) => e.cells.join() === "0,1")!,
    d = complete.entries.find((e) => e.cells.join() === "2,3")!;
...
test("ALS enforces <=5 cells, authenticates before field access and releases interrupted reservations", () => {
  ...
  expect([...buildAls(fixture(), limited)].at(-1)).toEqual({
    kind: "interrupted",
