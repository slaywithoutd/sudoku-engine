...
function fixture(cells = 4, symbols = 9, givens = Array(cells).fill(0)): ReadView {
  const result = assemble(
    canonicalProblem({
      ...
      symbols: Array.from({ length: symbols }, (_, i) => i + 1),
      givens,
      constraints: [],
    }),
  ...
  const table = (
    ...
    add(
      ...
      premises,
      {
        ...
        cells,
        count,
        definition: context.retained.size + nodes.length,
      } as Proposition,
      parameters,
    );
  const proposal = (roots: number[]): DeductionProposal => ({
...
test("count-only omission preserves weighted inequalities, provenance and complete support contracts", () => {
  const result = assemble(
    canonicalProblem({
      ...
      constraints: [
        ...
        { id: "capacity", type: "all-different@1", cells: [1, 2], parameters: {} },
      ],
    }),
    [new AllDifferentRule()],
  );
  if (!result.ok) throw Error("count-fixture");
  const view = initialize(result.value, "primary");
  const cover = [...view.facts.values()].find(
    (f) =>
      f.proposition.kind === "cover" &&
      f.proposition.symbol === 1 &&
      f.proposition.cells.join() === "0,1",
  )!.id;
  const capacity = [...view.facts.values()].find(
    (f) => f.proposition.kind === "all-different" && f.proposition.cells.join() === "1,2",
  )!.id;
  ...
  for (let lower = 1; lower <= 3; lower++)
    for (let upper = 1; upper <= 3; upper++) {
      const b = builder(view),
        root = b.add(
          "cover-count@1",
          [cover, capacity, view.state.domainFacts[0]],
          { kind: "literal", value: { cell: 2, symbol: 1, positive: false } },
          {
            symbol: 1,
            covers: [{ premise: cover, coefficient: lower }],
            capacities: [{ premise: capacity, coefficient: upper }],
          },
        );
      const terminal = b.check([root]);
      expect(terminal.kind, `weights ${lower}/${upper}`).toBe(
        upper >= lower ? "verified" : "rejected",
      );
      if (terminal.kind === "verified")
        expect(terminal.certificate.consequences[0]).toMatchObject({
          rules: ["base", "capacity"],
          conditional: false,
          openAssumptions: [],
        });
  ...
  const b = builder(view),
    support = b.add("support@1", [cover, view.state.domainFacts[0]], {
      kind: "cover",
      symbol: 1,
      cells: [1],
    });
  expect(b.check([support])).toMatchObject({
    kind: "rejected",
    code: "incomplete-domain-evidence",
  });
});
...
test("checks a 6,561-row Cartesian table as a bounded coverage DAG", () => {
  ...
  for (let a = 0; a < 9; a++) {
    ...
    for (let c = 0; c < 9; c++) {
      ...
      row =
        row === undefined
          ? leaf
          : b.table("table-union@1", [row, leaf], [0, 1, 2, 3], (c + 1) * 81);
    }
    rows.push(row!);
  }
  let root = rows[0];
  for (let a = 1; a < 9; a++)
    root = b.table("table-union@1", [root, rows[a]], [0, 1, 2, 3], (a + 1) * 729);
  ...
  expect(
    Math.max(...b.nodes.map((node) => new TextEncoder().encode(JSON.stringify(node)).length)),
  ).toBeLessThan(16384);
...
test.each([false, true])(
  "joint sum/order/row inference forces A=4 with both registry orders: %s",
  (reverse) => {
    const result = assemble(
      canonicalProblem({
        schema: 1,
        ...
        givens: [0, 0, 1, 2, 3],
        constraints: [
          { id: "sum:0", type: "sum@1", cells: [0, 1], parameters: { total: 10 } },
          { id: "order:0", type: "order@1", cells: [0, 1], parameters: {} },
          { id: "row:0", type: "all-different@1", cells: [0, 2, 3, 4], parameters: {} },
        ],
      }),
      reverse ? [...mockRuleRegistry].reverse() : mockRuleRegistry,
    );
    if (!result.ok) throw Error("fixture");
    const view = initialize(result.value, "primary"),
      b = builder(view);
    ...
    const joined = b.table(
      "table-join@1",
      relations.map((f) => f.root),
      [0, 1],
      4,
    );
    const rowTable = b.table(
      "table-filter@1",
      [0, ...[2, 3, 4].map((cell) => view.state.domainFacts[cell]), row.root],
      [0, 2, 3, 4],
      6,
      { cells: [0, 2, 3, 4], box: [511, 1, 2, 4] },
    );
    const mixed = b.table("table-join@1", [joined, rowTable], [0, 1, 2, 3, 4], 1);
    const place = b.add("table-project@1", [mixed], {
      kind: "literal",
      value: { cell: 0, symbol: 4, positive: true },
    });
    const domain = b.add("table-project@1", [mixed], { kind: "domain", cell: 0, mask: 8 });
    const proposal = {
      ...b.proposal([place, domain]),
      effects: [{ kind: "place", cell: 0, symbol: 4 } as const],
      pattern: { kind: "propagation" },
    };
    expect([...verifyCertificate(proposal, b.context)].at(-1)).toMatchObject({
      kind: "verified",
      certificate: {
        proposal: { effects: [{ kind: "place", cell: 0, symbol: 4 }] },
        consequences: [
          { rules: ["order:0", "row:0", "sum:0"] },
          { rules: ["order:0", "row:0", "sum:0"] },
        ],
      },
    });
    expect(assertSound(view, proposal).proposal.effects).toEqual([
      { kind: "place", cell: 0, symbol: 4 },
    ]);
...
test("replays a one-hole completion from original clues without discovery", () => {
  const givens = Array.from(SOLUTION, Number);
  givens[0] = 0;
  ...
  const single = [
    ...getTechniques("classic-expanded@1")[0].discover(view, discoveryContext()),
  ].find((e) => e.kind === "proposal");
  if (single?.kind !== "proposal") throw Error("single");
  proposals.push(single.proposal);
  const snapshot = makeSnapshot(problem, { kind: "manual" }, "replay", 0);
  expect([...replay(snapshot, proposals, result.value, limits)].at(-1)).toMatchObject({
    kind: "checked",
  });
  const altered = structuredClone(proposals);
  (altered.at(-1)!.effects[0] as { symbol: number }).symbol = 4;
...
test("exhaustively checks all nonempty three-symbol domain boxes under sum/order in both registry orders", () => {
  for (const reverse of [false, true]) {
    ...
    if (!result.ok) throw Error("fixture");
    const view = initialize(result.value, "primary");
    const relations = [...view.facts.values()]
      .filter((f) => f.proposition.kind === "relation")
      .map((f) => f.root);
    for (let a = 1; a < 8; a++)
      for (let c = 1; c < 8; c++) {
        ...
        const b = builder(view),
          root = b.table("table-filter@1", [0, 1, ...relations], [0, 1], count, {
            cells: [0, 1],
            box: [a, c],
          });
...
test("table enumeration yields before completion and rejects a depleted work budget", () => {
  ...
  const events = [
    ...verifyCertificate(b.proposal([root]), {
      ...b.context,
      limits: { ...limits, workUnits: 20 },
    }),
  ];
...
test("certificate-only tables preserve exact definitions without candidate or replay authority", () => {
  const view = fixture(2, 2),
    b = builder(view),
    session = new CertificateSession(b.context);
  const table = b.table("table-filter@1", [0, 1], [0, 1], 4, { cells: [0, 1], box: [3, 3] });
  const first = b.check([table]);
  if (first.kind !== "verified") throw Error("fixture proof");
  session.retain(first.certificate);
  const other = fixture(2, 2),
    unrelated = new CertificateSession({
      ...b.context,
      view: other,
      retained: retainedProof(other),
    });
  ...
  expect([...checkProposal(c.proposal([root]), session.context)].at(-1)).toMatchObject({
    kind: "rejected",
    code: "inauthentic-retained-node",
  });
  const snapshot = makeSnapshot(view.assembly.problem, { kind: "manual" }, "cached", 0);
  expect([...replay(snapshot, [b.proposal([table])], view.assembly, limits)].at(-1)?.kind).toBe(
    "rejected",
  );
});
...
test("inline relation projection preserves correlations and deduplicates every complete projected row", () => {
  ...
  const table = b.table("table-filter@1", [0, 1, 2], [0, 1, 2], 8, {
    cells: [0, 1, 2],
    box: [3, 3, 3],
  });
  const root = b.add("table-project@1", [table], {
    kind: "relation",
    cells: [0, 2],
    tuples: [
      [1, 1],
...
test("table projections reject ignored fields on their literal claims", () => {
  ...
  const root = b.add("table-project@1", [table], {
    kind: "literal",
    value: { cell: 0, symbol: 2, positive: false },
    ignored: true,
  } as Proposition);
...
test("cases cannot equate different tables merely because their cells and row counts match", () => {
  const b = builder(fixture(2, 2));
  const clause = b.add("cover-clause@1", [1], {
    kind: "clause",
    alternatives: [
      { cell: 1, symbol: 1, positive: true },
      { cell: 1, symbol: 2, positive: true },
    ],
  });
  const a = b.add("assume@1", [], {
    kind: "literal",
    value: { cell: 1, symbol: 1, positive: true },
  });
  const first = b.table("table-filter@1", [0, a], [0, 1], 2, { cells: [0, 1], box: [3, 1] });
  b.nodes[b.nodes.length - 1] = { ...b.nodes.at(-1)!, scope: [a] };
  const c = b.add("assume@1", [], {
    kind: "literal",
  ...
  const root = b.add(
    "cases@1",
    [clause, a, first, c, second],
    b.nodes.find((node) => node.id === first)!.conclusion,
  );
  expect(b.check([root])).toMatchObject({ kind: "rejected", code: "invalid-case-conclusion" });
});

test.each(["invalid", "work", "workspace", "nodes", "time"])(
  "empty replay still enforces startup budget: %s",
  (limit) => {
    const view = fixture(2, 2),
      snapshot = makeSnapshot(view.assembly.problem, { kind: "manual" }, "empty", 0);
...
function namedPath() {
  ...
  const assembly = assemble(problem, [new AllDifferentRule()]);
  if (!assembly.ok) throw Error("fixture");
  const initial = initialize(assembly.value, "primary");
  const event = [
    ...assembly.value.modules.get("row:0")!.propagate(initial, problem.constraints[0]),
  ].find((e) => e.kind === "proposal");
  if (event?.kind !== "proposal") throw Error("fixture");
  const first = [
    ...checkProposal(event.proposal, {
      view: initial,
      retained: retainedProof(initial),
      limits,
      policy: "unconditional",
      uniqueEvidenceId: null,
    }),
  ...
  const view = commitChecked(initial, first.step).view,
    b = builder(view);
  const root = b.add("cover-clause@1", [view.state.domainFacts[1]], {
    kind: "literal",
    value: { cell: 1, symbol: 2, positive: true },
  });
  const second: DeductionProposal = {
    ...b.proposal([root]),
    technique: "c01@1",
    pattern: { kind: "single", alias: "Naked Single", cell: 1, symbol: 2, house: null },
  };
  const checked = [...checkProposal(second, b.context)].at(-1);
  if (checked?.kind !== "checked") throw Error("cache");
  return {
    initial,
    proposals: [event.proposal, second],
    steps: [first.step, checked.step],
    snapshot: makeSnapshot(problem, { kind: "manual" }, "named-path", 0),
  };
}

test("replay charges cumulative headers even when every named bundle fits individually", () => {
  const path = namedPath(),
    bytes = (value: unknown) => new TextEncoder().encode(JSON.stringify(value)).length;
  const cap =
    [...retainedProof(path.initial).values()].reduce((sum, node) => sum + bytes(node), 0) +
    path.proposals.reduce((sum, p) => sum + bytes(p), 0) -
    1;
  const events = [
    ...replay(path.snapshot, path.proposals, path.initial.assembly, { ...limits, proofBytes: cap }),
  ];
...
test("replay includes checker bookkeeping in the cumulative work limit", () => {
  const path = namedPath(),
    estimator = initializationReservation(path.initial.assembly);
  let startup = 1,
    next = estimator.next();
  while (!next.done) {
    if (next.value.kind === "work") startup += next.value.units;
    next = estimator.next();
  }
  startup += next.value.workUnits;
  const cap = startup + path.steps.reduce((sum, step) => sum + checkedWorkUnits(step), 0) - 1;
  const events = [
    ...replay(path.snapshot, path.proposals, path.initial.assembly, { ...limits, workUnits: cap }),
  ];
  expect(events.filter((e) => e.kind === "checked")).toHaveLength(1);
...
test("an empty table proves exactly false and rejects unsupported conclusion fields", () => {
  ...
  b.nodes[1] = {
    ...b.nodes[1],
    conclusion: { kind: "false", ignored: true } as unknown as Proposition,
  };
  expect(b.check([root])).toMatchObject({ kind: "rejected", code: "invalid-table-projection" });
});
