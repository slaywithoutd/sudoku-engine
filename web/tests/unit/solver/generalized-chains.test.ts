...
import { candidateLiteral, buildCspVariables } from "../../../src/solver/techniques/csp-variables";
import {
...
import { independentGeneralized, independentOrForcing } from "../../solver/generalized-acceptance";
...
function orFixture(
  ...
  if ("sourceCertificateRecipe" in fixture) {
    ...
    ordinary.mode = "cache";
    ordinary.positions[ordinary.orPosition].variable = ordinary.clauseProof.variable;
  ...
  const proposal =
    p.kind === "or-forcing"
      ...
      : (assembly === "production" ? compileGeneralized : independentGeneralized)(
          view,
          p as GeneralizedPlan,
        );
...
test.each([
  ...
  (_name, compile) => {
    const { view, p } = orFixture(c28.find((f) => f.id === "C28-generalized-C25-z")!);
    ...
    expect(
      proposal.proof.nodes.find((n) => n.id === certificate.branches[index].result)?.conclusion,
    ).toEqual({ kind: "false" });
...
test.each(["bivalue", "braid"] as const)(
  ...
  (grammar) => {
    ...
    const { view, proposal, sourceProposal, p } = orFixture(fixture, "independent", grammar);
    expect(sourceProposal.technique).toBe(grammar === "bivalue" ? "c25@1" : "c27@1");
    ...
    expect(
      [
        ...replay(
          ...
          [...originalCluePrefix(fixture as unknown as TechniqueFixture), sourceProposal, proposal],
          view.assembly,
...
test.each(
  ...
  (fixture) => {
    const { view, proposal, p, sourceProposal } = orFixture(fixture, fixture.assembly);
    if ("expectedAdmission" in fixture && fixture.expectedAdmission === "out-of-profile") {
      const ctx = {
        view,
        retained: retainedProof(view),
        policy: "discharged" as const,
...
test.each([...c25, ...c26, ...c27])(
  ...
  (fixture) => {
    ...
    expect(result, JSON.stringify(result)).toMatchObject({
      kind:
        "expectedAdmission" in fixture && fixture.expectedAdmission === "out-of-profile"
          ? "rejected"
          : "checked",
    });
    ...
    if (result?.kind === "checked") {
      ...
      expect(
        [
          ...replay(
            { problem: view.assembly.problem } as SolverSnapshot,
            [...originalCluePrefix(fixture as unknown as TechniqueFixture), independent],
            view.assembly,
            discoveryContext().limits,
          ),
        ].at(-1)?.kind,
...
test.each(generalizedTechniques)(
  ...
  (descriptor) => {
    const fixture =
      descriptor.id === "c27@1" ? c27.find((f) => f.id === "C27-gwhip-group2")! : c26[0];
    const view = fixtureView(fixture as unknown as TechniqueFixture),
    ...
    try {
      for (const event of cursor) {
        ...
        if (plan.grammar === "z" && !plan.positions.slice(0, -1).some((p) => p.excluded.length))
          continue;
        if (plan.grammar === "t" && !plan.positions.slice(0, -1).some((p) => p.excluded.length))
          continue;
...
test("same-revision source publication invalidates completeness, preserves exact facts, and cannot repeat retention", () => {
  ...
  expect(() => retainCheckedFacts(view, checked.step)).toThrow("reused-proof-node");
...
test("a primitive-valid shorter proof cannot decorate or replace a complete ordered root", () => {
  ...
  const long = compileGeneralized(view, f.expectedPattern as unknown as GeneralizedPlan),
    short = compileGeneralized(
      view,
      c25.find((f) => f.id === "C25-z")!.expectedPattern as unknown as GeneralizedPlan,
    );
  ...
  mixed.proof.imports = [...new Set([...long.proof.imports, ...short.proof.imports])].sort(
    (a: any, b: any) => a - b,
  );
  const context = {
    view,
...
test.each(["cover-only", "extra-root", "effectful", "missing-position"])(
  ...
  (mutation) => {
    const { sourceProposal } = orFixture(c28.find((f) => f.id === "C28-inserted-or2")!),
      view = fixtureView(
        c28.find((f) => f.id === "C28-inserted-or2")! as unknown as TechniqueFixture,
      ),
      p = structuredClone(sourceProposal) as any;
    if (mutation === "cover-only") p.proof.roots = [p.pattern.certificate.positions[0].cover];
    if (mutation === "extra-root") p.proof.roots.push(p.pattern.certificate.positions[0].cover);
...
test.each(["source", "missing-case", "differing-effect", "inserted-alias", "source-sibling"])(
  "OR forcing rejects %s",
  (mutation) => {
...
test.each(["missing-alternative", "prefix", "suffix", "second-or", "thirteenth"])(
  "inserted OR rejects %s",
  (mutation) => {
    const { view, proposal } = orFixture(c28.find((f) => f.id === "C28-inserted-or3")!),
      p = structuredClone(proposal) as any,
      i = p.pattern.positions.findIndex((s: any) => s.role === "or");
    if (mutation === "missing-alternative") p.pattern.positions[i].alternatives.pop();
    if (mutation === "prefix")
      p.pattern.positions[i].leftConflict = p.pattern.positions.at(-2).right;
    if (mutation === "suffix") p.pattern.positions[i + 1].leftConflict = p.pattern.target;
    if (mutation === "second-or") p.pattern.positions[0].role = "or";
    if (mutation === "thirteenth") p.pattern.positions = Array(13).fill(p.pattern.positions[0]);
    expect(
      [
        ...checkProposal(p, {
          view,
...
test("OR case admits a generalized same-effect branch without a second assumption", () => {
  ...
  assertSound(view, proposal);
  expect(proposal.proof.nodes.filter((n) => n.rule === "assume@1")).toHaveLength(2);
...
test("a genuine 25-link OR branch is primitive-valid and rejected by the named static cap", () => {
  const { view, p } = orFixture(c28.find((f) => f.id === "C28-signed-or2-static24")!),
    plan = structuredClone(p) as any;
  const house = view.assembly.allDifferent.find(
    (h) =>
      h.cells.includes(49) &&
      h.cells.some((c) => c !== 49 && !view.state.values[c] && view.state.domains[c] & 2),
  )!;
...
test("signed retained OR clauses support forcing but cannot be inserted as positive candidates", () => {
  ...
  for (const e of buildProvedClauses(view, context.workspace))
    if (e.kind === "ready") {
      const entry = e.value.entries.find((e) => e.alternatives.every((a) => !a.positive))!;
  ...
  try {
    for (const e of cursor)
      if (e.kind === "proposal") {
        ...
        if (p.kind !== "or-forcing" || !p.alternatives.some((a: any) => !a.positive)) continue;
...
test("production target search discovers real nonpredecessor braid structure", () => {
  ...
  try {
    for (const e of cursor) {
      ...
      if (e.kind !== "plan" || !generalizedFeatures(e.plan).split(":").includes("true")) continue;
      const positions = e.plan.positions;
      const nonpredecessor = positions.some(
        (p, i) =>
          i > 0 && JSON.stringify(p.leftConflict) !== JSON.stringify(positions[i - 1].right),
      );
...
test.each([
  ...
  if (mutation === "omit-alternative") p.pattern.positions[3].alternatives.pop();
  if (mutation === "t-interior-z") {
    p.pattern.grammar = "t";
    p.pattern.alias = "t-whips";
  }
  if (mutation === "mixed-root") p.proof.roots.push(p.pattern.certificate.positions[0].result);
...
test.each(["member", "member-proof", "forward", "alias"])(
  ...
  (mutation) => {
    ...
    if (mutation === "member-proof") p.pattern.certificate.positions[3].exclusions[0].weak.pop();
    if (mutation === "forward") p.pattern.positions[2].leftConflict = p.pattern.positions[3].right;
    if (mutation === "alias") {
...
test("actual OR forcing and inserted discovery use retained source facts", () => {
  ...
  expect([...found].sort(), terminal).toEqual(["inserted-or-whip", "or-forcing"]);
  expect(context.workspace.usage).toEqual({ entries: 0, bytes: 0 });
}, 150000);

test("actual OR discovery includes an independently checked generalized case", () => {
  const { view } = orFixture(c28.find((f) => f.id === "C28-generalized-C25-z")!),
    context = discoveryContext();
  context.limits.timeMs = 120000;
  const cursor = orTechniques[0].discover(view, context);
  let found = false,
  ...
  try {
    for (const e of cursor) {
      ...
      if (e.kind !== "proposal") continue;
      const p = e.proposal.pattern as any;
      if (p.kind !== "or-forcing" || !p.branches.some((b: any) => b.generalized)) continue;
      assertSound(view, e.proposal);
      found = true;
      break;
    }
...
test.each(scalarGrammarAliases.filter(([grammar]) => grammar !== "bivalue" && grammar !== "braid"))(
  ...
  (grammar, alias) => {
    ...
    expect(plan.positions.every((position) => position.right !== null)).toBe(true);
    for (const [assembly, proposal] of [
      ["production", compileGeneralized(view, plan)],
      ["independent", independentGeneralized(view, plan)],
    ] as const) {
      expect((proposal.pattern as any).alias, assembly).toBe(alias);
      expect([...verifyCertificate(proposal, context)].at(-1), assembly).toMatchObject({
        kind: "verified",
      });
      assertCertificateSound(view, proposal);
      expect([...checkProposal(proposal, context)].at(-1), assembly).toMatchObject({
        kind: "rejected",
        code: "generalized-position-bound",
      });
    }
