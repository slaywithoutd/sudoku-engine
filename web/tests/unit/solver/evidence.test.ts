...
function exhaustion(
  ...
  witness: readonly number[] | null = [1, 2],
): CountEvidence {
  const proof = {
    kind: "root-exhausted" as const,
    key: context.run,
    method: EXACT_METHOD,
    stats: { nodes: 1, backtracks: 0, maxDepth: 0 },
    frontierEmpty: true as const,
  };
  return witness
    ? { kind: "unique", witness, evidenceId: "evidence", proof, rootExhausted: true }
...
function checked(
  ...
  imports: number[],
  roots: number[],
  effects: DeductionProposal["effects"],
): CheckedStep {
  const single = effects.some((e) => e.kind === "place") || effects.length === 0;
  const proposal: DeductionProposal = {
    technique: single ? "c01@1" : "rule-propagation@1",
    state: view.state.key,
    pattern: single
      ? { kind: "single", alias: "Naked Single", house: null, cell: 1, symbol: 2 }
      : { kind: "propagation" },
    effects,
...
function solved() {
  ...
  const nodes: ProofNode[] = [
    ...
    },
    {
      id: id++,
      rule: "resolution@1",
      premises: [id - 2, initial.state.domainFacts[0]],
      parameters: {},
      scope: [],
    ...
    },
    {
      ...
      parameters: {},
      scope: [],
      conclusion: { kind: "domain", cell: 1, mask: 2 },
    },
  ];
  const removal = checked(
    initial,
    nodes,
    [house, initial.state.domainFacts[0], 1],
    [nodes[1].id, nodes[2].id],
    [{ kind: "remove", cell: 1, symbol: 1 }],
  );
    ...
    [
      {
        id: id++,
        rule: "cover-clause@1",
        premises: [reduced.state.domainFacts[1]],
        parameters: {},
        scope: [],
        conclusion: { kind: "literal", value: { cell: 1, symbol: 2, positive: true } },
      },
    ],
    [reduced.state.domainFacts[1]],
    [id - 1],
    [],
  );
  ...
  const placement = checked(
    cached,
    [],
    [id - 1, cached.state.domainFacts[1]],
    [id - 1, cached.state.domainFacts[1]],
    [{ kind: "place", cell: 1, symbol: 2 }],
  );
  return {
    ...context,
    human: "solved" as const,
    accepted: [removal, cache, placement],
    acceptedView: commitChecked(cached, placement).view,
  };
}
...
describe("accepted-path authority and quality", () => {
  test("primitive certificates never supply accepted logical-path quality", () => {
    ...
    expect(
      deriveQuality(c.snapshot, "solved", [result.certificate as never], count, false, c),
    ).toBe("inconsistent");
  });
  test("authenticates exact committed prefix including unchanged-revision cache and cold indexes", () => {
    ...
    expect(isAcceptedPath(c.initialView, c.acceptedView, [c.accepted[0], c.accepted[2]])).toBe(
      false,
    );
    expect(isAcceptedPath(initialize(c.assembly, "primary"), c.acceptedView, c.accepted)).toBe(
      false,
    );
    expect(isAcceptedPath(c.initialView, c.initialView, c.accepted)).toBe(false);
  });
  test.each(["getter", "proxy"])(
    "rejects %s publications for accepted lineage and quality while trusting cold rebuilds",
    (kind) => {
      const c = solved(),
        count = mergeEvidence(unknown, exhaustion(c), c).count;
      const fake =
        kind === "getter"
          ? {
              ...c.acceptedView,
              get state() {
                return c.acceptedView.state;
              },
            }
          : new Proxy({ ...c.acceptedView }, {});
      expect(isAcceptedPath(c.initialView, fake, c.accepted)).toBe(false);
      expect(
        deriveQuality(c.snapshot, "solved", c.accepted, count, false, { ...c, acceptedView: fake }),
      ).toBe("inconsistent");
      expect(mergeEvidence(unknown, count, { ...c, acceptedView: fake }).diagnostics).toContain(
        "invalid-accepted-path",
      );
      const cold = {
        ...c,
        initialView: rebuildIndexes(c.initialView),
        acceptedView: rebuildIndexes(c.acceptedView),
      };
      expect(deriveQuality(c.snapshot, "solved", c.accepted, count, false, cold)).toBe(
        "perfect-verified",
      );
    },
  );
  test("requires accepted unconditional completion plus authenticated independent uniqueness", () => {
    ...
    expect(deriveQuality(c.snapshot, "solved", c.accepted, merged.count, false, c)).toBe(
      "perfect-verified",
    );
    expect(deriveQuality(c.snapshot, "solved", c.accepted, merged.count, true, c)).toBe(
      "not-established",
    );
  ...
  test("never grants Perfect to a conditional operation containing only unconditional steps", () => {
    ...
    const conditional = {
      ...c,
      run: { ...c.run, operation: "conditional" as const, parentEvidenceId: "evidence" },
    };
    expect(deriveQuality(c.snapshot, "solved", c.accepted, count, false, conditional)).toBe(
      "not-established",
    );
    expect(
      deriveQuality(
        ...
        count,
        false,
        undefined as unknown as QualityContext,
      ),
  ...
  test("rejects replacement complete-rule semantics even when the problem key matches", () => {
    ...
    const replacement = {
      ...c,
      assembly: { ...c.assembly, modules: new Map([["a", new AllDifferentRule()]]) },
    };
    expect(deriveQuality(c.snapshot, "solved", c.accepted, count, false, replacement)).toBe(
      "not-established",
    );
  });
  test("rejects missing original complete-rule semantics", () => {
    const c = solved(),
      count = mergeEvidence(unknown, exhaustion(c), c).count;
    expect(
      deriveQuality(c.snapshot, "solved", c.accepted, count, false, {
        ...c,
        assembly: { ...c.assembly, modules: new Map() },
      }),
    ).toBe("not-established");
  });
});
...
describe("monotone independently checked count evidence", () => {
  ...
  test.each([
    ...
    "inactive",
    "phase",
  ])("rejects %s exhaustion while preserving its valid witness", (mutation) => {
    const c = fixture(),
      incoming = structuredClone(exhaustion(c));
    ...
    const context =
      mutation === "inactive"
        ? { ...c, activeExactRun: null }
        : mutation === "phase"
          ? { ...c, phase: "human" as const }
          : c;
  ...
  test("duplicate-givens zero is checked directly against the original declared rule", () => {
    const c = fixture([1, 1]);
    const incoming: CountEvidence = {
      kind: "zero",
      evidenceId: "duplicate",
      proof: { kind: "duplicate-givens", constraintId: "a", symbol: 1, cells: [0, 1] },
    };
    expect(
      mergeEvidence(unknown, incoming, { ...c, phase: "human", activeExactRun: null }).count.kind,
    ).toBe("zero");
  ...
  test("a second valid witness discards incompatible uniqueness and invalidates a conflicting path", () => {
    ...
    const forged = {
      ...c.acceptedView,
      state: { ...c.acceptedView.state, values: [1, 2], domains: [1, 2] },
    };
    const conflicting = mergeEvidence(first, result.count, {
      ...c,
      acceptedView: forged,
      human: "solved",
    });
  ...
  test("rejects forged completion and preserves witnesses from malformed exhaustion", () => {
    ...
    const malformed = {
      ...
      rootExhausted: true,
      proof: null,
    } as unknown as CountEvidence;
    expect(mergeEvidence(unknown, malformed, c).count).toMatchObject({
      kind: "unknown",
      lowerBound: 1,
    });
    expect(
  ...
  test("rejects unknown proof fields and invalid statistics without losing existence", () => {
    ...
    for (const change of [
      ...
      { frontierEmpty: false },
      { stats: { nodes: -1, backtracks: 0, maxDepth: 0 } },
      { stats: { nodes: 1, backtracks: 2, maxDepth: 0 } },
      { stats: { nodes: 1, backtracks: 0, maxDepth: 1 } },
    ]) {
      const claim = exhaustion(c);
      const mutated = {
        ...claim,
        proof: { ...(claim as Extract<CountEvidence, { kind: "unique" }>).proof, ...change },
      } as CountEvidence;
      expect(mergeEvidence(unknown, mutated, c).count).toMatchObject({
        kind: "unknown",
        lowerBound: 1,
      });
    }
  ...
  test("an authentic complete logical board supplies existence despite a conflicting zero claim", () => {
    ...
    expect(result.count).toMatchObject({ kind: "unknown", lowerBound: 1, witnesses: [[1, 2]] });
    expect(result.diagnostics).toContain("incompatible-exhaustion");
    expect(mergeEvidence(unknown, unknown, c).count).toMatchObject({
      kind: "unknown",
      lowerBound: 1,
    });
  });
  test("independently valid existence invalidates a claimed human contradiction", () => {
    ...
    const result = mergeEvidence(
      unknown,
      { kind: "unknown", witnesses: [[1, 2]], lowerBound: 1 },
      { ...c, human: "contradiction" },
    );
