...
for (const raw of [...bounds.fixtures, ...caseBounds.fixtures])
  test(`${raw.id} exact distinct causal link bound`, () => {
    const seed = raw as UniqueSeed;
    independentUniqueTrade(seed);
    const h = uniqueHarness(seed),
      view = h.operation.view,
      effect = seed.expectedEffects[0] as any;
    try {
      const plan = independentUniquePlan(seed, effect);
      let distinct = 25;
      if ("independentOppositeBranchRecipes" in raw) {
        const recipe = raw.independentOppositeBranchRecipes[0];
        distinct = recipe.distinctLinks;
        const literal = (a: (number | boolean)[]) => ({
          cell: a[0] as number,
          symbol: a[1] as number,
          positive: a[2] as boolean,
        });
        (plan as any).consequence = {
          kind: "denial",
          paths: recipe.paths.map((path) =>
            path.map((link) => ({ ...link, from: literal(link.from), to: literal(link.to) })),
          ),
        };
      }
      const context = {
      ...
      for (const proposal of [
        independentNamedUnique(view, seed, h.parent.evidenceId, effect, plan),
        ...
        expect(
          Math.max(
            ...certificate.branches.map(
              (b: any) => new Set(b.paths.flatMap((p: any) => p.links)).size,
            ),
          ),
        ).toBe(distinct);
        expect(
          Math.max(
            ...certificate.branches.flatMap((b: any) => b.paths.map((p: any) => p.links.length)),
          ),
        ).toBeLessThan(24);
        ...
        if (distinct === 24) expect(named?.kind, JSON.stringify(named)).toBe("checked");
          ...
          expect(named).toEqual({
            kind: "rejected",
            code:
              plan.consequence.kind === "denial" ? "unique-denial-lineage" : "unique-case-bound",
          });
      }
      const replay = [
        ...replayConditional(h.operation, [
          compileUnique(view, plan, effect, h.operation.authority),
        ]),
      ].at(-1);
      ...
      const opposite = oracle({
        ...
        limit: 2,
        maxNodes: 1_000_000,
        force: [effect.cell, effect.symbol],
      });
      expect(opposite.exhausted && !opposite.interrupted && opposite.witnesses.length === 0).toBe(
        true,
      );
    } finally {
      h.operation.dispose();
