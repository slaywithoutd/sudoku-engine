...
test.each(fixtures)(
  "$id has complete core rejections and an authentic guardian theorem",
  (f) => {
    const view = fixtureView(f as unknown as TechniqueFixture),
      cursor = compileTridagon(view, f.expectedPattern as TridagonPlan);
    let proposal;
    while (true) {
      const n = cursor.next();
      if (n.done) {
...
test("C32 discovers an authentic multi-guardian OR publication", () => {
  ...
  expect(
    discoverSpecialized(f as unknown as TechniqueFixture, (p) => p.guardians.length > 1).proposal
      .effects,
  ).toEqual([]);
}, 200000);
