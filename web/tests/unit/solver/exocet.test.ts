...
test.each(fixtures)("$id checks the independent Junior counts and actual component join",f=> {
 const view=fixtureView(f as unknown as TechniqueFixture),cursor=compileExocet(view,f.expectedPattern as unknown as ExocetPlan);let proposal;
 ...
 for(const e of checkProposal(proposal!,{view,retained:retainedProof(view),limits,policy:"unconditional",uniqueEvidenceId:null}))if(e.kind!=="work")terminal=e;
 expect(terminal?.kind,JSON.stringify(terminal)).toBe("checked");
},150000);
import { discoverSpecialized } from "../../solver/specialized-acceptance";
test.each(["C31-junior-3","C31-double-shared-base"])("%s has actual semantic-family discovery",id=> {
 const f=fixtures.find(f=>f.id===id)!;
 expect(discoverSpecialized(f as unknown as TechniqueFixture,p=>id.includes("double")?p.alias==="Double Exocet":p.alias==="Junior Exocet").proposal.effects.length).toBeGreaterThan(0);
},200000);
