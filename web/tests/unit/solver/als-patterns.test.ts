...
import {
  alsFixture,
  alsFixtures,
  productiveAlsFixtures,
  independentAlsCertificate,
} from "./als-acceptance";
...
import {
  ...
  type AlsPattern,
  type BlossomPattern,
} from "../../../src/solver/techniques/als-certificate";
...
test.each(["C18-overlap", "C19-blossom-4"])(
  "%s rejects standalone and mixed primitive-valid substitute roots",
  (id) => {
    const f = alsFixture(id),
      view = fixtureView(f),
      genuine = independentAlsCertificate(f),
      e = f.expectedEffects[0];
    const seed = f.expectedPattern as any,
      cells = [
        ...seed.sets.flatMap((s: any) => s.cells),
        e.cell,
        ...(seed.stem === undefined ? [] : [seed.stem]),
      ];
    const house = view.assembly.allDifferent.find((h) =>
      cells.every((c: number) => h.cells.includes(c)),
    )!;
...
test("the six-set certificate is exact and seven/repeated sets cannot reuse its proof", () => {
  const f = alsFixture("C19-chain-6"),
    original = independentAlsCertificate(f),
    p = original.pattern as unknown as AlsPattern;
  expect(p.sets).toHaveLength(6);
  expect(p.rccs).toHaveLength(5);
  expect(p.routes[0].projections).toHaveLength(6);
  for (const count of [6, 7]) {
    const proposal = structuredClone(original),
      p = proposal.pattern as unknown as AlsPattern;
    if (count === 6) p.sets[5] = structuredClone(p.sets[0]);
    else p.sets.push(structuredClone(p.sets[0]));
    expect(checked(f, proposal)?.kind).toBe("rejected");
  }
});

test("a valid bivalue clause cannot replace a singleton ALS's complete table", () => {
  const f = alsFixture("C18-xy"),
    proposal = independentAlsCertificate(f),
    p = proposal.pattern as unknown as AlsPattern;
  const source = p.routes[0].projections[0],
    cell = p.sets[source.set].cells[0],
    view = fixtureView(f);
  const nodes = proposal.proof.nodes.map((n) =>
    n.id === source.root
      ? { ...n, rule: "cover-clause@1", premises: [view.state.domainFacts[cell]], parameters: {} }
      : n,
  );
  const needed = new Set<number>(),
    queue = [...proposal.proof.roots],
    byId = new Map(nodes.map((n) => [n.id, n]));
  while (queue.length) {
    const id = queue.pop()!;
    if (needed.has(id)) continue;
    needed.add(id);
    queue.push(...(byId.get(id)?.premises ?? []));
  }
  const substitute = {
    ...proposal,
    proof: {
      ...proposal.proof,
      nodes: nodes.filter((n) => needed.has(n.id)),
      imports: [...needed].filter((id) => view.facts.has(id)).sort((a, b) => a - b),
    },
  };
  expect(
    [
      ...verifyCertificate(substitute, {
        view,
        retained: retainedProof(view),
        limits: discoveryContext().limits,
        policy: "discharged",
...
test.each(alsFixtures.filter((f) => f.expectation === "out-of-profile"))(
  "$id is primitive-sound but rejected at the named bound",
  (f) => {
    const view = fixtureView(f),
      proposal = independentAlsCertificate(f);
    expect(
...
test("durable input hashes reproduce every satisfiable prestate and exhaustive opposite", () => {
  ...
  for (const f of alsFixtures) {
    ...
    expect(createHash("sha256").update(JSON.stringify(input)).digest("hex"), f.id).toBe(
      (f as any).oracleRecord.inputHash,
    );
    expect(oracle(input), f.id).toMatchObject({
      interrupted: false,
      witnesses: [expect.any(Array)],
    });
    fixtures++;
    for (const effect of f.expectedEffects) {
      expect(oracle({ ...input, force: [effect.cell, effect.symbol] }), f.id).toMatchObject({
        interrupted: false,
        exhausted: true,
        witnesses: [],
      });
...
test("all advertised ALS and stem bounds have explicit authored geometry", () => {
  ...
  const patterns = productiveAlsFixtures.map(
    (f) =>
      f.expectedPattern as unknown as {
        sets: AlsPattern["sets"];
        stemSymbols?: number[];
        petals?: number[];
      },
  );
  expect([...new Set(patterns.flatMap((p) => p.sets.map((s) => s.cells.length)))].sort()).toEqual([
    1, 2, 3, 4, 5,
  ]);
  expect(
    [...new Set(patterns.flatMap((p) => (p.stemSymbols ? [p.stemSymbols.length] : [])))].sort(),
  ).toEqual([2, 3, 4]);
  const shared = patterns.find((p) => p.stemSymbols && p.sets.length === 1)!;
  expect(shared.petals).toEqual([0, 0]);
  expect(shared.sets[0].cells).toHaveLength(5);
});
...
test("a broad ALS-chain search checks every emitted effect and reports its work cap honestly", () => {
  ...
  for (const event of getTechniques("classic-expanded@1")
    ...
    expect(() => assertSound(view, event.proposal), JSON.stringify(p)).not.toThrow();
    proposals++;
...
test.each(["c18@1", "c19@1"])(
  "%s releases all resources on return, cancellation, proof/work/workspace limits and exhaustion",
  (id) => {
    const f = alsFixture("C19-blossom-4"),
      view = fixtureView(f),
      descriptor = getTechniques("classic-expanded@1").find((d) => d.id === id)!;
...
test("source-prefix growth and candidate revision invalidate stale ALS search and certificates", () => {
  ...
  const context = discoveryContext();
  let als: AlsIndex | undefined, implications: ImplicationIndex | undefined;
  ...
  const extended = retainCheckedFacts(view, accepted.step),
    ...
    graph = new PatternGraph(implications!, context, lease);
  try {
    expect(als!.acceptsView(extended, () => {})).toBe(true);
    expect(als!.completeFor(extended)).toBe(false);
    expect(() => [...new AlsSearch(extended, graph, als!).prepare()]).toThrow(
      "incomplete-als-source-prefix",
    );
  } finally {
    lease.dispose();
    als!.dispose();
  ...
  const f = alsFixture("C18-overlap"),
    proposal = independentAlsCertificate(f),
    checkedStep = checked(f, proposal);
...
test.each(productiveAlsFixtures)(
  "$id has independent named proof and exhaustive counterfactual evidence",
  (f) => {
...
test.each(productiveAlsFixtures)(
  "$id compiles exact authored geometry through production",
  (f) => {
    const view = fixtureView(f),
      context = discoveryContext();
    ...
    try {
      const independent = independentAlsCertificate(f),
        compiler = new AlsCertificate(view, graph).compile(
          independent.pattern as unknown as AlsPattern | BlossomPattern,
          [...independent.effects],
        );
...
test.each(productiveAlsFixtures)(
  "$id replays original clues and declared rules",
  (f) => {
    const view = fixtureView(f),
...
test.each([
  ...
  "%s has sound measured family discovery",
  (id) => {
    const f = alsFixture(id),
      view = fixtureView(f),
      context = discoveryContext();
    ...
    for (const event of descriptor.discover(view, context)) {
      ...
      expect(
        ...
        JSON.stringify(event.proposal.pattern),
      ).not.toThrow();
      const p = event.proposal.pattern as unknown as AlsPattern | BlossomPattern;
      ...
      if (
        p.kind === "als" &&
        ((id.includes("overlap") && !p.overlaps.some((o) => o.cells.length)) ||
          (id.includes("double") && p.rccs.length !== 2) ||
          (id.includes("locked-effects") && !p.routes.some((r) => r.form === "rcc")))
      )
        continue;
      found = true;
      break;
...
test.each([
  ...
  "repeat-set",
  "wrong-double-class",
])("rejects %s in independently compiled proof", (mutation) => {
  const f = alsFixture(
    ["remove-petal", "wrong-stem", "wrong-branch-root", "escape"].includes(mutation)
      ? "C19-blossom-4"
      : mutation === "wrong-double-class"
        ? "C18-xz-double-rcc"
        : "C18-overlap",
  );
  const proposal = independentAlsCertificate(f),
    p = proposal.pattern as unknown as AlsPattern | BlossomPattern;
  if (p.kind === "blossom") {
