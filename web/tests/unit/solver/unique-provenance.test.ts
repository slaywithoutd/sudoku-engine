...
const limits = { timeMs: 20000, workUnits: 10_000_000, exactNodes: 1_000_000, stepNodes: 4096,
  runNodes: 65536, proofBytes: 16_000_000, stepBytes: 2_000_000, batchBytes: 65536, inFlightBatches: 2, workspaceBytes: 128_000_000 };
function primary() {
  const problem = normalizeClassic({ kind: "classic", version: 1, width: 9, height: 9, givens: [...u01.fixtures[0].givens].map(Number) });
  ...
  const run: RunKey = { requestId: "primary", snapshotId: snapshot.snapshotId, inputRevision: 0, problemKey: problem.key,
  ...
  const context: evidence.EvidenceContext = { snapshot, run, assembly: result.value, initialView, acceptedView: initialView,
  ...
  for (const event of exactSteps(snapshot.problem, context.assembly)) {
    ...
    if (event.kind === "exhausted" && witnesses.length === 1) raw = { kind: "unique", witness: witnesses[0], evidenceId: "unique-parent", rootExhausted: true,
  ...
  const merged = evidence.mergeEvidence({ kind: "unknown", lowerBound: 0, witnesses: [] }, raw, context);
...
test("parent capability requires real exact/merge authority and a non-quarantined lifecycle", () => {
  ...
  expect(evidence.acceptedUniqueParent(count, { ...context, human: "invalidated" }, false)).toBeUndefined();
...
test("operation capabilities reject cloned brands, same-label owners and revoked parents", async () => {
  ...
  const parent = evidence.acceptedUniqueParent(count, context, false)!;
  const run: RunKey = { ...context.run, requestId: "conditional", operation: "conditional", profile: "classic-conditional@1", parentEvidenceId: parent.evidenceId };
  ...
  expect(uniqueAuthorityMatches(op.authority, initialize(context.assembly, op.view.state.key.branch))).toBe(false);
  expect(await op.digestPrefix()).toMatch(/^[a-f0-9]{64}$/);
  evidence.revokeUniqueParent(parent);
  expect(uniqueAuthorityMatches(op.authority, op.view)).toBe(false);
  op.dispose(); expect(workspace.usage).toEqual({ entries: 0, bytes: 0 });
});

function emptyOperation() {
  const {context,count}=primary(), parent=evidence.acceptedUniqueParent(count,context,false)!;
  const run:RunKey={...context.run,requestId:"conditional-port",operation:"conditional",profile:"classic-conditional@1",parentEvidenceId:parent.evidenceId};
  const workspace=new IndexWorkspace({entryLimit:1_000_000,byteLimit:limits.workspaceBytes});
  const operation=ConditionalOperation.begin(parent,run,limits,workspace); [...operation.rebuildPrefix()];
...
test("dedicated grant waits for exact prefix, refuses duplicates and releases both owners",async()=>{
  ...
  const workspace=new IndexWorkspace({entryLimit:1_000_000,byteLimit:limits.workspaceBytes});
  const installed=ConditionalOperation.installTrustedBootstrap(channel.port2,expected,h.context.assembly,limits,workspace);
  let ready=false;void installed.ready.then(()=>{ready=true;});
  await new Promise(resolve=>setTimeout(resolve,10));expect(ready).toBe(false);
  ...
  expect(()=>ConditionalOperation.installTrustedBootstrap(channel.port2,expected,h.context.assembly,limits,workspace)).toThrow("conditional-port-already-installed");
  channel.port1.postMessage({kind:"conditional-grant@1",envelope:expected});
  await new Promise(resolve=>setTimeout(resolve,10));expect(worker.active).toBe(false);
...
test("foreign or changed grant cannot mint worker authority",async()=>{
  ...
  const workspace=new IndexWorkspace({entryLimit:1_000_000,byteLimit:limits.workspaceBytes});
  const installed=ConditionalOperation.installTrustedBootstrap(foreign.port2,expected,h.context.assembly,limits,workspace);
  const rejected=expect(installed.ready).rejects.toThrow("conditional-grant-mismatch");
  await installed.providePrefix([]);
  foreign.port1.postMessage({kind:"conditional-grant@1",envelope:{...expected,generation:1}});
  await rejected;expect(workspace.usage).toEqual({entries:0,bytes:0});
  source.port2.close();foreign.port1.close();h.operation.dispose();
});

test("revocation across awaited hashing disposes every temporary reservation",async()=>{
  const h=emptyOperation(), original=crypto.subtle.digest.bind(crypto.subtle);
  const spy=vi.spyOn(crypto.subtle,"digest").mockImplementation(async (...args)=>{
    const value=await original(...args);evidence.revokeUniqueParent(h.parent);return value;
  });
  const before=h.workspace.usage;
  try {await expect(h.operation.digestPrefix()).rejects.toThrow("revoked-unique-authority");}
  finally{spy.mockRestore();}
  expect(h.workspace.usage).toEqual(before);h.operation.dispose();
...
test("fresh Node realm independently captures prefix and checks a uniqueness bundle",async()=>{
  ...
  const seed=u01.fixtures[0] as any,h=uniqueHarness(seed),channel=new MessageChannel();
  const expected=await h.operation.grantBootstrap(channel.port1,"nonce-worker-realm-001",3);
  const proposal=independentNamedUnique(h.operation.view,seed,h.parent.evidenceId,seed.expectedEffects[0]);
  const worker=new Worker(new URL("../../solver/unique-worker.mjs",import.meta.url),{workerData:{
    expected,prefix:h.operation.prefix,bundles:[proposal],limits:uniqueLimits,port:channel.port2},transferList:[channel.port2 as any]});
  try {
    const result:any=await new Promise((resolve,reject)=>{worker.once("message",resolve);worker.once("error",reject);});
    expect(result.error).toBeUndefined();expect(result.ready).toBe(true);
    expect(result.events).toEqual([{kind:"checked",conditional:true}]);
    expect(result.usage).toEqual({entries:0,bytes:0});
  }finally{await worker.terminate();h.operation.dispose();}
  expect(h.workspace.usage).toEqual({entries:0,bytes:0});
},30000);
...
test("a valid grant plus partial or altered nonempty prefix never reaches ready",async()=>{
 ...
 try {
  ...
  for(const alter of [false,true]) {
   const channel=new MessageChannel(),expected=await h.operation.grantBootstrap(channel.port1,`nonce-partial-prefix-${alter}`,4);
   const workspace=new IndexWorkspace({entryLimit:1_000_000,byteLimit:uniqueLimits.workspaceBytes});
   const installation=ConditionalOperation.installTrustedBootstrap(channel.port2,expected,h.operation.assembly,uniqueLimits,workspace);
   const rejected=expect(installation.ready).rejects.toThrow("conditional-prefix-digest-mismatch");
   ...
   await expect(installation.providePrefix(prefix)).rejects.toThrow("conditional-prefix-digest-mismatch");
   await rejected;expect(workspace.usage).toEqual({entries:0,bytes:0});
  }
...
test("conditional disposal preserves the accepted primary parent for a new branch",()=>{
 const h=emptyOperation();h.operation.dispose();
 const next=ConditionalOperation.begin(h.parent,{...h.operation.run,requestId:"conditional-again"},limits,h.workspace);
 expect(evidence.uniqueParentDetails(h.parent)).toBeDefined();
 [...next.rebuildPrefix()];expect(uniqueAuthorityMatches(next.authority,next.view)).toBe(true);
 next.dispose();expect(h.workspace.usage).toEqual({entries:0,bytes:0});
});

test("task-yielded prefix replay stays private and revokes before a second slice",async()=>{
 const {uniqueHarness,uniqueLimits}=await import("../../solver/unique-harness");
 const h=uniqueHarness(u01.fixtures[0] as any),channel=new MessageChannel();
 const expected=await h.operation.grantBootstrap(channel.port1,"nonce-replay-task-yield",5);
 const workspace=new IndexWorkspace({entryLimit:1_000_000,byteLimit:uniqueLimits.workspaceBytes});
 let slices=0,ready=false,release!:()=>void,paused!:()=>void;
 const started=new Promise<void>(resolve=>{paused=resolve;});
 const installation=ConditionalOperation.installTrustedBootstrap(channel.port2,expected,h.operation.assembly,uniqueLimits,workspace,{
  yieldTask:()=>{slices++;paused();return new Promise<void>(resolve=>{release=resolve;});}
 });
 const rejected=expect(installation.ready).rejects.toThrow("revoked-unique-authority");
 void installation.ready.then(()=>{ready=true;},()=>{});
 await installation.providePrefix(h.operation.prefix);await started;
 expect(slices).toBe(1);expect(ready).toBe(false);
 h.operation.dispose();
...
test("public accounting includes initialization and charges scheduler work without resetting allowance",async()=>{
 const h=emptyOperation(),initial=h.operation.usage.workUnits;
 expect(initial).toBeGreaterThan(0);
 await h.operation.digestPrefix();
 expect(h.operation.usage.workUnits).toBeGreaterThan(initial);
 const before=h.operation.usage.workUnits;h.operation.charge(17);
 ...
 h.operation.dispose();expect(()=>h.operation.charge(1)).toThrow("revoked-unique-authority");
});
...
test("conditional replay refuses a shaped operation and its fabricated event stream",async()=>{
 ...
 expect([...replayConditional({active:true,checkAndCommit:function*(){yield {kind:"checked",step:{}};}} as any,[])]).
