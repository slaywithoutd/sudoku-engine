...
function classic(puzzle: string) {
  const problem = normalizeClassic({ kind: "classic", version: 1, width: 9, height: 9, givens: [...puzzle].map(Number) });
...
describe("original-problem exact enumeration", () => {
  test.each(Object.entries(counts.cases))("independently matches seed count %s", (_name, fixture) => {
  ...
  test("closing after one witness does not emit exhaustion", () => {
    ...
    const events = [];
    for (const event of iterator) { events.push(event); if (event.kind === "witness") break; }
  ...
  test("ignores fabricated human capability metadata and retains both original completions", () => {
    ...
    const misleading = { ...assembly, peers: problem.cells.map(() => problem.cells), covers: [], allDifferent: [], relations: [] };
    expect([...exactSteps(problem, misleading)].filter(event => event.kind === "witness")).toHaveLength(2);
  });
  test("rejects mismatched semantic identity and malformed witnesses", () => {
    const first = classic(counts.cases.oneHole.puzzle), other = classic(counts.cases.empty.puzzle);
    ...
    expect(isWitness(first.problem, other.assembly, [...counts.cases.complete.puzzle].map(Number))).toBe(false);
  });
  test("deterministically orders decisions and yields bounded accounting", () => {
    ...
    expect(events[0]).toMatchObject({ kind: "work", units: exactInitializationReservation(problem, assembly).workUnits, stats: { nodes: 0 } });
  ...
  test("checks additional complete-rule semantics without discovery", () => {
    const problem = canonicalProblem({ schema: 1, cells: [0, 1], symbols: [1, 2, 3], givens: [0, 0],
      constraints: [{ id: "only", type: "complete-only@1", cells: [0, 1], parameters: {} }] });
    const module = { type: "complete-only@1", normalize: (rule: typeof problem.constraints[number]) => rule,
      validate: () => [], checkComplete: (_rule: unknown, assignment: { values: readonly number[] }) => assignment.values[0] === 3 && assignment.values[1] === 2,
      capabilities: () => ({ allDifferent: [], covers: [], relations: [], primitiveIds: [] }),
      *propagate(): Generator<never> { throw new Error("discovery called"); }, checkPrimitive: () => { throw new Error("proof called"); } };
    const result = assemble(problem, [module]);
    if (!result.ok) throw new Error(JSON.stringify(result));
    const events = [...exactSteps(problem, result.value)];
    expect(events.filter(event => event.kind === "witness").map(event => event.values)).toEqual([[3, 2]]);
  ...
  test("bounds dimensions/scopes and refuses accessors before canonical traversal", () => {
    ...
    Object.defineProperty(oversized, 0, { get: () => { touched = true; throw new Error("read"); } });
    expect(() => exactSteps({ ...problem, cells: oversized }, assembly)).toThrow("exact-cell-limit");
    ...
    Object.defineProperty(accessor, "givens", { get: () => { touched = true; throw new Error("read"); }, enumerable: true });
  ...
  test("isolates original values before generator consumption and accounts finite setup", () => {
    ...
    expect(events.filter(event => event.kind === "witness")).toHaveLength(1);
    expect(events[0]).toMatchObject({ kind: "work", units: reservation.workUnits, stats: { nodes: 0 } });
  });
});
