...
function view() {
  const assembly = assemble(
    canonicalProblem({ schema: 1, cells: [0], symbols: [1], givens: [1], constraints: [] }),
    [new AllDifferentRule()],
  );
  if (!assembly.ok) throw Error("fixture");
  return initialize(assembly.value, "primary");
}
function jobs(_view: ReadView): TechniqueJobs {
  return {
    rules: [],
    techniques: [
      {
...
describe("human logical phase", () => {
  test("waits for explicit acceptance before allowing the next selection", () => {
    const initial = view(),
      workspace = new IndexWorkspace({ entryLimit: 1000, byteLimit: limits.workspaceBytes });
    const selection = humanSteps(initial, jobs(initial), {
      ...schedulingOptions({ limits }),
      workspace,
      accept: (_step: unknown) => initial,
    });
  ...
  test("reports logical budget interruption as incomplete", () => {
    const initial = view(),
      workspace = new IndexWorkspace({ entryLimit: 1000, byteLimit: limits.workspaceBytes });
    const result = [
      ...humanSteps(initial, jobs(initial), {
        ...schedulingOptions({ limits, phaseWorkUnits: 0 }),
        workspace,
        accept: (_step: unknown) => initial,
      }),
    ].at(-1);
