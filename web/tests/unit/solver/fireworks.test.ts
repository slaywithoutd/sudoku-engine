...
test.each(fixtures)(
  "$id compiles its complete original-clue certificate and every removal is checked",
  (f) => {
...
test.each(fixtures)(
  "$id is reached by actual Fireworks discovery",
  (f) => {
    const result = discoverSpecialized(
      f as unknown as TechniqueFixture,
      (p) => p.components.length === f.expectedPattern.components.length,
    );
