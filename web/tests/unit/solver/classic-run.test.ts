...
async function solve(text:string){
  ...
  await solveClassic({givens:givens(text)},{clock:{now:()=>Date.now()},emit:event=>events.push(event)});
  const result=events.at(-1);if(result?.kind!=="result")throw Error(JSON.stringify(result));
...
describe("classic solve composition",()=>{
  test("explains and independently verifies a unique puzzle",async()=>{
    ...
    expect(events.find(event=>event.kind==="precount")).toMatchObject({solution:givens(solution)});
  });
  test("reports multiple, zero and conflicting inputs without the logical phase",async()=>{
    expect((await solve("0".repeat(80)+"1")).result).toMatchObject({count:"multiple",human:"not-started",steps:0});
    expect((await solve("123456780000000009"+"0".repeat(63))).result).toMatchObject({count:"zero",human:"not-started"});
    expect((await solve("55"+"0".repeat(79))).result).toMatchObject({count:"zero",human:"not-started"});
  });
  test("rejects malformed givens",async()=>{
    await expect(solveClassic({givens:[1,2,3]},{clock:{now:()=>0},emit:()=>{}})).rejects.toThrow();
  });
});
