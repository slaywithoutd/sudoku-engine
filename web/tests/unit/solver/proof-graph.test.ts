...
function fixture(givens = [1, 0], symbols = [1, 2], scopes = [[0, 1]]) {
  const result = assemble(
    canonicalProblem({
      schema: 1,
      cells: givens.map((_, cell) => cell),
      symbols,
      givens,
      constraints: scopes.map((cells, index) => ({
        id: `row:${index}`,
...
const lit = (cell: number, symbol: number, positive = true): Proposition => ({
  kind: "literal",
...
test("a caller-supplied primitive registry cannot mint checked authority", () => {
  const b = fixture(),
    root = b.add("invented@1", [], lit(1, 2));
  class ForgedRegistry extends PrimitiveRegistry {
    override check(input: ProofNode) {
      return { conclusion: input.conclusion, openAssumptions: [], conditional: false, rules: [] };
    }
  }
  const Constructor = ProofChecker as unknown as new (registry: PrimitiveRegistry) => ProofChecker;
  expect(
    [...new Constructor(new ForgedRegistry()).checkProposal(b.proposal([root]), b.context)].at(-1)
      ?.kind,
  ).toBe("rejected");
});
...
test("discharges only the contradicted assumption and derives row provenance", () => {
  ...
  const weak = b.add("weak-link@1", [4], {
    kind: "clause",
    alternatives: [
  ...
  expect(b.check([root])).toMatchObject({
    kind: "verified",
    certificate: { consequences: [{ openAssumptions: [], conditional: false, rules: ["row:0"] }] },
  });
  b.nodes[0] = {
    ...b.nodes[0],
    conclusion: { ...lit(1, 1), ignored: true } as unknown as Proposition,
  };
...
test("cases requires every exhaustive alternative with its own branch", () => {
  ...
  const second = b.add("conjunction@1", [2, c], { kind: "and", terms: [lit(0, 1), lit(1, 2)] }, [
    c,
  ]);
...
test("rejects a sibling assumption even when its literal matches a desired conclusion", () => {
  const b = fixture(),
    a = b.add("assume@1", [], lit(1, 1)),
    sibling = b.add("assume@1", [], lit(1, 2));
  const root = b.add(
    "conjunction@1",
    [a, sibling],
    { kind: "and", terms: [lit(1, 1), lit(1, 2)] },
    [a],
  );
  expect(b.check([root]).kind).toBe("rejected");
});
...
test("support rebuilds every cell from checked domain facts, ignoring the view cache", () => {
  ...
  b.nodes[0] = {
    ...b.nodes[0],
    premises: [5, 2],
    conclusion: { kind: "cover", symbol: 1, cells: [0] },
  };
  expect(b.check([root]).kind).toBe("rejected");
});
...
test("Hall singleton removes its digit from the rest of its all-different scope", () => {
  const b = fixture(),
    root = b.add("hall@1", [4, 2], lit(1, 1, false));
  expect(b.check([root]).kind).toBe("verified");
...
test("incidence counts account for a twice-weighted overlap instead of assuming disjoint covers", () => {
  ...
  const root = b.add("cover-count@1", [cover, 4, 2, 1], { kind: "false" }, [], {
    symbol: 1,
    covers: [{ premise: cover, coefficient: 2 }],
    capacities: [{ premise: 4, coefficient: 1 }],
  });
  expect(b.check([root]).kind).toBe("rejected");
});
...
test("cover-count derives a supported elimination across overlapping scopes", () => {
  ...
  const cover = [...view.facts.values()].find(
    (f) => f.proposition.kind === "cover" && f.proposition.symbol === 1 && f.rules[0] === "row:0",
  )!;
  const capacity = [...view.facts.values()].find(
    (f) => f.proposition.kind === "all-different" && f.rules[0] === "row:1",
  )!;
  const root = b.add(
    "cover-count@1",
    [cover.root, capacity.root, ...view.state.domainFacts],
    lit(3, 1, false),
    [],
    {
      symbol: 1,
      covers: [{ premise: cover.root, coefficient: 1 }],
      capacities: [{ premise: capacity.root, coefficient: 1 }],
    },
  );
  const domain = b.add("domain-restrict@1", [3, root], { kind: "domain", cell: 3, mask: 6 });
  const proposal = {
    ...b.proposal([root, domain]),
    effects: [{ kind: "remove", cell: 3, symbol: 1 } as const],
    pattern: { kind: "propagation" },
  };
  expect(assertSound(view, proposal).consequences[0].rules).toEqual(["row:0", "row:1"]);
  b.nodes[0] = {
    ...b.nodes[0],
    conclusion: { ...lit(3, 1, false), invented: true } as unknown as Proposition,
  };
  expect(b.check([root, domain])).toMatchObject({
    kind: "rejected",
    code: "invalid-count-conclusion",
  });
});
...
test.each(["forward", "unsupported", "uniqueness", "false-strong-link"])(
  "rejects malformed graph mutation: %s",
  (mutation) => {
    const b = fixture();
    let root: number;
    if (mutation === "forward")
      root = b.add("conjunction@1", [999], { kind: "and", terms: [lit(0, 1)] });
    else if (mutation === "unsupported") root = b.add("solution-axiom@1", [], lit(1, 2));
    else if (mutation === "uniqueness")
      root = b.add("unique-transform@1", [], lit(1, 2), [], { evidenceId: "invented" });
    else
      root = b.add("cover-clause@1", [4], {
        kind: "clause",
        alternatives: [
          { cell: 0, symbol: 1, positive: true },
          { cell: 1, symbol: 1, positive: true },
        ],
      });
    expect(b.check([root]).kind).toBe("rejected");
...
test.each(["wrapped-symbol", "extra-fields"])(
  "Hall rejects a malformed literal: %s",
  (mutation) => {
    const b = fixture();
    const conclusion =
      mutation === "wrapped-symbol"
        ? lit(1, 33, false)
        : ({ ...lit(1, 1, false), invented: true } as unknown as Proposition);
