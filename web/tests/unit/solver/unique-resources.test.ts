  ...
  try {
    ...
    for (const resource of ["work", "storage"] as const) {
      ...
      const limits = {
        ...uniqueLimits,
        workUnits:
          resource === "work" ? initializationWork + prefixLength - 1 : uniqueLimits.workUnits,
      };
      expect(() =>
        ConditionalOperation.begin(h.parent, h.operation.run, limits, workspace),
...
test("prefix limits reserve before capture and unwind partial allocations", async () => {
  ...
  ).rejects.toThrow("workspace-byte-limit");
  expect(workspace.usage).toEqual({ entries: 0, bytes: 0 });
});
...
test("prefix header and node records retain the16KiB wire cap", async () => {
  ...
  for (const header of [true, false]) {
    ...
    const proposal: any = {
      ...
      state,
      effects: [],
      pattern: header ? { large: "x".repeat(16384) } : {},
      proof: {
        state,
        imports: [],
        roots: [],
          ...
          : [
              {
                ...
                premises: [],
                scope: [],
                parameters: { large: "x".repeat(16384) },
...
test("asynchronous prefix cancellation releases copied headers and hash buffers", async () => {
  ...
  const original = crypto.subtle.digest.bind(crypto.subtle),
    spy = vi.spyOn(crypto.subtle, "digest").mockImplementation(async (...args) => {
...
test("unique discovery cancellation, work, memory and deadline terminals release all leases", () => {
  const h = uniqueHarness(u01.fixtures[0] as any),
    descriptor = getTechniques("classic-conditional@1").find((d) => d.id === "u01@1")!;
  try {
    for (const kind of ["cancelled", "work-limit", "workspace-byte-limit", "time-limit"] as const) {
      const workspace = new IndexWorkspace({
        entryLimit: 1_000_000,
        byteLimit: kind === "workspace-byte-limit" ? 1 : uniqueLimits.workspaceBytes,
        cancelled: () => kind === "cancelled",
      });
      const limits = {
        ...uniqueLimits,
        workUnits: kind === "work-limit" ? 0 : uniqueLimits.workUnits,
        timeMs: kind === "time-limit" ? 0 : uniqueLimits.timeMs,
      };
      const terminal = [
        ...descriptor.discover(h.operation.view, {
          workspace,
          limits,
          uniqueAuthority: h.operation.authority,
        }),
      ].at(-1);
      expect(terminal).toEqual({ kind: "interrupted", reason: kind });
      expect(workspace.usage).toEqual({ entries: 0, bytes: 0 });
    }
  } finally {
    h.operation.dispose();
