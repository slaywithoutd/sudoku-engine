...
import {
  shortFixtures,
  findShort,
  independentShortCertificate,
  FixtureProof,
} from "../../solver/short-pattern-acceptance";
...
import {
  checkPatternProof,
  validateShortPattern,
  validateWingPattern,
  validateBentPattern,
  validateRemotePattern,
} from "../../../src/solver/techniques/pattern-contracts";
import type { ProofNode } from "../../../src/solver/proof/types";
test.each(shortFixtures.filter((f) => ["C11", "C12"].includes(f.rowId)))(
  "$id has independent named evidence and productive discovery",
  (f) => {
    const view = fixtureView(f);
    expect(() => assertSound(view, independentShortCertificate(f))).not.toThrow();
    const found = findShort(f);
    expect(found.proposal.effects).toEqual(expect.arrayContaining(f.expectedEffects));
    expect(() => assertSound(view, found.proposal)).not.toThrow();
  },
  30000,
);
test.each(allShortFixtures.filter((f) => f.expectation !== "productive"))(
  "$id independently rejects invalid named geometry",
  (f) => {
    const view = fixtureView(f);
    const validators = {
      C10: validateShortPattern,
      C11: validateWingPattern,
      C12: validateBentPattern,
      C13: validateRemotePattern,
    };
    expect(() =>
      validators[f.rowId as keyof typeof validators](
        view,
...
test.each(shortFixtures)("$id enforces its named alias and exact tuple/path boundary", (f) => {
  ...
  const validate = (p: Json) =>
    validators[f.rowId as keyof typeof validators](view, p as never, f.expectedEffects);
  ...
  if (f.rowId === "C11") {
    if (p.alias === "W-Wing") p.endpoints.push(p.endpoints[0]);
    else p.wings.push(p.pivot);
  }
...
test.each(shortFixtures)(
  "$id rejects primitive mutations and replays the original clue prefix",
  (f) => {
    const view = fixtureView(f),
      proposal = independentShortCertificate(f),
      limits = discoveryContext().limits;
    const check = (p: typeof proposal) =>
      [
        ...checkProposal(p, {
          view,
    ...
    const badRule = {
      ...
      proof: {
        ...proposal.proof,
        nodes: proposal.proof.nodes.map((n, i) => (i ? n : { ...n, rule: "invented@1" })),
      },
    };
    ...
    const badPremise = {
      ...
      proof: {
        ...proposal.proof,
        nodes: proposal.proof.nodes.map((n, i) => (i ? n : { ...n, premises: [] })),
      },
    };
    expect(check(badPremise)?.kind).toBe("rejected");
    const badPattern = {
      ...proposal,
      pattern: { ...(proposal.pattern as object), unexplained: true },
    };
    expect(check(badPattern)?.kind).toBe("rejected");
    const snapshot = {
      snapshotId: "independent-prefix",
      inputRevision: 0,
      problem: view.assembly.problem,
      source: { kind: "manual" as const },
    };
    const events = [
      ...replay(snapshot, [...originalCluePrefix(f), proposal], view.assembly, limits),
    ];
    expect(events.at(-1)?.kind).toBe("checked");
    expect(events.filter((e) => e.kind === "checked")).toHaveLength(
      originalCluePrefix(f).length + 1,
    );
  },
  30000,
);
test("all original clue prestates independently recompute every domain without using production indexes", () => {
  for (const f of shortFixtures)
    for (let cell = 0; cell < 81; cell++) {
      const given = Number(f.givens[cell]);
      let mask = 0;
      for (let s = 1; s <= 9; s++)
        if (
          given
            ...
            : ![...f.givens].some(
                (digit, peer) =>
                  Number(digit) === s &&
                  peer !== cell &&
                  (Math.floor(peer / 9) === Math.floor(cell / 9) ||
                    peer % 9 === cell % 9 ||
                    Math.floor(peer / 27) * 3 + Math.floor((peer % 9) / 3) ===
                      Math.floor(cell / 27) * 3 + Math.floor((cell % 9) / 3)),
              )
        )
          mask += 2 ** (s - 1);
      expect(f.preState.domains[cell], `${f.id}:${cell}`).toBe(mask);
      expect(f.preState.values[cell]).toBe(given);
    }
});
test("durable seed hashes reproduce SAT and both forced and forbidden counterfactuals independently", () => {
  ...
  for (const f of shortFixtures) {
    ...
    for (const e of f.expectedEffects) {
      const force = oracle({ ...input, force: [e.cell, e.symbol] });
      expect(force, f.id).toMatchObject({ interrupted: false, exhausted: true, witnesses: [] });
      const forbid = oracle({ ...input, forbid: [e.cell, e.symbol] });
      expect(forbid.interrupted).toBe(false);
      expect(forbid.witnesses).toHaveLength(1);
      counterfactuals++;
    }
  }
...
test("named XY-Wing consumes a relation conflict through unconditional domain filters and clause projection", () => {
    ...
    canonicalProblem({
      ...
      constraints: [
        ...[
          ...
          [1, 3],
          [2, 3],
        ].map((cells, i) => ({ id: `scope:${i}`, type: "all-different@1", cells, parameters: {} })),
        { id: "relation", type: "order@1", cells: [0, 1], parameters: {} },
      ],
    }),
    mockRuleRegistry,
  );
  ...
  const descriptor = getTechniques("classic-expanded@1").find((d) => d.id === "c11@1")!,
    context = discoveryContext();
  const event = [...descriptor.discover(view, context)].find(
    (e) =>
      e.kind === "proposal" &&
      JSON.stringify(e.proposal.pattern) ===
        JSON.stringify({ alias: "XY-Wing", pivot: 0, wings: [1, 2], x: 1, y: 2, z: 3 }),
  );
  expect(event?.kind).toBe("proposal");
  if (event?.kind !== "proposal") throw Error("no-wing");
  ...
  expect(
    assertSound(view, event.proposal).consequences.some((c) => c.rules.includes("relation")),
  ).toBe(true);
  expect(context.workspace.usage).toEqual({ entries: 0, bytes: 0 });
});

const boundaryTestName =
  "both Dual Empty Rectangle roots and every C12 table boundary remain necessary";
...
test(`${boundaryTestName}: complete fixture matrix`, () => {
  expect(c12BoundaryFixtures.map((f) => f.id).sort()).toEqual(
    shortFixtures
      .filter((f) => f.rowId === "C12")
      .map((f) => f.id)
      .sort(),
  );
...
test(`${boundaryTestName}: Dual Empty Rectangle`, () => {
  const dual = shortFixtures.find((f) => f.id === "C10-dual-er")!,
    view = fixtureView(dual),
    proposal = independentShortCertificate(dual),
    limits = discoveryContext().limits;
  const check = (p: typeof proposal) =>
    [
      ...checkProposal(p, {
  ...
  for (const index of [0, 1])
    expect(
      check({
        ...proposal,
        pattern: { ...pattern, paths: pattern.paths.filter((_, i) => i !== index) },
      })?.kind,
    ).toBe("rejected");
      ...
      check({
        ...
        proof: {
          ...
          nodes: proposal.proof.nodes.map((n) =>
            n.id === node.id ? { ...n, premises: n.premises.slice(0, -1) } : n,
          ),
        },
      })?.kind,
    ).toBe("rejected");
...
test.each(
  c12BoundaryFixtures.flatMap((f) =>
    Array.from({ length: f.leafCount }, (_, filterIndex) => ({ ...f, filterIndex })),
  ),
)(`${boundaryTestName}: $id filter $filterIndex`, ({ id, leafCount, filterIndex }) => {
  const f = shortFixtures.find((f) => f.id === id)!;
  expect(f).toBeDefined();
  const localView = fixtureView(f),
    p = independentShortCertificate(f);
  ...
  const base = {
    view: localView,
    retained: retainedProof(localView),
    limits: discoveryContext().limits,
  ...
  const foreign = {
    ...
    proof: {
      ...p.proof,
      nodes: p.proof.nodes.map((n) =>
        n.id === filter.id
          ? { ...n, parameters: { ...params, cells: [80, ...params.cells.slice(1)] } }
          : n,
      ),
    },
  };
  ...
  const partial = {
    ...
    proof: {
      ...p.proof,
      nodes: p.proof.nodes.map((n) =>
        n.id === filter.id ? { ...n, premises: n.premises.slice(1) } : n,
      ),
    },
  };
...
test("named C12 requires nonempty survivors and cannot invent an unrestricted symbol by omitting a conflict", () => {
  const f = shortFixtures.find((f) => f.id === "C12-n4")!,
    view = fixtureView(f),
    p = f.expectedPattern as Record<string, any>;
  const table: ProofNode = {
    id: 9999,
    rule: "table-filter@1",
    premises: [],
    parameters: {},
    scope: [],
    conclusion: { kind: "table", cells: p.cells, count: 0, definition: 9999 },
  };
  const projection: ProofNode = {
    id: 10000,
    rule: "table-project@1",
    premises: [9999],
    parameters: {},
    scope: [],
    conclusion: {
      kind: "clause",
      alternatives: p.occurrences[p.nonrestrictedSymbol].map((cell: number) => ({
        cell,
        symbol: p.nonrestrictedSymbol,
        positive: true,
      })),
    },
  };
    ...
    checkPatternProof(
      {
        ...
        proof: { state: view.state.key, nodes: [projection], imports: [], roots: [10000] },
      },
      view,
      new Map([
        [9999, table],
...
test("C12 partitions a 5000-assignment original-clue local table and checks all 220 survivors", () => {
  const f = shortFixtures.find((f) => f.id === "C12-partition-n6")!,
    view = fixtureView(f),
    independent = independentShortCertificate(f),
    found = findShort(f);
  const pattern = f.expectedPattern as { cells: number[] };
  const volume = pattern.cells.reduce(
    (n, cell) =>
      n *
  ...
  for (const proposal of [independent, found.proposal]) {
    const unions = proposal.proof.nodes.filter((n) => n.rule === "table-union@1");
    expect(unions.length).toBeGreaterThan(0);
    expect(unions.at(-1)?.conclusion).toMatchObject({
      kind: "table",
      count: 220,
      cells: pattern.cells,
    });
    ...
    for (const leaf of proposal.proof.nodes.filter((n) => n.rule === "table-filter@1")) {
      ...
      expect(
        masks.reduce(
          (n, mask) =>
            n *
            Array.from({ length: 9 }, (_, i) => i + 1).filter((s) => mask & (1 << (s - 1))).length,
          1,
        ),
    ...
    const events = [
      ...replay(
        {
          snapshotId: "partition-fixture",
          inputRevision: 0,
          problem: view.assembly.problem,
          source: { kind: "manual" },
        },
        [...originalCluePrefix(f), proposal],
        view.assembly,
...
test.each(["omitted-conflict", "no-table", "unrelated-table-root", "unrelated-effect-root"])(
  "C12 rejects a primitive-valid %s substitution for its mandatory local table",
  (substitution) => {
    const f = shortFixtures.find((f) => f.id === "C12-n4")!,
      view = fixtureView(f),
      b = new FixtureProof(view),
      p = f.expectedPattern as Record<string, any>;
    const positive = (cell: number, symbol: number) => ({ cell, symbol, positive: true }),
      effect = f.expectedEffects[0],
      target = positive(effect.cell, effect.symbol);
    const clauses: number[] = [];
    if (substitution === "omitted-conflict") {
      const reduced = {
        ...p,
        conflicts: p.conflicts.filter(([a, c]: number[]) => a !== 19 || c !== 37),
      };
      clauses.push(b.table(reduced));
      expect(b.nodes.find((n) => n.rule === "table-filter@1")?.conclusion).toMatchObject({
        kind: "table",
        count: 5,
      });
      clauses.push(
        ...p.occurrences[p.nonrestrictedSymbol].map((cell: number) =>
          b.weak(positive(cell, effect.symbol), target),
        ),
      );
    } else {
      clauses.push(
        b.cell(2),
        b.cell(19),
        b.weak(positive(2, 9), positive(19, 9)),
        b.weak(positive(2, 4), target),
        b.weak(positive(19, 4), target),
      );
    }
    const roots = [b.eliminate(clauses, target)];
    if (substitution === "unrelated-table-root") roots.push(b.table(p));
    if (substitution === "unrelated-effect-root")
      roots.push(
        b.eliminate(
          [
            b.table(p),
    ...
    const proposal = b.finish(f, roots),
      context = {
        view,
        retained: retainedProof(view),
        limits: discoveryContext().limits,
    ...
    if (substitution === "no-table") {
      expect(proposal.proof.nodes).toHaveLength(10);
      expect(proposal.proof.nodes.some((n) => n.rule.startsWith("table-"))).toBe(false);
    }
    ...
    const primitive = [
      ...verifyCertificate(
        substitution === "unrelated-table-root" ? { ...proposal, effects: [] } : proposal,
        context,
      ),
    ].at(-1);
    expect(primitive?.kind, primitive?.kind === "rejected" ? primitive.code : undefined).toBe(
      "verified",
    );
    if (substitution === "unrelated-table-root")
      expect(() =>
        checkPatternProof(
          proposal,
          view,
          new Map([...retainedProof(view), ...proposal.proof.nodes.map((n) => [n.id, n] as const)]),
        ),
      ).toThrow("missing-bent-effect-lineage");
