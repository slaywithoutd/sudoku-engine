...
function countProof(given = 0) {
  ...
  const node: ProofNode = {
    id,
    rule: "cover-count-clause@1",
    premises: [cover, scope, ...view.state.domainFacts],
    scope: [],
    parameters: {
      symbol: 1,
      covers: [{ premise: cover, coefficient: 2 }],
      capacities: [{ premise: scope, coefficient: 1 }],
    },
    conclusion: {
      kind: "clause",
      alternatives: [
  ...
  const proposal: DeductionProposal = {
    ...
    proof: {
      state: view.state.key,
      nodes: [node],
      imports: [...node.premises].sort((a, b) => a - b),
      roots: [id],
    },
  };
  const check = () =>
...
test("signed cover count proves an OR from negative incidences", () =>
  expect(countProof().check()?.kind).toBe("verified"));
test("signed cover count rejects a false implication under a compatible falsification", () => {
  const b = countProof();
  (b.node.conclusion as any).alternatives[0].positive = false;
...
test("positive incidences prove the negative OR and preserve checked integer weights", () => {
  const b = countProof();
  (b.node.parameters as any).covers[0].coefficient = 1;
  (b.node.parameters as any).capacities[0].coefficient = 2;
  (b.node.conclusion as any).alternatives.forEach((v: any) => (v.positive = false));
  expect(b.check()?.kind).toBe("verified");
  (b.node.parameters as any).covers[0].coefficient = 40;
  (b.node.parameters as any).capacities[0].coefficient = 81;
  expect(b.check()?.kind).toBe("verified");
});
test.each([
  "wrong-symbol",
  ...
  "complement",
  "duplicate-literal",
])("signed count rejects %s", (mode) => {
  const b = countProof(),
    p = b.node.parameters as any,
    c = b.node.conclusion as any,
    n = b.node as any;
  if (mode === "wrong-symbol") c.alternatives[0].symbol = 2;
  if (mode === "missing-domain") {
    n.premises.pop();
    (b.proposal.proof as any).imports = [...n.premises].sort((a: number, b: number) => a - b);
  }
...
test.each([1, 2])(
  "singleton/absent domains reject incompatible falsification for given %s",
  (given) => {
    const b = countProof(given);
    if (given === 2) (b.node.conclusion as any).alternatives[0].positive = false;
    expect(b.check()).toMatchObject({
      kind: "rejected",
      code: "incompatible-count-clause-falsification",
    });
  },
);
test("absent counted symbol remains a forced zero in compatible count entailment", () =>
  expect(countProof(2).check()?.kind).toBe("verified"));
...
test("signed-count strategy carries every premise's scopes, conditional taint, and original rules", () => {
  const b = countProof(),
    metadata = new Map(
      b.node.premises.map((id, i) => [
        id,
        {
          conclusion: b.retained.get(id)!.conclusion,
          openAssumptions: [100 + i],
          conditional: i === 1,
          rules: [`source:${i}`],
        },
      ]),
    );
  const cursor = new CoverCountClauseChecker().check(b.node, {
    view: b.view,
    retained: b.retained,
    limits: discoveryContext().limits,
    policy: "discharged",
    uniqueEvidenceId: null,
    workspaceRemaining: 100000,
  ...
  expect(result).toMatchObject({
    conditional: true,
    openAssumptions: [100, 101, 102, 103],
    rules: ["source:0", "source:1", "source:2", "source:3"],
  });
  expect(() =>
    new CoverCountClauseChecker()
      .check(b.node, {
        view: b.view,
        retained: b.retained,
        limits: discoveryContext().limits,
        policy: "discharged",
        uniqueEvidenceId: null,
        workspaceRemaining: 65535,
        premiseInferences: metadata,
      })
      .next(),
  ).toThrow("count-clause-workspace-limit");
});
