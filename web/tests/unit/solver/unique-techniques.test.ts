...
import { independentUniqueTrade, independentUniqueCertificate, independentUniquePlan, independentNamedUnique, type UniqueSeed } from "../../solver/unique-independent";
...
const seeds: UniqueSeed[] = [u01, u02, u03, u04, u05].flatMap(file => file.fixtures as UniqueSeed[]);
...
describe("independent original-clue uniqueness", () => {
  test.each(seeds)("$id", fixture => {
    ...
    for (const effect of fixture.expectedEffects) {
      const opposite = oracle({ givens, limit: 2, maxNodes: 1_000_000,
        ...(effect.kind === "place" ? { forbid: [effect.cell, effect.symbol] as [number, number] }
...
for (const seed of seeds.filter(s => s.id !== "U01-nonunique-trade")) {
  ...
  test(`${seed.id} has an authentic complete primitive trade`, async () => {
  const h = uniqueHarness(seed);
  try {
    const view = h.operation.view, proposal = independentUniqueCertificate(view, seed, h.parent.evidenceId);
    const result = [...verifyCertificate(proposal, { view, retained: retainedProof(view), policy: "unique-only", uniqueEvidenceId: h.parent.evidenceId,
      uniqueAuthority: h.operation.authority, limits: uniqueLimits })].at(-1);
    expect(result?.kind, JSON.stringify(result)).toBe("verified");
    if (result?.kind === "verified") expect(result.certificate.consequences.every(c => c.conditional && !c.openAssumptions.length)).toBe(true);
    for (const effect of seed.expectedEffects as Effect[]) {
      const plan = independentUniquePlan(seed,effect);
      for (const proposal of [independentNamedUnique(view,seed,h.parent.evidenceId,effect), compileUnique(view,plan,effect,h.operation.authority)]) {
        const checked = [...checkProposal(proposal, { view, retained: retainedProof(view), policy: "unique-only", uniqueEvidenceId: h.parent.evidenceId,
          uniqueAuthority: h.operation.authority, limits: uniqueLimits })].at(-1);
        if (seed.id.includes("length14")) expect(checked).toEqual({ kind: "rejected", code: "unique-loop-bound" });
        else {
          expect(checked?.kind, `${seed.id}:${JSON.stringify(checked)}`).toBe("checked");
          const ctx={view,retained:retainedProof(view),policy:"unique-only" as const,uniqueEvidenceId:h.parent.evidenceId,uniqueAuthority:h.operation.authority,limits:uniqueLimits};
          expect([...checkProposal(proposal,{...ctx,limits:{...uniqueLimits,stepNodes:proposal.proof.nodes.length-1}})].at(-1)?.kind).toBe("rejected");
          if(seed.id==="U01-type6-independent")for(const mutate of [
            (p:any)=>p.effects.pop(),(p:any)=>p.effects.push(p.effects[0]),
            (p:any)=>delete p.pattern.certificate.companion,
            (p:any)=>p.pattern.geometry.strongHouses.pop(),
            (p:any)=>{p.pattern.certificate.companion.root=p.pattern.certificate.root;},
          ]) {const bad=structuredClone(proposal);mutate(bad);expect([...checkProposal(bad,ctx)].at(-1)?.kind).toBe("rejected");}
          const missing=structuredClone(proposal);(missing.pattern as any).geometry.guardians.pop();
          expect([...checkProposal(missing,ctx)].at(-1)?.kind).toBe("rejected");
        }
      }
    }
    if (!seed.id.includes("length14")) {
      const descriptor = getTechniques("classic-conditional@1").find(d => d.id === `${seed.rowId.toLowerCase()}@1`)!;
      const baseline = h.workspace.usage;
      const cursor = descriptor.discover(view,{ workspace:h.workspace,limits:uniqueLimits,uniqueAuthority:h.operation.authority });
      const expected=independentUniquePlan(seed,seed.expectedEffects[0]).geometry;
      let found=false,terminal="unfinished",proposals=0;
      try {
        for (const event of cursor) {
          if(event.kind!=="work")terminal=event.kind;
          if(event.kind!=="proposal")continue;
          proposals++;
          const checked=[...checkProposal(event.proposal,{view,retained:retainedProof(view),policy:"unique-only",uniqueEvidenceId:h.parent.evidenceId,
            uniqueAuthority:h.operation.authority,limits:uniqueLimits})].at(-1);
          expect(checked?.kind,`discovery:${seed.id}:${JSON.stringify(checked)}`).toBe("checked");
          for(const effect of event.proposal.effects) {
            const opposite=oracle({givens:[...seed.givens].map(Number),limit:2,maxNodes:1_000_000,
              ...(effect.kind==="place"?{forbid:[effect.cell,effect.symbol] as [number,number]}:{force:[effect.cell,effect.symbol] as [number,number]})});
            expect(opposite.exhausted&&opposite.witnesses.length===0&&!opposite.interrupted).toBe(true);
          }
          const g=(event.proposal.pattern as any).geometry;
          if(g.kind===expected.kind&&g.cells.length===expected.cells.length&&g.guardians.length===expected.guardians.length&&g.auxiliaryCells.length===expected.auxiliaryCells.length) {found=true;break;}
        }
      ...
      const compiled=compileUnique(view,independentUniquePlan(seed,seed.expectedEffects[0]),seed.expectedEffects[0] as Effect,h.operation.authority);
      const committed=[...h.operation.checkAndCommit(compiled)].at(-1);expect(committed?.kind,JSON.stringify(committed)).toBe("checked");
      const replayWorkspace=new IndexWorkspace({entryLimit:1_000_000,byteLimit:uniqueLimits.workspaceBytes});
      const fresh=ConditionalOperation.begin(h.parent,h.operation.run,uniqueLimits,replayWorkspace);
      try {
