...
test.each(foundationFixtures.filter((f) => f.expectation === "productive"))(
  "independently discovers $id with its exact named geometry and effects",
  (fixture) => {
...
const aliasFixtures = [
  ...new Map(
    foundationFixtures
      .filter((f) => f.expectation === "productive")
      .map((f) => [`${f.rowId}:${f.alias}`, f.id]),
  ).values(),
];
...
test.each(aliasFixtures)(
  "replays the original-clue prefix and independently authored %s certificate",
  (id) => {
    const view = fixtureView(fixtureCase(id)),
      proposals = [...fixturePrefix(id), fixtureCertificate(id)];
    const limits = {
      timeMs: 20000,
      workUnits: 1000000,
      exactNodes: 1000000,
    ...
    const events = [
      ...replay(
        makeSnapshot(view.assembly.problem, { kind: "manual" }, id, 0),
        proposals,
        view.assembly,
        limits,
      ),
...
test.each(foundationFixtures.filter((f) => f.expectation !== "productive"))(
  "rejects independently authored $id without substituting another pattern",
  (fixture) => {
...
test("covers every cell and all 27 houses × nine symbols through original-clue single prefixes", () => {
  const covers = new Set<string>(),
    descriptors = getTechniques("classic-expanded@1");
  for (let cell = 0; cell < 81; cell++) {
    const f = fixtureCase("C01-one-hole"),
      values = [...SOLUTION].map(Number),
      symbol = values[cell];
    values[cell] = 0;
    f.givens = values.join("");
    f.preState = { values, domains: [...SOLUTION].map((d) => 2 ** (Number(d) - 1)) };
    const view = fixtureView(f);
    const naked = [...descriptors[0].discover(view, discoveryContext())].flatMap((e) =>
    ...
    for (const alias of ["Naked Single", "Full House", "Last Digit"]) {
      ...
      expect(proposal.effects).toEqual([{ kind: "place", cell, symbol }]);
      assertSound(view, proposal);
    }
    const hidden = [...descriptors[1].discover(view, discoveryContext())].flatMap((e) =>
      e.kind === "proposal" ? [e.proposal] : [],
    );
    expect(hidden).toHaveLength(3);
    for (const proposal of hidden) {
      covers.add((proposal.pattern as { cover: string }).cover);
      assertSound(view, proposal);
    }
  }
...
test.each(["Locked Candidates", "direct forms"])(
  "independently checks the %s alias without automatically placing a direct consequence",
  (alias) => {
    const f = fixtureCase("C03-point-row"),
      view = fixtureView(f);
    ...
    expect(proposal.effects).toEqual(f.expectedEffects);
    expect(proposal.effects.every((e) => e.kind === "remove")).toBe(true);
    assertSound(view, proposal);
  },
);
...
test("small all-different scopes cannot produce Hidden Single covers", () => {
  const result = assemble(
    canonicalProblem({
      ...
      givens: [1, 2, 0],
      constraints: [
        { id: "small-cage", type: "all-different@1", cells: [0, 1, 2], parameters: {} },
      ],
    }),
  ...
  expect(
    [...getTechniques("classic-expanded@1")[1].discover(view, discoveryContext())].filter(
      (e) => e.kind === "proposal",
    ),
  ).toEqual([]);
});
test.each(["record", "getter", "proxy"])(
  "a forged candidate view (%s) cannot promote a naked single to a false Full House",
  (kind) => {
    const result = assemble(
      canonicalProblem({
        schema: 1,
    ...
    for (const rule of view.assembly.problem.constraints)
      for (const e of [...view.assembly.modules.get(rule.id)!.propagate(view, rule)])
    ...
    const event = [
      ...
      (e) =>
        e.kind === "proposal" &&
        (e.proposal.pattern as { cell: number; alias: string }).cell === 1 &&
        (e.proposal.pattern as { alias: string }).alias === "Naked Single",
    );
    if (event?.kind !== "proposal") throw Error("single");
    const forged = {
      ...event.proposal,
      pattern: { kind: "single", alias: "Full House", cell: 1, symbol: 2, house: "edge:1" },
    };
    const spoof = () => {
        ...
        return new Proxy(
          { ...view },
          {
            get(target, key, receiver) {
              return key === "state" ? state() : Reflect.get(target, key, receiver);
            },
          },
        );
    ...
    const cache = {
      ...
      proof: {
        ...forged.proof,
        nodes: forged.proof.nodes.slice(0, 1),
        roots: [forged.proof.nodes[0].id],
      },
    };
    expect(
      [
...
test("C01 cache retains one bounded fact without revision churn and rejects duplicate work", () => {
  const view = fixtureView(fixtureCase("C01-one-hole")),
    full = fixtureCertificate("C01-one-hole");
  const proposal = {
    ...full,
    effects: [],
    proof: { ...full.proof, nodes: full.proof.nodes.slice(0, 1), roots: [full.proof.nodes[0].id] },
  };
  const limits = {
    timeMs: 20000,
    workUnits: 1000000,
  ...
  const event = [...checkProposal(proposal, context)].at(-1);
  expect(event?.kind).toBe("checked");
  if (event?.kind !== "checked") return;
  const next = retainCheckedFacts(view, event.step);
  expect(next.state.key.revision).toBe(view.state.key.revision);
  expect(next.state.domains).toEqual(view.state.domains);
  ...
  const repeated = {
    ...
    proof: {
      ...proposal.proof,
      nodes: proposal.proof.nodes.map((n) => ({ ...n, id: n.id + 1 })),
      roots: proposal.proof.roots.map((n) => n + 1),
    },
  };
  expect(
    [...checkProposal(repeated, { ...context, view: next, retained: retainedProof(next) })].at(-1),
  ).toMatchObject({ kind: "rejected", code: "invalid-single-cache" });
  ...
  expect(
    [...checkProposal(generic, { ...context, admission: "certificate" } as typeof context)].at(-1)
      ?.kind,
  ).toBe("rejected");
});
...
test("mandatory all-different maintenance proposes checked peer exclusions before named discovery", () => {
  ...
  const assembly = assemble(
    normalizeClassic({ kind: "classic", version: 1, width: 9, height: 9, givens }),
    [new AllDifferentRule()],
  );
...
test("C01 independently checks the one-hole placement after checked maintenance", () => {
  ...
  const event = [...technique.discover(view, discoveryContext())].find(
    (e) => e.kind === "proposal",
  );
