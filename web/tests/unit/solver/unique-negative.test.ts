...
const context = (h: ReturnType<typeof uniqueHarness>) => ({
  view: h.operation.view,
  retained: retainedProof(h.operation.view),
...
function ordinarySingleCache(view: ReadView): DeductionProposal {
  for (const event of new NakedSingles().discover(view)) {
    ...
    const full = event.proposal,
      node = full.proof.nodes[0];
    return {
      ...full,
      effects: [],
      proof: { ...full.proof, nodes: [node], roots: [node.id], imports: [...node.premises] },
    };
  }
...
test.each(["unconditional", "discharged"] as const)(
  ...
  (policy) => {
    ...
    try {
      ...
      const committed =
        terminal?.kind === "checked" ? commitChecked(view, terminal.step).view : undefined;
      const cache = [
        ...checkProposal(ordinarySingleCache(committed!), {
          ...omitted,
          view: committed!,
          retained: retainedProof(committed!),
        }),
      ].at(-1);
      ...
      if (terminal?.kind === "checked") {
        ...
        if (cache?.kind === "checked")
          expect
            .soft(() => retainCheckedFacts(committed!, cache.step))
            .toThrow("revoked-unique-authority");
      }
    } finally {
      h.operation.dispose();
    }
...
test("same-label independent and primary owners keep ordinary semantics without borrowing a conditional lifetime", () => {
  ...
  try {
    ...
    const ordinaryCheck = (view: ReadView) =>
      [
        ...checkProposal(ordinaryTemplate(view), {
          view,
          retained: retainedProof(view),
    ...
    const bound = ordinaryCheck(conditional),
      unbound = ordinaryCheck(sibling);
    const primary = h.context.acceptedView,
      primaryStep = ordinaryCheck(primary);
    expect(bound?.kind).toBe("checked");
    expect(unbound?.kind).toBe("checked");
    expect(primaryStep?.kind).toBe("checked");
    if (
      bound?.kind === "checked" &&
      unbound?.kind === "checked" &&
      primaryStep?.kind === "checked"
    ) {
      ...
      expect(() => retainCheckedFacts(conditionalAfter, unbound.step)).toThrow(
        "missing-unique-authority",
      );
      expect(() => commitChecked(sibling, bound.step)).toThrow("revoked-unique-authority");
      h.operation.dispose();
      for (const [view, step] of [
        [sibling, unbound.step],
        [primary, primaryStep.step],
      ] as const) {
        const after = commitChecked(view, step).view;
        const cache = [
          ...checkProposal(ordinarySingleCache(after), {
            view: after,
            retained: retainedProof(after),
            policy: "unconditional",
            uniqueEvidenceId: null,
            limits: uniqueLimits,
          }),
        ...
        expect(cache?.kind).toBe("checked");
        if (cache?.kind === "checked")
          expect(() => retainCheckedFacts(after, cache.step)).not.toThrow();
...
test("forged authority, missing complete sources and altered trade geometry reject", () => {
  ...
  try {
    ...
    for (const authority of [undefined, structuredClone(h.operation.authority), {}])
      expect(
        [...checkProposal(proposal, { ...ctx, uniqueAuthority: authority as any })].at(-1)?.kind,
      ).toBe("rejected");
    expect(
      [
        ...checkProposal(proposal, {
          ...ctx,
          view: initialize(h.operation.assembly, h.operation.view.state.key.branch),
        }),
      ].at(-1)?.kind,
    ).toBe("rejected");
    const mutations: ((p: any) => void)[] = [
      (p) => {
        p.proof.nodes.find((n: any) => n.rule === "unique-transform@1").conclusion = {
          kind: "false",
        };
      },
      (p) => {
        p.proof.nodes.find((n: any) => n.rule === "unique-transform@1").parameters.coreMasks[0] =
          511;
      },
      (p) => {
        p.proof.nodes.find((n: any) => n.rule === "unique-transform@1").parameters.cells[0] = 80;
      },
      (p) => {
        p.proof.nodes.find((n: any) => n.rule === "unique-transform@1").parameters.evidenceId =
          "matching-label-is-not-authority";
      },
      (p) => {
        p.proof.nodes.find((n: any) => n.rule === "unique-transform@1").premises.pop();
      },
      (p) => {
        p.pattern.geometry.guardians.pop();
      },
      (p) => {
        p.pattern.geometry.alias = "invented unique family";
      },
      (p) => {
        p.proof.roots.pop();
      },
      (p) => {
        p.effects[0].symbol = 9;
      },
    ];
    for (const mutate of mutations) {
      const changed = structuredClone(proposal);
      mutate(changed);
      expect([...checkProposal(changed, ctx)].at(-1)?.kind).toBe("rejected");
    }
    const descriptor = getTechniques("classic-conditional@1").find((d) => d.id === "u01@1")!;
    expect([
      ...descriptor.discover(h.operation.view, { workspace: h.workspace, limits: uniqueLimits }),
    ]).toEqual([{ kind: "disabled", reason: "missing-unique-authority" }]);
  } finally {
    h.operation.dispose();
  }
}, 30000);
...
test("unconditional displayed roots cannot publish an unused conditional intermediate after revocation", () => {
  ...
  try {
    ...
    for (let symbol = 1; symbol <= 9 && !ordinary; symbol++) {
      ...
      if (independentTemplates(fixture as any).effects.length)
        ordinary = independentTemplateCertificate(fixture as any, view);
    }
    expect(ordinary).toBeDefined();
    const unique = independentUniqueCertificate(view, seed, h.parent.evidenceId),
      offset = ordinary!.proof.nodes.at(-1)!.id + 1 - unique.proof.nodes[0].id;
    const ids = new Set(unique.proof.nodes.map((n) => n.id));
    const nodes = unique.proof.nodes.map((n) => ({
      ...n,
      id: n.id + offset,
      premises: n.premises.map((id) => (ids.has(id) ? id + offset : id)),
    }));
    const proposal = {
      ...ordinary!,
      proof: {
        ...ordinary!.proof,
        nodes: [...ordinary!.proof.nodes, ...nodes],
        imports: [...new Set([...ordinary!.proof.imports, ...unique.proof.imports])].sort(
          (a, b) => a - b,
        ),
      },
    };
    expect([...checkProposal(proposal, context(h))].at(-1)).toEqual({
      kind: "rejected",
...
for (const [raw, alias] of [
  [u04.fixtures[0], "BUG"],
  [u05.fixtures[0], "Generalized BUG"],
] as const)
  test(`${alias} is an admitted named alias`, () => {
    const f = raw as UniqueSeed,
      h = uniqueHarness(f);
    try {
      const plan = independentUniquePlan(f, f.expectedEffects[0]);
      (plan.geometry as any).alias = alias;
      const proposal = compileUnique(
        h.operation.view,
        plan,
        f.expectedEffects[0] as any,
      ...
      const changed = structuredClone(proposal);
      (changed.pattern as any).geometry.guardians.pop();
      expect([...checkProposal(changed, context(h))].at(-1)?.kind).toBe("rejected");
    } finally {
      h.operation.dispose();
    }
...
test("affected mixed rules reject despite authentic uniqueness; disjoint rules preserve unchanged", () => {
  const exact = oracle({ givens: [...seed.givens].map(Number), limit: 2, maxNodes: 1_000_000 }),
    values = exact.witnesses[0];
  const core = seed.expectedPattern.cells as number[];
  for (const affected of [true, false]) {
    const first = affected
      ? core[0]
      : Array.from({ length: 81 }, (_, c) => c).find((c) => !core.includes(c))!;
    const second = Array.from({ length: 81 }, (_, c) => c).find(
      (c) => c !== first && !core.includes(c) && values[c] !== values[first],
    )!;
    const cells = values[first] < values[second] ? [first, second] : [second, first];
    const h = uniqueHarness(seed, {
      constraints: [{ id: "extra-order:0", type: "order@1", cells, parameters: {} }],
      modules: mockRuleRegistry,
    });
    try {
      const proposal = independentNamedUnique(
        h.operation.view,
      ...
      if (affected)
        expect(terminal).toEqual({
          kind: "rejected",
          code: "unique-unsupported-rule-preservation",
        });
