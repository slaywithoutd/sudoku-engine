...
function example(overlap = true) {
  const assembly = assemble(
    canonicalProblem({
      ...
      symbols: [1, 2, 3],
      givens: [0, 0, 0],
      constraints: [{ id: "row:0", type: "all-different@1", cells: [0, 1, 2], parameters: {} }],
    }),
  ...
  const leaf = (cells: number[]) => {
    ...
    return add(
      "table-filter@1",
      [...cells.map((c) => view.state.domainFacts[c]), subset],
      { kind: "table", cells, count: cells.length === 1 ? 3 : 6, definition: next },
      { cells, box: cells.map(() => 7) },
    );
  };
  const left = leaf(overlap ? [0, 1] : [0]),
    right = leaf(overlap ? [1, 2] : [1]);
  const filter = add("all-different-subset@1", [scope], {
    kind: "all-different",
    cells: overlap ? [0, 2] : [0, 1],
  });
  const cells = overlap ? [0, 1, 2] : [0, 1];
  const root = add("table-join-filter@1", [left, right, filter], {
    kind: "table",
    cells,
    count: 6,
    definition: next,
  ...
  const check = (limits = discoveryContext().limits) =>
    [
...
test("join filters preserve every clause alternative and reject one outside the union", () => {
  const b = example(false),
    source = [...b.view.facts.values()].find(
      (f) => f.proposition.kind === "cover" && f.proposition.symbol === 1,
    )!.id;
  const root = b.nodes.pop()!,
    clause = b.add("cover-clause@1", [source], {
      kind: "clause",
      alternatives: [0, 1, 2].map((cell) => ({ cell, symbol: 1, positive: true })),
    });
  b.nodes.push({
    ...root,
    id: clause + 1,
    premises: [b.left, b.right, clause],
    conclusion: { kind: "table", cells: b.cells, count: 4, definition: clause + 1 },
  });
  // Use a freshly rooted wire after replacing the final allocation.
  const p = b.proposal();
  (p.proof as any).roots = [clause + 1];
  expect(
    [
      ...verifyCertificate(p, {
        view: b.view,
  ...
  ).toMatchObject({ kind: "rejected", code: "invalid-join-filter-constraint" });
});
test.each(["incomplete", "copied", "parameters", "union"])(
  "filtered tables reject %s authority substitutions",
  (mode) => {
    const b = example(),
      root = b.nodes.at(-1)!;
    if (mode === "incomplete") {
      const leaf = b.nodes.find((n) => n.id === b.left)!;
      (leaf.parameters as any).box[0] = 1;
      (leaf.conclusion as any).count = 2;
    }
...
function admittedTable() {
  const b = example(),
    checker = new TableChecker(),
    retained = new Map(b.retained),
    metadata = new Map<number, CheckedInference>();
  for (const [id, n] of retained)
    metadata.set(id, {
  ...
  const context = (n: ProofNode, workspaceRemaining = 1000000): CheckContext => ({
    view: b.view,
    retained,
    limits: discoveryContext().limits,
    policy: "discharged",
    uniqueEvidenceId: null,
    premiseInferences: metadata,
    currentNode: n,
    workspaceRemaining,
  });
  for (const n of b.nodes) {
    retained.set(n.id, n);
    if (n.rule === "all-different-subset@1")
      metadata.set(n.id, {
        conclusion: n.conclusion,
        openAssumptions: n.id === b.filter ? [999] : [],
        conditional: n.id === b.filter,
        rules: [`scope:${n.id}`],
      });
    else {
      const cursor = checker.check(n, context(n));
      while (true) {
        const r = cursor.next();
        if (r.done) {
          metadata.set(n.id, r.value);
          break;
        }
      }
    }
...
test("filtered definition preserves constraint taint and budgets nested recomputation on projection", () => {
  const b = admittedTable();
  expect(b.metadata.get(b.root)).toMatchObject({ conditional: true, openAssumptions: [999] });
...
test("filtered row traversal can be closed and restarted without retaining expanded rows", () => {
  ...
  const cursor = b.checker.check(n, b.context(n));
  for (let i = 0; i < 20; i++) expect(cursor.next().done).toBe(false);
...
test("filtered join reconstructs counts and checks every supplied filter", () => {
  const b = example();
  (b.nodes.at(-1)!.conclusion as any).count = 12;
