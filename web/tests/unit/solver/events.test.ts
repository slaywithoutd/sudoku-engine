...
const before = { problemKey: "problem", branch: "primary", revision: 2 },
  after = { ...before, revision: 3 };
const changes: ChangeSet = {
  before,
  after,
  removed: [{ cell: 1, symbol: 1, positive: false }],
  placed: [],
  cells: [1],
  constraintIds: ["a"],
  coverIds: ["a:1"],
...
test("invalidates every affected dependency and all graph consumers while preserving unrelated exhaustion", () => {
  const ledger: Ledger = [
    ...
    },
    {
      ...
      state: before,
      status: "excluded",
      dependencies: [{ kind: "graph" }],
      work: 4,
      reason: "old",
    },
    ...(["cover", "relation", "constraint", "all"] as const).map((kind) => ({
      technique: "scope@1",
      scopeKey: kind,
      state: before,
      ...
      dependencies: [
        kind === "all"
          ? { kind }
          : { kind, id: kind === "cover" ? "a:1" : kind === "relation" ? "r" : "a" },
      ],
      work: 2,
    })),
  ];
...
test("discards stale cursors and unscoped exhaustion instead of carrying it to a new revision", () => {
  const ledger: Ledger = [
    ...
    },
    {
      technique: "scan@1",
      scopeKey: "stale",
      state: { ...before, revision: 1 },
      status: "exhausted",
      dependencies: [{ kind: "cell", cell: 7 }],
      work: 2,
    },
  ];
  expect(invalidate(changes, ledger).map((row) => row.status)).toEqual([
    "pending",
    "pending",
    "pending",
  ]);
