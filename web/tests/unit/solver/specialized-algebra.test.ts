...
test.each(fixtures)("$id independently proves every declared Junior/Double removal", (f) => {
  ...
  for (const effect of f.expectedEffects)
    expect(
      relation.rows.filter((row) => row[relation.cells.indexOf(effect.cell)] === effect.symbol),
      JSON.stringify(effect),
    ).toEqual([]);
});
test.each(["companion", "cross-line", "S-cells", "assigned-S", "cover-house", "base-symbols"])(
  "independent Junior checker rejects malformed %s geometry",
  (mutation) => {
    const f: any = structuredClone(fixtures[0]),
      p = f.expectedPattern;
    ...
    if (mutation === "assigned-S")
      p.covers.find((c: any) => c.assignedOccurrences.length).assignedOccurrences = [];
