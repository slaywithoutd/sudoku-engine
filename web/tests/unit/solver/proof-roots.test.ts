...
function fixture() {
  ...
  const state = {
    ...
    domains: Array<number>(81).fill(511),
    domainFacts: Array.from({ length: 81 }, (_, i) => i),
  };
  const context: CheckContext = {
    view: { assembly, facts, state, supports: () => [] },
  ...
  const node: ProofNode = {
    ...
    premises: [],
    parameters: {},
    conclusion: { kind: "literal", value: { cell: 0, symbol: 5, positive: true } },
    scope: [],
  };
  const proposal: DeductionProposal = {
    ...
    effects: [],
    pattern: { kind: "roots" },
    proof: { state: state.key, nodes: [node], imports: [], roots: [node.id] },
  };
...
describe("original proof authority", () => {
  test.each([
    ...
    "bounds original root allocation for %i cells, %i symbols and %i rules",
    (cells, symbols, rules) => {
  ...
  test.each([
    ...
    "applies matching primitive bounds for %i cells, %i symbols and %i rules",
    (cells, symbols, rules) => {
      const { context } = fixture();
      const assembly = smallAssembly(cells, symbols, rules);
      expect(() =>
        primitiveRegistry.check(
          {
            rule: "domain-axiom@1",
            premises: [],
            parameters: {},
          ...
          { ...context, view: { ...context.view, assembly } },
        ),
      ).toThrow();
    },
  );
  ...
  test.each(["allDifferent", "covers"] as const)(
    "bounds %s before canonical problem traversal",
    (kind) => {
      const { assembly } = fixture();
      const oversized = {
        ...assembly,
        [kind]: Array(kind === "covers" ? 2305 : 257),
  ...
  test.each(["sparse-scope", "nested-parameters"])(
    "preflights malformed %s before canonical traversal",
    (kind) => {
      const { assembly } = fixture();
      const changedRule = {
        ...assembly.problem.constraints[0],
        ...(kind === "sparse-scope"
          ? { cells: Array<number>(2) }
          : { parameters: { nested: { payload: true } } }),
      };
      const changed = {
        ...assembly,
        problem: {
          ...assembly.problem,
          constraints: [changedRule, ...assembly.problem.constraints.slice(1)],
          key: "forged-key-must-not-be-traversed-first",
        },
      };
      expect(() => createRoots(changed)).toThrow(
        kind === "sparse-scope" ? "invalid-root-array" : "invalid-root-parameters",
      );
    },
  );
  test("rejects accessor rule parameters without evaluating unbounded external code", () => {
    ...
    Object.defineProperty(changedRule, "parameters", {
      enumerable: true,
      get: () => {
        reads++;
        return {};
  ...
  test("binds original capability dependencies to their factory prefix too", () => {
    ...
    const result = terminal(
      { ...proposal, proof: { ...proposal.proof, nodes: [], imports: [109], roots: [109] } },
      { ...context, retained },
    );
    expect(result.kind).toBe("rejected");
    if (result.kind === "rejected") expect(result.code).toBe("substituted-retained-dependency");
  });
  test.each(["ascending", "descending", "dependent-first"])(
    "binds retained dependencies to the proved objects with %s map order",
    (order) => {
      const { context, proposal, node } = fixture();
      const rule: ProofNode = {
        ...node,
        rule: "rule-instance@1",
        conclusion: { kind: "rule", constraintId: "box:0" },
      };
      const scope: ProofNode = {
        ...node,
        id: node.id + 1,
        rule: "all-different@1",
        premises: [rule.id],
        parameters: { constraintId: "box:0" },
        conclusion: { kind: "all-different", cells: [0, 1, 2, 9, 10, 11, 18, 19, 20] },
      };
      const original = terminal(
        { ...proposal, proof: { ...proposal.proof, nodes: [rule, scope], roots: [scope.id] } },
        context,
      );
      ...
      const retained = (dependency: ProofNode) => {
        const entries: [number, ProofNode][] = [
          ...context.retained,
          [provedRule.id, dependency],
          [provedScope.id, provedScope],
        ];
      ...
      const imported = {
        ...proposal,
        proof: { ...proposal.proof, nodes: [], imports: [provedScope.id], roots: [provedScope.id] },
      };
      expect(terminal(imported, { ...context, retained: retained(provedRule) }).kind).toBe(
        "verified",
      );
      expect(terminal(imported, { ...context, retained: retained(provedGiven) })).toEqual({
        kind: "rejected",
        code: "substituted-retained-dependency",
      });
    },
  );
  test("materializes full original domains, exact clues and capability dependencies in canonical order", () => {
    ...
    expect(facts.get(81)?.proposition).toEqual({
      kind: "literal",
      value: { cell: 0, symbol: 5, positive: true },
    });
  ...
  test.each([
    [
      "fabricated clue",
      { kind: "literal", value: { cell: 0, symbol: 4, positive: true } },
      "given@1",
    ],
    ...
    expect(
      terminal(
        {
          ...proposal,
          proof: { ...proposal.proof, nodes: [{ ...node, conclusion, rule } as ProofNode] },
        },
        context,
      ).kind,
    ).toBe("rejected");
  ...
  test("reconstructs declared cover semantics and derives rule provenance from the rule premise", () => {
    ...
    expect(terminal({ ...good, proof: { ...good.proof, nodes: [altered] } }, context).kind).toBe(
      "rejected",
    );
  });
  test("checks original domains even when candidate state claims a narrower mask", () => {
    const { context, proposal, node } = fixture();
    const domain: ProofNode = {
      ...node,
      rule: "domain-axiom@1",
      conclusion: { kind: "domain", cell: 2, mask: 511 },
    };
    const changed = {
      ...context,
      view: {
        ...context.view,
        state: { ...context.view.state, domains: Array<number>(81).fill(1) },
      },
    };
    expect(
      terminal({ ...proposal, proof: { ...proposal.proof, nodes: [domain] } }, changed).kind,
    ).toBe("verified");
  });
  ...
  test.each(["arity", "bytes", "sparse", "accessor"])(
    "rejects malformed or oversized node %s before acceptance",
    (kind) => {
      const { context, proposal, node } = fixture();
      let altered = node;
      if (kind === "arity")
        altered = { ...node, premises: Array.from({ length: 65 }, (_, i) => i) };
      if (kind === "bytes") altered = { ...node, parameters: { data: "x".repeat(16 * 1024) } };
      if (kind === "sparse") altered = { ...node, premises: Array<number>(1) };
      if (kind === "accessor") {
        altered = { ...node };
        Object.defineProperty(altered, "parameters", {
          get: () => {
      ...
      expect(
        terminal({ ...proposal, proof: { ...proposal.proof, nodes: [altered] } }, context).kind,
      ).toBe("rejected");
    },
  );
  test("rejects capability scope forgery even when capability metadata is internally consistent", () => {
    ...
    const altered = {
      ...assembly,
      covers: assembly.covers.map((cover, index) => (index ? cover : { ...cover, cells: [0] })),
    };
  ...
  test("allocates capability facts canonically even when input capability iteration order changes", () => {
    ...
    const reordered = createRoots({
      ...assembly,
      allDifferent: [...assembly.allDifferent].reverse(),
      covers: [...assembly.covers].reverse(),
    });
  ...
  test("rejects duplicated capability identities", () => {
    const { assembly } = fixture();
    expect(() =>
      createRoots({ ...assembly, covers: [...assembly.covers, assembly.covers[0]] }),
    ).toThrow();
  });
  test.each([
    "forward",
    ...
    "parameters",
    "stale",
  ])("rejects %s at the DAG boundary", (kind) => {
    const { proposal, context, node } = fixture();
    ...
    if (kind === "forward" || kind === "cycle" || kind === "missing")
      changed = {
        ...
        proof: {
          ...proposal.proof,
          nodes: [{ ...node, premises: [kind === "cycle" ? node.id : node.id + 1] }],
        },
      };
    if (kind === "duplicate")
      changed = { ...proposal, proof: { ...proposal.proof, nodes: [node, node] } };
    if (kind === "dangling-root")
      changed = { ...proposal, proof: { ...proposal.proof, roots: [99999] } };
    if (kind === "unlisted-import")
      changed = { ...proposal, proof: { ...proposal.proof, nodes: [{ ...node, premises: [81] }] } };
    if (kind === "forged-import") {
      ctx = { ...context, retained: new Map([[81, { ...node, id: 81 }]]) };
      changed = {
        ...proposal,
        proof: { ...proposal.proof, nodes: [], roots: [81], imports: [81] },
      };
    }
    if (kind === "sibling-scope")
      changed = { ...proposal, proof: { ...proposal.proof, nodes: [{ ...node, scope: [81] }] } };
    if (kind === "unknown-technique") changed = { ...proposal, technique: "magic-solve@1" };
    if (kind === "effect")
      changed = { ...proposal, effects: [{ kind: "place", cell: 0, symbol: 5 }] };
    if (kind === "parameters")
      changed = {
        ...proposal,
        proof: { ...proposal.proof, nodes: [{ ...node, parameters: { solution: [5] } }] },
      };
    if (kind === "stale") changed = { ...proposal, state: { ...proposal.state, revision: 1 } };
    expect(
      kind === "unknown-technique"
        ? [...checkProposal(changed, ctx)].at(-1)?.kind
        : terminal(changed, ctx).kind,
    ).toBe("rejected");
  });
  test.each(["stepNodes", "runNodes", "stepBytes", "proofBytes", "workUnits"] as const)(
    "charges %s including retained initialization",
    (limit) => {
      const { context, proposal } = fixture();
      expect(
        terminal(proposal, {
          ...context,
          limits: { ...context.limits, [limit]: limit === "stepNodes" ? 0 : 1 },
        }).kind,
      ).toBe("rejected");
    },
  );
  test("checks imported original facts and emits bounded work events", () => {
    ...
    const events = [
      ...verifyCertificate(
        { ...proposal, proof: { ...proposal.proof, nodes: [], imports: [81], roots: [81] } },
        context,
      ),
    ];
    ...
    if (result.kind === "verified")
      expect(Object.isFrozen(result.certificate.consequences[0])).toBe(true);
  });
  ...
  test("yields after each bounded node before traversing the next node payload", () => {
    ...
    const events = [
      ...verifyCertificate(
        {
          ...proposal,
          proof: { ...proposal.proof, nodes: [node, next], roots: [node.id, next.id] },
        },
        context,
      ),
    ];
    expect(events.at(-1)?.kind).toBe("rejected");
    expect(events.filter((event) => event.kind === "work").length).toBeGreaterThan(
      context.retained.size,
    );
  });
  test("charges UTF-8 bytes of object keys against the individual node cap", () => {
    ...
    expect(
      terminal({ ...proposal, proof: { ...proposal.proof, nodes: [oversized] } }, context),
    ).toEqual({ kind: "rejected", code: "proof-byte-limit" });
  });
});
