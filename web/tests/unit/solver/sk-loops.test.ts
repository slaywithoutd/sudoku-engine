...
test("C30 all-double has complete eight-pair closure and checked effects",()=> {
 const f=fixtures.find(f=>f.id==="C30-all-double")! as unknown as TechniqueFixture,view=fixtureView(f),cursor=compileSkLoop(view,f.expectedPattern as unknown as SkPlan);
 let proposal;while(true){const n=cursor.next();if(n.done){proposal=n.value;break;}}
 expect(proposal).not.toBeNull();expect(proposal!.effects).toEqual(expect.arrayContaining(f.expectedEffects));
 const limits={...discoveryContext().limits,timeMs:180000,workUnits:200000000};let terminal;
...
test("C30 discovers and independently admits a real closed eight-group ring",()=> {
 ...
 expect(discoverSpecialized(f as unknown as TechniqueFixture,p=>p.linkMultiplicity===16).proposal.effects.length).toBeGreaterThan(0);
},200000);
