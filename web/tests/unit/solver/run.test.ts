...
describe("bounded solver phases",()=>{
  test("does not resume human discovery after exact fallback starts",async()=>{
    const published:Array<{kind?:string}>=[];let accepted=0;
    await runSolver({mode:"conditional",conditional:true,run:{operation:"conditional"},human:async()=>({human:"incomplete"})},{
      clock:{now:()=>0},yieldTask:async()=>{},publish:async (event:unknown)=>published.push(event as {kind?:string}),awaitAcceptance:async()=>"human-stopped"
    } as any);
    expect(accepted).toBe(0);expect(published.some(event=>event.kind==="terminal")).toBe(true);
  });
  test("runs a complete logical phase into independently exhausted exact evidence",async()=>{
    const problem=canonicalProblem({schema:1,cells:[0],symbols:[1],givens:[1],constraints:[]}),assembled=assemble(problem,[]);
    if(!assembled.ok)throw Error("fixture");
    const snapshot=makeSnapshot(problem,{kind:"manual"},"run-test",0),view=initialize(assembled.value,"primary"),workspace=new IndexWorkspace({entryLimit:100000,byteLimit:64000000});
    ...
    const run={requestId:"run-test",snapshotId:snapshot.snapshotId,inputRevision:0,problemKey:problem.key,operation:"primary" as const,mode:"explain" as const,engine:"engine@1",profile:"classic-expanded@1",scheduler:"scheduler@1",checker:"checker@1",exact:"original-dfs@1",optionsKey:"options",parentEvidenceId:null};
