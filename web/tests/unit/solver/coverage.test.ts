...
test("catalogues all 38 exact matrix rows and only independently accepted implementations", () => {
  ...
  expect(coverageEntries.filter(e => e.id > "C32").every(e => e.status === "specified")).toBe(true);
  expect(coverageEntries.filter(e=>e.id>="C25"&&e.id<="C32").every(e=>e.status==="implemented")).toBe(true);
...
test("partial catalogue cannot start an expanded run or silently stall as exhausted", () => {
  ...
  const future=getTechniques("classic-expanded@1").find(t=>t.id==="c33@1")!;
