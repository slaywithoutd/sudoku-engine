...
test("preserves exact UTF8 matrix bounds for every catalogue row", () => {
  const source = readFileSync(
    new URL(
      "../../../../docs/superpowers/specs/2026-09-12-m2-technique-coverage.md",
      import.meta.url,
    ),
    "utf8",
  );
  ...
  for (const line of rows) {
    ...
    const id = columns[0].slice(0, 3),
      direct = id.startsWith("C") && Number(id.slice(1)) <= 24;
    expect(coverageEntries.find((entry) => entry.id === id)?.bounds, id).toBe(
      columns[direct ? 3 : 2],
    );
  }
});
...
test("catalogues all 38 exact matrix rows and only independently accepted implementations", () => {
  ...
  expect(
    coverageEntries.filter((e) => e.id > "C33").every((e) => e.status === "independently-verified"),
  ).toBe(true);
  expect(coverageEntries.find((e) => e.id === "C33")?.status).toBe("independently-verified");
  expect(
    coverageEntries
      .filter((e) => e.id >= "C25" && e.id <= "C28")
      .every((e) => e.status === "independently-verified"),
  ).toBe(true);
  expect(validateCoverage(coverageEntries)).toEqual([]);
  expect(
    coverageEntries.filter((e) => e.status === "independently-verified").map((e) => e.id),
  ).toEqual([
    "C01",
    "C02",
    "C03",
...
test("reviewed profiles assemble bounded jobs while conditional authority remains separate", () => {
  const view = fixtureView(fixtureCase("C01-one-hole"));
  expect(assembleTechniqueJobs(view.assembly, "classic-expanded@1").techniques).toHaveLength(33);
  expect(assembleTechniqueJobs(view.assembly, "classic-conditional@1").techniques).toHaveLength(38);
  expect(assembleTechniqueJobs(view.assembly, "classic-expanded@1").rules).toHaveLength(
    view.assembly.problem.constraints.length,
  );
  const fake = {
    ...view.assembly,
    problem: {
      ...view.assembly.problem,
      constraints: Array(230).fill(view.assembly.problem.constraints[0]),
    },
  };
  ...
  expect([...future.discover(view, discoveryContext())]).toEqual([
...
test("refuses unsupported profile versions and verified rows lacking independent evidence", () => {
  ...
  const fake = coverageEntries.map((e, i) =>
    i
      ? e
      : {
          ...e,
          status: "independently-verified" as const,
  ...
  const unknown = coverageEntries.map((e, i) =>
    i ? e : { ...e, aliases: [...e.aliases, "invented technique alias"] },
  );
  expect(validateCoverage(unknown)).toContain("unknown-alias");
});

test("reviewed U rows retain complete per-alias evidence", () => {
  const proposed = coverageEntries.map((e) =>
    e.id.startsWith("U") ? { ...e, status: "independently-verified" as const } : e,
  );
  expect(validateCoverage(proposed)).toEqual([]);
  expect(
    coverageEntries
      .filter((e) => e.id.startsWith("U"))
      .every((e) => e.status === "independently-verified"),
  ).toBe(true);
});

test("reviewed C25-C28 retain complete per-alias evidence", () => {
  const reviewed = coverageEntries.filter((entry) => entry.id >= "C25" && entry.id <= "C28");
  expect(reviewed).toHaveLength(4);
  expect(reviewed.every((entry) => entry.status === "independently-verified")).toBe(true);
  expect(validateCoverage(coverageEntries)).toEqual([]);
});
