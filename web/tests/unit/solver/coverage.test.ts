...
test("catalogues all 38 exact matrix rows and only independently accepted implementations", () => {
  ...
  expect(coverageEntries.filter(e=>e.id>="C25"&&e.id<="C28").every(e=>e.status==="independently-verified")).toBe(true);
  expect(validateCoverage(coverageEntries)).toEqual([]);
  expect(coverageEntries.filter(e=>e.status==="independently-verified").map(e=>e.id)).toEqual(["C01","C02","C03","C04","C05","C06","C07","C08","C09","C10","C11","C12","C13","C14","C15","C16","C17","C18","C19","C20","C21","C22","C23","C24","C25","C26","C27","C28","C29","C30","C31","C32","C33","U01","U02","U03","U04","U05"]);
...
test("reviewed profiles assemble bounded jobs while conditional authority remains separate", () => {
  ...
  expect(assembleTechniqueJobs(view.assembly,"classic-expanded@1").rules).toHaveLength(view.assembly.problem.constraints.length);
...
test("reviewed C25-C28 retain complete per-alias evidence", () => {
  const reviewed = coverageEntries.filter(
