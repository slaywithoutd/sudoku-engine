...
test("catalogues all 38 exact matrix rows and only independently accepted implementations", () => {
  ...
  expect(coverageEntries.filter(e => e.id > "C13").every(e => e.status === "specified")).toBe(true);
  expect(validateCoverage(coverageEntries)).toEqual([]);
  expect(coverageEntries.filter(e=>e.status==="independently-verified").map(e=>e.id)).toEqual(["C01","C02","C03","C04","C05","C06","C07","C08","C09","C10","C11","C12","C13"]);
...
test("partial catalogue cannot start an expanded run or silently stall as exhausted", () => {
  ...
  const future=getTechniques("classic-expanded@1").find(t=>t.id==="c14@1")!;
