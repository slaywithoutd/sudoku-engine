...
const digit = (n: 2 | 5, corner=false): BoardAction => ({type:'digit',digit:n,corner});
...
test.each([[0,0,-1,8],[8,0,1,0],[0,-1,0,72],[72,1,0,0]])('wraps from %i', (selected,dr,dc,want) => {
  expect(reduceEditor(ctx,{...emptyEditor(),selected},{type:'move',dr,dc}).selected).toBe(want);
});
test('givens lock edits but permit selection; malformed moves and indices ignored', () => {
  const c={...ctx,givens:[7,...Array(80).fill(0)]}; const s=emptyEditor();
  for (const a of [digit(2),digit(2,true),{type:'erase'},{type:'reset'},{type:'select',index:81},{type:'move',dr:2,dc:0}] as BoardAction[]) expect(reduceEditor(c,s,a)).toBe(s);
...
test('no-op keeps redo, navigation is outside history, new edits discard redo', () => {
  ...
  s=reduceEditor(ctx,s,{type:'redo'});
  expect(s.selected).toBe(1); expect(s.cells[0].value).toBe(5);
  s=reduceEditor(ctx,s,{type:'undo'}); s=reduceEditor(ctx,s,digit(2)); expect(s.future).toEqual([]);
});
test('reset is one undoable edit, keeps selection/tool and never cleans peers', () => {
  let s=reduceEditor(ctx,emptyEditor(),digit(2,true));
  s=reduceEditor(ctx,s,{type:'select',index:1}); s=reduceEditor(ctx,s,digit(2,true)); s=reduceEditor(ctx,s,digit(5));
  expect(s.cells[0].notes).toEqual([2]);
  const prior=s; s=reduceEditor(ctx,s,{type:'reset'});
...
test('corner toggle sorts notes; creation always enters clues', () => {
  let s=reduceEditor(ctx,emptyEditor(),digit(5,true)); s=reduceEditor(ctx,s,digit(2,true));
  expect(s.cells[0].notes).toEqual([2,5]); s=reduceEditor(ctx,s,digit(5,true)); expect(s.cells[0].notes).toEqual([2]);
  expect(reduceEditor({...ctx,mode:'create'},emptyEditor(),digit(2,true)).cells[0]).toEqual({value:2,notes:[]});
});
