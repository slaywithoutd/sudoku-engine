...
for (const [width, height] of [
  ...
  test(`desktop visual acceptance ${width}x${height}`, async ({ page }, info) => {
    ...
    await expect(page.getByRole("button", { name: "Sudoku Engine", exact: true })).toBeVisible();
    ...
    const numbers = await page.getByRole("button", { name: "Number 1", exact: true }).boundingBox();
    ...
    await page
      .getByRole("button", { name: "Number 5", exact: true })
      .click({ modifiers: ["Shift"] });
    expect(await page.getByRole("grid").evaluate((e) => getComputedStyle(e).borderWidth)).toBe(
      "0px",
    );
