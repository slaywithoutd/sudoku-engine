...
test("opens the Solve screen with the same editing surface as Create", async ({ page }) => {
  ...
  await expect(page.getByRole("radio", { name: "Explain" })).toHaveAttribute("aria-checked", "true");
  ...
  await page.getByRole("button", { name: "Library", exact: false }).filter({ has: page.locator(".select-text") }).click();
...
test("explains a puzzle step by step with highlighted deductions", async ({ page }) => {
  ...
  await expect(page.locator(".game-solve")).toHaveAttribute("data-phase", "result", { timeout: 60_000 });
  ...
  const shown = await page.getByRole("grid").getByRole("gridcell").evaluateAll((nodes) =>
  ...
  await expect(page.getByTestId("clue-summary")).toHaveText(`${easy.replace(/0/g, "").length} clues`);
});
...
test("solves a full puzzle in a real worker with visible progress", async ({ page }) => {
  ...
  await page.getByRole("radio", { name: "Analyze" }).click();
  await expect(page.getByTestId("solver-verification")).toContainText("Exactly one solution, verified by exhaustive search.", { timeout: 150_000 });
