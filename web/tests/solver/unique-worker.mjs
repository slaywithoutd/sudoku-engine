...
try {
  const {ConditionalOperation,uniqueAuthorityMatches}=await import("../../src/solver/conditional.ts");
  ...
  const workspace=new IndexWorkspace({entryLimit:1_000_000,byteLimit:workerData.limits.workspaceBytes});
  const installation=ConditionalOperation.installTrustedBootstrap(workerData.port,workerData.expected,assembled.value,workerData.limits,workspace);
  ...
  const events=[];
  for(const proposal of workerData.bundles)for(const event of operation.checkAndCommit(proposal))
    if(event.kind!=="work")events.push(event.kind==="checked"?{kind:event.kind,conditional:event.step.consequences.every(c=>c.conditional)}:event);
  const values=operation.view.state.values,domains=operation.view.state.domains;
  operation.dispose();
