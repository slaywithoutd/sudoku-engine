...
export function specializedState(
  f: TechniqueFixture & { requiredPrefixSteps?: readonly { cell: number; symbol: number }[] },
) {
  const cached = cache.get(f.id);
  if (cached) return cached;
  if (!f.requiredPrefixSteps?.length) {
    const result = { view: fixtureView(f), prefix: originalCluePrefix(f) };
    cache.set(f.id, result);
    return result;
  }
  const values = [...f.givens].map(Number),
    domains = values.map((v, c) =>
      v
        ? 2 ** (v - 1)
              ...
              (s) =>
                !values.some(
                  (x, q) =>
                    x === s &&
                    q !== c &&
                    (Math.floor(c / 9) === Math.floor(q / 9) ||
                      c % 9 === q % 9 ||
                      (Math.floor(c / 27) === Math.floor(q / 27) &&
                        Math.floor((c % 9) / 3) === Math.floor((q % 9) / 3))),
                ),
            )
            .reduce((n, s) => n + 2 ** (s - 1), 0),
    );
  const before = { ...f, preState: { values, domains } },
    prefix = [...originalCluePrefix(before)];
  let view = fixtureView(before);
  for (const step of f.requiredPrefixSteps) {
    ...
    try {
      for (const e of cursor)
        if (
          e.kind === "proposal" &&
          e.proposal.effects.some(
            (x) => x.kind === "place" && x.cell === step.cell && x.symbol === step.symbol,
          )
        ) {
          proposal = e.proposal;
          break;
    ...
    if (!proposal) throw Error("missing-independent-singleton-prefix");
    let checked;
    ...
    prefix.push(checked.step.proposal);
  ...
  if (
    ...
    throw Error("independent-specialized-prefix-mismatch");
