...
export function fixtureCase(id: string): TechniqueFixture {
  const aliases: Record<string, string> = {
    "C04-naked-4": "C04-naked-4-row",
    "C05-triple": "C05-triple-row",
  };
...
export function fixtureView(fixture: TechniqueFixture): ReadView {
  ...
  if (cached) {
    if (
      JSON.stringify(cached.state.values) !== JSON.stringify(fixture.preState.values) ||
      JSON.stringify(cached.state.domains) !== JSON.stringify(fixture.preState.domains)
    )
      throw Error("independent-prefix-mismatch");
    return cached;
  }
  ...
  for (const rule of view.assembly.problem.constraints) {
    const proposals = [...view.assembly.modules.get(rule.id)!.propagate(view, rule)].filter(
      (e) => e.kind === "proposal",
    );
    for (const event of proposals)
      if (event.kind === "proposal") {
        ...
        if (terminal?.kind !== "checked")
          throw Error(`preamble-rejected:${JSON.stringify(terminal)}`);
        bundles.push(terminal.step.proposal);
        view = commitChecked(view, terminal.step).view;
      }
  }
  if (
    JSON.stringify(view.state.values) !== JSON.stringify(fixture.preState.values) ||
    JSON.stringify(view.state.domains) !== JSON.stringify(fixture.preState.domains)
  )
    throw Error("independent-prefix-mismatch");
...
export function fixturePrefix(id: string): readonly DeductionProposal[] {
  ...
  fixtureView(fixture);
  return prefixBundles.get(fixture.givens)!;
}
...
export function discoverFixture(id: string) {
  ...
  const detector = getTechniques("classic-expanded@1").find(
    (d) => d.id === `${fixture.rowId.toLowerCase()}@1`,
  )!;
  const events = [...detector.discover(view, discoveryContext())];
  const proposals = events.flatMap((e) =>
    e.kind === "proposal" &&
    JSON.stringify(e.proposal.pattern) === JSON.stringify(fixture.expectedPattern)
      ? [e.proposal]
      : [],
  );
  ...
  const outside =
    p.cells &&
    (fixture.rowId === "C03"
      ? p.cells.length < 2 || p.cells.length > 3
      : fixture.rowId === "C04"
        ? p.cells.length < 2 || p.cells.length > 4
        : fixture.rowId === "C05"
          ? p.cells.length < 2 || p.cells.length > 3
          : false);
  const status = proposals.length
    ? ("productive" as const)
    : outside
      ? ("out-of-profile" as const)
      : events.at(-1)?.kind === "exhausted"
        ? ("reject" as const)
  ...
  return {
    ...
    get proposal() {
      if (proposals.length !== 1)
        throw Error(`expected-one-productive-proposal:${id}:${proposals.length}`);
      return proposals[0];
    },
  };
}
...
export function fixtureCertificate(id: string): DeductionProposal {
  const fixture = fixtureCase(id),
    view = fixtureView(fixture),
    pattern = fixture.expectedPattern as Record<string, any>;
  const nodes: ProofNode[] = [],
    imports = new Set<number>(),
    roots: number[] = [];
  const prefix = retainedProof(view);
  ...
  const add = (rule: string, premises: number[], conclusion: Proposition) => {
    ...
    const id = next++;
    nodes.push({ id, rule, premises, conclusion, scope: [], parameters: {} });
    return id;
  };
  const lit = (cell: number, symbol: number, positive: boolean): Proposition => ({
    kind: "literal",
  ...
  const clause = (terms: { cell: number; symbol: number; positive: boolean }[]): Proposition => {
    ...
    return unique.length === 1
      ? { kind: "literal", value: unique[0] }
      : { kind: "clause", alternatives: unique };
  };
  const scopeFact = (
    kind: "cover" | "all-different",
    ...
    if (!fact) throw Error("fixture-premise-missing");
    return fact.root;
  };
  const proofs = new Map<string, number>();
  const effectRoot = (cell: number, symbol: number, root: number) => {
    roots.push(root);
    proofs.set(`${cell}:${symbol}`, root);
  };
  let positive: number | undefined;
  if (fixture.rowId === "C01")
    positive = add(
      "cover-clause@1",
      [view.state.domainFacts[pattern.cell]],
      lit(pattern.cell, pattern.symbol, true),
    );
  if (fixture.rowId === "C02" || fixture.rowId === "C03") {
    ...
    const cells = cover.cells.filter(
      (c) => Math.floor(view.state.domains[c] / 2 ** (cover.symbol - 1)) % 2 === 1,
    );
    const support = add(
      "support@1",
      [
        scopeFact("cover", cover.cells, cover.symbol),
        ...cover.cells.map((c) => view.state.domainFacts[c]),
      ],
      { kind: "cover", symbol: cover.symbol, cells },
    );
    const exhaustive = add(
      "cover-clause@1",
      [support],
      clause(cells.map((cell) => ({ cell, symbol: cover.symbol, positive: true }))),
    );
    if (fixture.rowId === "C02") positive = exhaustive;
      ...
      for (const effect of fixture.expectedEffects) {
        ...
        for (let index = 0; index < cells.length; index++) {
          const weak = add(
            "weak-link@1",
            [scopeFact("all-different", group.cells)],
            clause([
              { cell: cells[index], symbol: effect.symbol, positive: false },
          ...
          current = add(
            ...
            clause([
              ...cells
                .slice(index + 1)
                .map((cell) => ({ cell, symbol: effect.symbol, positive: true })),
              { cell: effect.cell, symbol: effect.symbol, positive: false },
            ]),
          );
        }
    ...
    for (const effect of fixture.expectedEffects) {
      if (effect.kind === "place") effectRoot(effect.cell, effect.symbol, positive);
        ...
        const house = view.assembly.allDifferent.find(
          (h) => h.cells.includes(pattern.cell) && h.cells.includes(effect.cell),
        )!;
        const weak = add(
          "weak-link@1",
          [scopeFact("all-different", house.cells)],
          clause([
            { cell: pattern.cell, symbol: effect.symbol, positive: false },
            { cell: effect.cell, symbol: effect.symbol, positive: false },
          ]),
        );
        effectRoot(
          effect.cell,
          effect.symbol,
          add("resolution@1", [weak, positive], lit(effect.cell, effect.symbol, false)),
        );
      }
    }
  if (fixture.rowId === "C04" || fixture.rowId === "C05") {
    for (const houseId of pattern.houses ?? [pattern.house]) {
      ...
      const selected: number[] =
        pattern.form === "hidden"
          ? house.cells.filter((c) => !pattern.cells.includes(c))
          : pattern.cells;
      for (const effect of fixture.expectedEffects)
        if (house.cells.includes(effect.cell) && !selected.includes(effect.cell)) {
          effectRoot(
            effect.cell,
            effect.symbol,
            add(
              ...
              lit(effect.cell, effect.symbol, false),
            ),
          );
        }
    }
  ...
  for (const effect of fixture.expectedEffects) {
    ...
    const bit = 2 ** (effect.symbol - 1),
      mask = effect.kind === "place" ? bit : prior.mask - bit;
    domains.set(effect.cell, {
      mask,
      root: add("domain-restrict@1", [prior.root, proofs.get(`${effect.cell}:${effect.symbol}`)!], {
        kind: "domain",
        cell: effect.cell,
        mask,
      }),
  ...
  return {
    technique: `${fixture.rowId.toLowerCase()}@1`,
    state: view.state.key,
    pattern: fixture.expectedPattern,
...
export function checkFixtureMutation(id: string, mutation: FixtureMutation) {
  const view = fixtureView(fixtureCase(id)),
    proposal = structuredClone(fixtureCertificate(id));
  const p = proposal as unknown as {
    pattern: Record<string, unknown>;
    proof: { nodes: ProofNode[]; imports: number[] };
    effects: { cell: number }[];
  };
  ...
  if (mutation === "undischarged")
    (p.proof.nodes[0] as unknown as { scope: number[] }).scope = [p.proof.nodes[0].id];
  if (mutation === "missing-import") p.proof.imports.pop();
  if (mutation === "wrong-domain")
    (p.proof.nodes.at(-1)!.conclusion as { mask: number }).mask = 511;
  if (mutation === "outside-bound") {
    if (Array.isArray(p.pattern.cells))
      p.pattern.cells = Array.from(
        { length: fixtureCase(id).rowId === "C04" ? 5 : 4 },
        (_, i) => i,
      );
...
function isClassic(view: ReadView): boolean {
  ...
  for (let i = 0; i < 9; i++) {
    ...
    scopes.push(
      Array.from(
        { length: 9 },
        (_, j) => (Math.floor(i / 3) * 3 + Math.floor(j / 3)) * 9 + (i % 3) * 3 + (j % 3),
      ).join(","),
    );
  }
  const actual = problem.constraints
    .map((rule) =>
      rule.type === "all-different@1" && Object.keys(rule.parameters ?? {}).length === 0
        ? [...rule.cells].sort((a, b) => a - b).join(",")
        : "unsupported",
    )
    .sort();
...
function finiteOracle(view: ReadView, input: OracleInput): OracleResult {
  const problem = view.assembly.problem,
    values = Array(problem.cells.length).fill(0) as number[];
  for (const rule of problem.constraints)
    if (!["all-different@1", "sum@1", "order@1"].includes(rule.type))
      throw Error(`No independent acceptance semantics for ${rule.type}`);
  const choices = problem.cells.map((cell) =>
    problem.symbols.filter(
      (symbol) =>
        (!input.givens[cell] || input.givens[cell] === symbol) &&
        Math.floor((input.domains?.[cell] ?? 2 ** problem.symbols.length - 1) / 2 ** (symbol - 1)) %
          2 ===
          1 &&
  ...
  const compatible = () =>
    problem.constraints.every((rule) => {
      ...
      if (rule.type === "all-different@1")
        return new Set(assigned.filter(Boolean)).size === assigned.filter(Boolean).length;
      ...
      const sum = assigned.reduce((a, b) => a + b, 0),
        missing = rule.cells.filter((cell) => !values[cell]);
  ...
  const visit = (depth: number): boolean => {
    ...
    if (depth === order.length) {
      ...
      if (witnesses.length === input.limit) {
        capped = true;
        return false;
      }
      return true;
    ...
    for (const symbol of choices[cell]) {
      values[cell] = symbol;
      if (!visit(depth + 1)) {
        values[cell] = 0;
        return false;
...
export function assertSound(
  ...
  if (terminal?.kind !== "checked")
    throw Error(
      `Proof rejected: ${terminal?.kind === "rejected" ? terminal.code : "incomplete checker"}`,
    );
  if (terminal.step.consequences.some((item) => item.conditional || item.openAssumptions.length))
    throw Error("Conditional acceptance is unsupported");
...
export function assertCertificateSound(
  view: ReadView,
  proposal: DeductionProposal,
  options: { oracleMaxNodes?: number; limits?: Limits } = {},
) {
  ...
  if (terminal?.kind !== "verified")
    throw Error(
      `Certificate rejected: ${terminal?.kind === "rejected" ? terminal.code : "incomplete checker"}`,
    );
  assertEffectsSound(view, proposal, options.oracleMaxNodes);
  return terminal.certificate;
}
function assertEffectsSound(
  view: ReadView,
  proposal: DeductionProposal,
  oracleMaxNodes?: number,
): void {
  const input: OracleInput = {
    givens: [...view.state.values],
    domains: [...view.state.domains],
    limit: 1,
    maxNodes: oracleMaxNodes ?? 1000000,
  };
  const run = isClassic(view) ? oracle : (query: OracleInput) => finiteOracle(view, query);
  const pre = run(input);
  if (pre.interrupted)
    throw Error("Acceptance oracle interrupted before establishing satisfiability");
  ...
  for (const effect of proposal.effects) {
    ...
    const result = run({
      ...input,
      ...(effect.kind === "remove" ? { force: restriction } : { forbid: restriction }),
    });
    if (result.interrupted) throw Error("Acceptance counterfactual oracle interrupted");
    if (!result.exhausted || result.witnesses.length !== 0)
      throw Error("Effect has a counterexample or lacks exhaustive refutation");
  }
}
