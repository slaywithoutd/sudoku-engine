...
export const uniqueLimits = {
  timeMs: 60000,
  workUnits: 100_000_000,
  exactNodes: 1_000_000,
  stepNodes: 8192,
  runNodes: 250000,
  proofBytes: 64_000_000,
  stepBytes: 2_000_000,
  batchBytes: 65536,
  inFlightBatches: 2,
  workspaceBytes: 256_000_000,
};

/** Real exact/merge/check/commit only. Fixture witnesses never enter this harness. */
export function uniqueHarness(
  seed: UniqueSeed,
  extra?: { constraints: readonly ConstraintInstance[]; modules: readonly RuleModule[] },
) {
  const classic = normalizeClassic({
    kind: "classic",
    version: 1,
    width: 9,
    height: 9,
  ...
  const problem = extra
    ? canonicalProblem({
        schema: classic.schema,
        cells: classic.cells,
        symbols: classic.symbols,
        givens: classic.givens,
        constraints: [...classic.constraints, ...extra.constraints],
      })
    : classic;
  ...
  const assembly = assembled.value,
    snapshot = makeSnapshot(assembly.problem, { kind: "manual" }, seed.id, 0);
  const run: RunKey = {
    requestId: `${seed.id}:primary`,
    snapshotId: snapshot.snapshotId,
    inputRevision: 0,
    problemKey: assembly.problem.key,
    operation: "primary",
    mode: "explain",
    engine: "engine@1",
  ...
  const initialView = initialize(assembly, "primary");
  ...
  let raw: CountEvidence | undefined,
    work = 0;
  ...
  if (merged.count.kind !== "unique" || merged.diagnostics.length)
    throw Error("unique-test-merge-rejected");
  const commit = (proposal: DeductionProposal) => {
    ...
    accepted.push(result.step);
    view = commitChecked(view, result.step).view;
  };
  for (const rule of assembly.problem.constraints)
    for (const event of assembly.modules.get(rule.id)!.propagate(view, rule))
  ...
  for (const op of seed.prefix ?? []) {
    ...
    if (op.kind === "forcing" || op.kind === "forcing-placement") {
      commit(
        compileForcing(
          view,
          { ...op.expectedPattern, alias: "Digit forcing chains" } as ForcingPlan,
          op.expectedEffects[0],
        ),
      );
    } else {
      ...
      try {
        for (const event of cursor)
          if (
            event.kind === "proposal" &&
            (op.kind === "locked"
              ? event.proposal.effects.length === op.removed.length &&
                event.proposal.effects.every(
                  (e) =>
                    e.kind === "remove" && e.symbol === op.symbol && op.removed.includes(e.cell),
                )
              : event.proposal.effects.some(
                  (e) => e.kind === "place" && e.cell === op.cell && e.symbol === op.symbol,
                ))
          ) {
      ...
      if (op.kind === "single-placement") {
        if (
          view.state.domains[op.cell] !== 1 << (op.symbol - 1) ||
          JSON.stringify([...proposal.effects].sort((a, b) => a.cell - b.cell)) !==
            JSON.stringify([...op.expectedEffects].sort((a, b) => a.cell - b.cell))
        )
  ...
  if (
    JSON.stringify(view.state.values) !== JSON.stringify(seed.preState.values) ||
    JSON.stringify(view.state.domains) !== JSON.stringify(seed.preState.domains)
  )
  ...
  const conditionalRun: RunKey = {
    ...run,
    requestId: `${seed.id}:conditional`,
  ...
  for (const event of operation.rebuildPrefix())
    if (event.kind === "rejected") throw Error(`unique-rebound-prefix:${event.code}`);
